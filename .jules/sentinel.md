...
## 2026-01-30 - Secure Edge Function Authentication
**Vulnerability:** Unauthenticated RCE in `aws-manager` allowed arbitrary domain registration via AWS SDK.
**Learning:** Edge Functions using `serve` do not automatically validate Supabase Auth tokens. Relying on obscure URLs or client-side checks is insufficient. Also, returning `error.stack` leaks internal paths and logic.
**Prevention:** Always verify `Authorization` header using `supabase.auth.getUser()` at the start of any sensitive Edge Function. Catch errors and return sanitized messages (e.g. `error.message` or generic text), never `error.stack`.
