...
@Component({
  ...
  template: `
    <div class="fixed inset-0 z-[99999] bg-black flex flex-col">
      <!-- Header -->
      <div
        class="absolute top-0 left-0 right-0 z-[60] flex justify-between items-center p-4 bg-gradient-to-b from-black/50 to-transparent pointer-events-none"
      >
        <button
          (click)="close.emit()"
          class="pointer-events-auto text-white p-3 rounded-full bg-black/20 backdrop-blur-md hover:bg-white/10 transition-colors"
          title="Cerrar"
        >
        ...
        <div class="flex gap-4 pointer-events-auto">
          ...
          <button
            ...
            class="text-white p-3 rounded-full bg-black/20 backdrop-blur-md hover:bg-white/10 transition-colors"
      ...
      <div class="flex-1 relative flex items-center justify-center bg-black overflow-hidden">
        ...
        @if (loading() || error()) {
          <div class="absolute inset-0 flex items-center justify-center bg-black/90 z-50 px-6">
            <div class="text-center max-w-sm">
              ...
              <h3 class="text-white text-xl font-bold mb-3">
                {{ error() ? 'No se puede acceder a la cámara' : 'Iniciando Cámara...' }}
              </h3>
              ...
              @if (error()) {
                <div class="flex flex-col gap-3">
                  <button
                    (click)="initCamera()"
                    class="px-6 py-3 bg-blue-600 active:bg-blue-700 text-white rounded-xl font-medium transition-colors"
                  >
                    Reintentar
                  </button>
                  <button
                    (click)="fileInput.click()"
                    class="px-6 py-3 bg-gray-700 active:bg-gray-600 text-white rounded-xl font-medium transition-colors"
                  >
        ...
        @if (!error() && !loading()) {
          <div
            class="absolute inset-0 pointer-events-none border-2 border-white/20 m-8 rounded-2xl z-10"
          >
            <div
              class="absolute top-0 left-0 w-8 h-8 border-t-4 border-l-4 border-white/50 rounded-tl-xl"
            ></div>
            <div
              class="absolute top-0 right-0 w-8 h-8 border-t-4 border-r-4 border-white/50 rounded-tr-xl"
            ></div>
            <div
              class="absolute bottom-0 left-0 w-8 h-8 border-b-4 border-l-4 border-white/50 rounded-bl-xl"
            ></div>
            <div
              class="absolute bottom-0 right-0 w-8 h-8 border-b-4 border-r-4 border-white/50 rounded-br-xl"
            ></div>
          </div>
        }
      </div>
...
export class CameraCaptureComponent implements OnDestroy, AfterViewInit {
  ...
  async checkContextAndInit() {
    ...
    if (!window.isSecureContext) {
      ...
      this.error.set(
        'Seguridad del Navegador: La cámara requiere acceso HTTPS o Localhost. Si estás probando en móvil por IP, esto no funcionará. Usa la opción "Usar Galería".',
      );
      return;
    }
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      this.loading.set(false);
      this.error.set(
        'Tu navegador no soporta acceso a la cámara. Intenta usar otro navegador o subir desde galería.',
      );
      return;
    }
