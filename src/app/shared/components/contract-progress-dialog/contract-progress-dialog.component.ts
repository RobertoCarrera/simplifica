  ...
  template: `
    ...
    @if (visible()) {
      ...
      >
        <!-- Modal Container -->
        <div
          class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md transform transition-all animate-modal-appear"
          role="dialog"
          aria-modal="true"
        ...
        >
              ...
              @for (step of steps(); track step; let i = $index) {
                <div class="flex items-start gap-4">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    @if (step.status === 'in-progress') {
                      <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Procesando...</p>
                    }
                    @if (step.errorMessage) {
                      <p class="text-sm text-red-500 dark:text-red-400 mt-0.5">
                        {{ step.errorMessage }}
                      </p>
                    }
                  </div>
          ...
          <!-- Actions -->
          <div
            class="p-6 border-t border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-700/50 rounded-b-2xl"
          >
            ...
            @if (paymentUrl() && paymentOptions().length === 0) {
              <button
                (click)="goToPayment()"
            ...
            @if (isComplete() && !paymentUrl() && paymentOptions().length === 0) {
              <button
                (click)="close()"
...
export class ContractProgressDialogComponent {
  ...
  isComplete = computed(() => {
    ...
    return (
      ...
      currentSteps.every((s) => s.status === 'completed' || s.status === 'error')
    );
  });
  ...
  updateStep(stepId: string, status: ContractProgressStep['status'], errorMessage?: string) {
    this.steps.update((steps) =>
      steps.map((s) => (s.id === stepId ? { ...s, status, errorMessage } : s)),
    );
  }
  ...
  completeSuccess(result: ContractResult) {
    ...
    this.steps.update((steps) => steps.map((s) => ({ ...s, status: 'completed' as const })));
    ...
    if (result.paymentOptions && result.paymentOptions.length > 0) {
      ...
      this.resultMessage.set(
        result.message || '¡Todo listo! Selecciona cómo quieres realizar el pago.',
      );
    } else if (result.paymentUrl) {
      this.paymentUrl.set(result.paymentUrl);
      this.resultMessage.set(
        result.message || '¡Todo listo! Haz clic en el botón para completar el pago.',
      );
    } else {
      this.resultMessage.set(
        result.message ||
          'El servicio ha sido contratado correctamente. Te contactaremos para el pago.',
      );
    }
  }
  ...
  completeError(stepId: string, errorMessage: string, fallbackMessage?: string) {
    ...
    this.steps.update((steps) =>
      steps.map((s, i) => (i > errorIndex ? { ...s, status: 'pending' as const } : s)),
    );

    this.resultMessage.set(
      fallbackMessage || 'Se ha producido un error. Por favor, contacta con nosotros.',
    );
  }
  ...
  completeFallback(message: string) {
    ...
    this.steps.update((steps) =>
      steps.map((s) => (s.id === 'payment' ? { ...s, label: 'Contacto configurado' } : s)),
    );
