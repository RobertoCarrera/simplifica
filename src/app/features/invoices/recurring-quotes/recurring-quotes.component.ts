                  ...
                  <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    <td class="px-4 py-3">
                      <div class="flex items-center">
                        <span
                          class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-xs font-bold mr-3"
                    ...
                    <td class="px-4 py-3">
                      @if (quote.next_run_at) {
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400">
                          {{ getRelativeDate(quote.next_run_at) }}
                        </p>
                      } @else {
                    ...
                    <td class="px-4 py-3 text-right">
                      <span class="text-sm font-medium text-gray-900 dark:text-gray-100">
                        {{ getDisplayAmount(quote) | number: '1.2-2' }}
                        {{ quote.currency || 'EUR' }}
                      </span>
                    </td>
                    <td class="px-4 py-3 text-right">
...
export class RecurringQuotesComponent {
  ...
  async viewHistory(quote: RecurringQuote): Promise<void> {
    ...
    try {
      ...
      const { data, error } = await client
        .from('invoices')
        .select(
          'id, invoice_number, invoice_series, invoice_date, total, status, recurrence_period',
        )
  ...
  async pauseRecurrence(quote: RecurringQuote): Promise<void> {
    ...
    try {
      const client = this.supabase.instance;
      await client.from('quotes').update({ status: 'paused' }).eq('id', quote.id);
  ...
  async resumeRecurrence(quote: RecurringQuote): Promise<void> {
    try {
      ...
      await client.from('quotes').update(updates).eq('id', quote.id);
  ...
  async cancelRecurrence(quote: RecurringQuote): Promise<void> {
    ...
    try {
      ...
      if (hasInvoices) {
        ...
        await client.from('invoices').delete().eq('source_quote_id', quote.id);
      }

      // Eliminar items del presupuesto
      await client.from('quote_items').delete().eq('quote_id', quote.id);
  ...
  async saveRecurrenceDay(): Promise<void> {
    ...
    try {
      ...
      const { error } = await client.from('quotes').update(updateData).eq('id', quote.id);
