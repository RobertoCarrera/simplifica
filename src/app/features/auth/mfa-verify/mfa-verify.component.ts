      ...
      >
        <div class="text-center mb-6">
          ...
          <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
            Tu cuenta requiere verificación adicional para acceder a esta
            sección.
          </p>
        </div>
...
export class MfaVerifyComponent implements OnInit, OnDestroy {
  ...
  async verify() {
    ...
    try {
      ...
      const { error } =
        await this.authService.client.auth.mfa.challengeAndVerify({
          factorId,
          code: this.code,
        });
        ...
        if (this.failedAttempts >= MfaVerifyComponent.MAX_ATTEMPTS) {
          const lockoutMs =
            ...
            Math.pow(
              2,
              Math.floor(
                this.failedAttempts / MfaVerifyComponent.MAX_ATTEMPTS,
              ) - 1,
            );
