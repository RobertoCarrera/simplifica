...
@Component({
  ...
  template: `
    <div class="space-y-4">
      <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <div class="flex items-center gap-2 w-full sm:w-auto">
          <div class="relative flex-1 sm:w-64">
            ...
            />
            <i
              class="fas fa-search absolute left-3 top-2 text-gray-400 dark:text-gray-500 text-xs"
            ></i>
            @if (searchTerm()) {
              <button
      ...
      <div
        class="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-slate-800 shadow-sm"
      >
        <table class="w-full text-sm text-left">
          <thead
            class="bg-gray-50 dark:bg-slate-700/50 text-gray-500 dark:text-gray-400 uppercase text-xs"
          >
            <tr>
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            @for (log of paginatedLogs(); track log) {
              <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                ...
                <td class="px-6 py-4">
                  <span
                    [ngClass]="getActionClass(log.action_type)"
                    class="px-2 py-1 rounded-full text-xs font-medium border dark:border-transparent"
                  >
                ...
                <td class="px-6 py-4 font-medium text-gray-900 dark:text-gray-100">
                  <div class="flex flex-col">
                    ...
                    <span class="text-xs text-gray-500 dark:text-gray-400">{{
                      log.subject_email || 'Email no registrado'
                    }}</span>
                  </div>
                </td>
            ...
            @if (paginatedLogs().length === 0 && !loading()) {
              <tr>
                <td
                  colspan="5"
                  class="px-6 py-12 text-center text-gray-500 dark:text-gray-400 italic bg-gray-50/30 dark:bg-slate-800/50"
                >
      ...
      @if (totalPages() > 1) {
        <div
          class="px-6 py-4 bg-white dark:bg-slate-800 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between rounded-lg shadow-sm"
        >
          ...
          <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p class="text-sm text-gray-700 dark:text-gray-300">
                ...
                de <span class="font-medium">{{ filteredLogs().length }}</span> resultados
              </p>
            </div>
            <div>
              <nav
          ...
          <div class="p-6 overflow-y-auto">
            ...
            <div class="bg-slate-900 rounded-lg p-4 overflow-x-auto relative group">
              ...
              >
                <button
                  (click)="copyJson()"
                  class="text-xs bg-slate-700 text-slate-300 px-2 py-1 rounded hover:bg-slate-600"
                >
                  Copiar
                </button>
              </div>
              <pre class="text-xs text-green-400 font-mono leading-relaxed">{{
                getFormattedJson(selectedLog()!)
              }}</pre>
            </div>
          </div>
          <!-- Footer -->
          <div
            class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-slate-900/50 flex justify-end"
          >
            <button
              (click)="closeDetails()"
              class="px-4 py-2 bg-gray-200 dark:bg-slate-700 text-gray-800 dark:text-gray-200 rounded-lg font-medium hover:bg-gray-300 dark:hover:bg-slate-600 transition-colors"
            >
              Cerrar
            </button>
          </div>
