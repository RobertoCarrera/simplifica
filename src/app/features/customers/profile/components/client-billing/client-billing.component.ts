...
@Component({
  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        >
          <i
            ...
            [class.fa-spinner]="isCreating()"
            [class.fa-spin]="isCreating()"
          ></i>
          {{
            isCreating()
              ? 'Creando...'
      ...
      <!-- Content Area -->
      <div
        class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden min-h-[300px]"
      >
        <!-- Loading -->
        @if (isLoading()) {
          <div class="p-6">
        ...
        <!-- Invoices List -->
        @if (!isLoading() && activeTab() === 'invoices' && invoices().length > 0) {
            ...
            @for (invoice of invoices(); track invoice) {
              ...
              >
                <div class="flex items-center gap-4">
                  <div
                    class="w-10 h-10 rounded-full bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 flex items-center justify-center flex-shrink-0"
                  >
                    <i class="fas fa-file-invoice-dollar"></i>
                  </div>
                  <div>
                    <h4 class="text-sm font-bold text-gray-900 dark:text-white">
                      {{ invoice.full_invoice_number || invoice.invoice_number }}
                    </h4>
                ...
                <div class="flex items-center gap-6 sm:ml-auto">
                  <div class="text-right">
                    ...
                    <span
                      ...
                      [ngClass]="getInvoiceStatusClass(invoice.status)"
                    >
                  ...
                  <div class="flex gap-2">
                    <button
                      ...
                      class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 text-sm font-medium mr-2"
                    >
                      Ver
                    </button>
                    <button
        ...
        <!-- Quotes List -->
        @if (!isLoading() && activeTab() === 'quotes' && quotes().length > 0) {
          <div class="divide-y divide-gray-100 dark:divide-slate-700">
            @for (quote of quotes(); track quote) {
              ...
              >
                <div class="flex items-center gap-4">
                  <div
                    class="w-10 h-10 rounded-full bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 flex items-center justify-center flex-shrink-0"
                  >
                    <i class="fas fa-file-contract"></i>
                  </div>
                  <div>
                    <h4 class="text-sm font-bold text-gray-900 dark:text-white">
                      {{ quote.full_quote_number || 'Borrador' }}
                    </h4>
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                      {{ quote.quote_date | date: 'mediumDate' }}
                    </p>
                  </div>
                </div>
                <div class="flex items-center gap-6 sm:ml-auto">
                  <div class="text-right">
                    <span class="block text-sm font-bold text-gray-900 dark:text-white">{{
                      quote.total_amount | currency: 'EUR'
                    }}</span>
                    <span
                      class="px-2 py-0.5 rounded text-xs font-medium"
                      [ngClass]="getQuoteStatusClass(quote.status)"
                    >
  ...
  getQuoteStatusClass(status: QuoteStatus) {
    switch (status) {
      ...
      case QuoteStatus.REJECTED:
        return 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400';
      default:
        return 'bg-purple-50 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400';
    }
  }
