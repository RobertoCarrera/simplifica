...
@Component({
  ...
  template: `
    <div class="secure-notes-container">
      <!-- Header / Add Note -->
      <div
        class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-4 mb-6 relative overflow-hidden"
      >
        ...
        <h3
          class="flex items-center gap-2 text-lg font-bold text-slate-800 dark:text-white mb-4 relative z-10"
        >
          <div
      ...
      <div class="space-y-4">
        ...
        @for (note of notes(); track note) {
          ...
          >
            <!-- Bullet -->
            <div
              class="absolute left-[-9px] top-0 w-4 h-4 rounded-full bg-slate-200 dark:bg-slate-700 border-2 border-white dark:border-slate-800"
            ></div>
            <div
              class="bg-white dark:bg-slate-800 rounded-lg p-4 shadow-sm border border-slate-100 dark:border-slate-700 group hover:border-emerald-200 dark:hover:border-emerald-900/30 transition-colors"
            >
              ...
              <div class="relative">
                ...
                @if (!revealedNotes.has(note.id)) {
                  ...
                  >
                    <div
                      class="px-3 py-1 bg-slate-900/80 text-white text-xs rounded-full flex items-center gap-1 shadow-lg backdrop-blur-md"
                    >
    ...
    this.notesService.getNotes(this.clientId).subscribe({
      next: (data) => {
        ...
        this.gdprService.logGdprEvent(
          ...
          this.clientId,
          undefined,
          'User accessed clinical notes timeline',
        );
      },
      error: (err) => {
