    ...
    <div class="gdpr-panel w-full space-y-8">
      <div class="gdpr-panel w-full space-y-8">
        ...
        @if (!loading && !error) {
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="space-y-6">
              ...
              >
                <h4
                  class="text-base font-semibold text-gray-900 dark:text-white mb-6 flex items-center gap-2"
                >
                  <div
                    class="p-2 rounded-lg bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400"
                  >
                ...
                <div class="space-y-4">
                  ...
                  <!-- Last Update -->
                  <div
                    class="flex items-center gap-2 text-xs text-gray-400 mt-4 pt-4 border-t border-gray-100 dark:border-gray-700"
                  >
            ...
            <div class="space-y-6">
              ...
              <div
                class="bg-blue-50 dark:bg-blue-900/10 rounded-xl p-6 border border-blue-100 dark:border-blue-800"
              >
                <h4
                  class="text-base font-semibold text-blue-900 dark:text-blue-100 mb-2 flex items-center gap-2"
                >
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                      stroke-linecap="round"
              ...
              >
                ...
                >
                  <div
                    class="p-2 rounded-lg bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-300"
                  >
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path
                        stroke-linecap="round"
                ...
                <div class="space-y-3">
                  ...
                  >
                    <div class="flex items-center gap-3">
                      <div
                        class="p-2 bg-gray-50 dark:bg-gray-700 rounded text-gray-500 group-hover:text-blue-600"
                      >
                        @if (!exporting) {
                          <svg
                            class="w-4 h-4"
        ...
        @if (showAnonymizeModal) {
          <div
            class="fixed inset-0 bg-gray-600 bg-opacity-75 overflow-y-auto h-full w-full z-[10000] flex items-center justify-center modal-backdrop"
            (click)="closeAnonymizeModal()"
          >
            <div
              class="relative p-5 border w-11/12 md:w-1/3 rounded-lg bg-white dark:bg-slate-800 dark:border-slate-600 modal-content-box"
            ...
            >
              <div class="mt-3 text-center">
                ...
                <div
                  class="mt-2 px-4 py-2 bg-red-50 dark:bg-red-900/10 rounded text-left border border-red-100 dark:border-red-800"
                >
                  <p class="text-sm text-red-800 dark:text-red-300">
                    Estás a punto de anonimizar los datos de
                    <span class="font-bold">{{ clientName }}</span
                    >.
                  </p>
                  <ul
                    class="list-disc list-inside text-xs text-red-700 dark:text-red-400 mt-2 space-y-1"
                  >
...
export class ClientGdprPanelComponent implements OnInit {
  ...
  loadGranularConsents() {
    this.gdprService.getConsentRecords(this.clientEmail).subscribe({
      next: (records) => {
        ...
        const privacy = records
          .filter((r) => r.consent_type === 'privacy_policy')
          .sort(
            (a, b) => new Date(b.created_at || 0).getTime() - new Date(a.created_at || 0).getTime(),
          )[0];
        const marketing = records
          .filter((r) => r.consent_type === 'marketing')
          .sort(
            (a, b) => new Date(b.created_at || 0).getTime() - new Date(a.created_at || 0).getTime(),
          )[0];
  ...
  private getPurposeLabel(type: string): string {
    switch (type) {
      ...
      case 'privacy_policy':
        return 'Política de Privacidad y Términos';
      case 'marketing':
        return 'Comunicaciones Comerciales';
      default:
        return type;
    }
  }
