      ...
      >
        ...
        >
          ...
          @if (config.type === 'rectification') {
            <div class="mb-6">
              <div class="space-y-4">
                @for (field of rectificationFields; track field) {
                  <div
                  ...
                  >
                    ...
                    @if (field.selected) {
                      <div class="ml-7 mt-3 space-y-4 animate-fadeIn">
                        ...
                        @if (field.key === 'address') {
                          <div
                            class="space-y-3 bg-white dark:bg-slate-900/50 p-4 rounded-xl border dark:border-slate-700 shadow-inner"
                          >
                            <div class="relative">
                              <label
                              ...
                              @if (viaDropdownOpen) {
                                <div
                                  class="absolute z-50 top-full left-0 w-full bg-white dark:bg-gray-800 border dark:border-slate-700 rounded-lg shadow-xl mt-1 max-h-48 overflow-y-auto ring-1 ring-black/5"
                                >
                                  @for (via of filteredVias; track via) {
                                    <div
                                      (click)="selectVia(via)"
                            ...
                            <div class="relative">
                              ...
                              @if (localityDropdownOpen) {
                                <div
                                  class="absolute z-50 top-full left-0 w-full bg-white dark:bg-gray-800 border dark:border-slate-700 rounded-lg shadow-xl mt-1 max-h-56 overflow-y-auto ring-1 ring-black/5"
                                >
                                  @for (loc of filteredLocalities; track loc) {
                                    ...
                                    >
                                      <div class="flex flex-col">
                                        <span
                                          class="text-sm font-semibold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400"
                                          >{{ loc.nombre }}</span
                                        >
                            ...
                            @if (formData.addressLocalidadId && !localityDropdownOpen) {
                              <div
...
export class GdprRequestModalComponent implements OnInit, OnDestroy {
  ...
  onAddressViaInput(event: Event) {
    ...
    if (!v) {
      ...
      this.filteredVias = this.addressVias.filter((v_str) =>
        v_str.toLowerCase().includes(v.toLowerCase()),
      );
    }
