  ...
  template: `
    ...
    @if (visible()) {
      ...
      >
        <!-- Modal Container -->
        <div
          class="bg-slate-800 rounded-3xl shadow-2xl w-full max-w-md transform transition-all animate-modal-appear border border-slate-700 overflow-hidden text-center relative"
        >
          <!-- Close Button -->
          <button
            (click)="cancel()"
          ...
          <div class="pt-8 pb-4 px-6 flex flex-col items-center">
            <div
              class="w-12 h-12 rounded-full bg-emerald-500/20 text-emerald-500 flex items-center justify-center mb-0 opacity-0 hidden"
            ></div>
            <!-- keeping structure but hidden if needed -->
            <h3 class="text-2xl font-bold text-white flex items-center gap-2">
              <div
                class="w-7 h-7 bg-emerald-500 rounded flex items-center justify-center text-slate-900 text-sm"
              >
            ...
            @if (displayInvoiceNumber()) {
              <p class="text-gray-400 mt-1">Factura {{ displayInvoiceNumber() }}</p>
            }
            @if (!displayInvoiceNumber() && subtitle()) {
              <p class="text-gray-400 mt-1">{{ subtitle() }}</p>
            }
            @if (isRecurring()) {
              <div class="mt-2 text-sm text-gray-500">
                Suscripción:
                <span class="text-white font-bold">{{ amount | currency: 'EUR' }}</span> /
                {{ billingPeriod() }}
              </div>
            }
            @if (!isRecurring()) {
          ...
          <div class="px-6 mb-6">
            <div
              class="bg-slate-900/50 rounded-xl p-4 flex items-center gap-4 text-left border border-slate-700"
            >
              <div
                class="w-10 h-10 rounded-full bg-emerald-900/40 text-emerald-500 flex-shrink-0 flex items-center justify-center"
              >
                <i class="fas fa-check"></i>
              </div>
              <div>
          ...
          <div class="px-6 mb-4 text-left">
            <div
              class="bg-emerald-900/20 text-emerald-400 p-4 rounded-lg text-sm border border-emerald-900/50"
            >
          ...
          <div class="p-6 pt-0 space-y-3">
            ...
            @if (hasStripe()) {
              ...
              >
                ...
                <div class="z-10">
                  <i
                    class="fas fa-arrow-right text-white/70 group-hover:text-white group-hover:translate-x-1 transition-all"
                  ></i>
                </div>
              </button>
            }
            ...
            @if (hasPayPal()) {
              ...
              >
                ...
                <div class="z-10 font-bold text-lg flex-1 text-left pl-2">Pagar con PayPal</div>
                <div class="z-10">
                  <i
                    class="fas fa-arrow-right text-white/70 group-hover:text-white group-hover:translate-x-1 transition-all"
                  ></i>
          ...
          <div class="pb-6 px-6 text-xs text-slate-500">Selecciona tu método de pago preferido</div>
        </div>
      </div>
    }
  `,
