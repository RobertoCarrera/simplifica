      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div class="p-4 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center bg-gray-50 dark:bg-slate-800/50">
          <div class="flex items-center gap-4 flex-1">
            <h2 class="text-xl font-bold text-gray-900 dark:text-white">Generar Documento Profesional</h2>
            ...
            <input 
              ...
              placeholder="Título del documento (ej. Contrato Laboral)...">
          </div>
        ...
        <div class="p-4 border-t border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-800 flex justify-between items-center">
          ...
          <div class="flex gap-3">
            ...
            <button 
              ...
              <i class="fas fa-file-export" *ngIf="!isSaving()"></i>
              <i class="fas fa-spinner fa-spin" *ngIf="isSaving()"></i>
              {{ isSaving() ? 'Procesando...' : 'Generar Documento' }}
            </button>
          </div>
        </div>
...
export class ProfessionalContractDialogComponent {
  ...
  professionalService = inject(SupabaseProfessionalsService);
  ...
  // Sidebar items
  variables = [
    { label: 'Nombre Profesional', value: '{{professional_name}}' },
    { label: 'Fecha Actual', value: '{{today_date}}' },
    { label: 'Empresa', value: '{{company_name}}' }, // Placeholder for future
    { label: 'Cargo', value: '{{role}}' }, // Placeholder
  ];

  blocks = [
    { label: 'Título Grande', icon: 'fas fa-heading', content: '<h1 data-label="Título H1">Título del Documento</h1>' },
    { label: 'Subtítulo', icon: 'fas fa-font', content: '<h2 data-label="Subtítulo">Sección Importante</h2>' },
    { label: 'Párrafo', icon: 'fas fa-paragraph', content: '<p data-label="Párrafo">Escribe aquí el contenido...</p>' },
    { label: 'Lista', icon: 'fas fa-list-ul', content: '<ul data-label="Lista"><li>Cláusula 1</li><li>Cláusula 2</li></ul>' },
    { label: 'Firma', icon: 'fas fa-signature', content: '<div data-label="Firma" style="margin-top: 50px; border-top: 1px solid #ccc; width: 200px; padding-top: 10px;">Firma del Empleado</div>' },
  ];
  ...
  // Drag and Drop Logic (Copied from Client version)
  ...
  showPlaceholder(event: DragEvent) {
    ...
    if (!this.dragPlaceholder) {
      ...
      this.dragPlaceholder.style.border = '2px dashed #93c5fd';
      this.dragPlaceholder.style.height = '40px';
      this.dragPlaceholder.style.margin = '12px 0';
      this.dragPlaceholder.style.borderRadius = '8px';
      this.dragPlaceholder.style.backgroundColor = 'rgba(219, 234, 254, 0.4)';
  ...
  onDrop(event: DragEvent) {
    ...
    if (this.dragPlaceholder && this.dragPlaceholder.parentElement === editorEl) {
      ...
      if (!insertBeforeNode) appendToEnd = true;
    }

    this.removePlaceholder();

    if (this.draggedElement) {
      if (appendToEnd) editorEl.appendChild(this.draggedElement);
      else if (insertBeforeNode) editorEl.insertBefore(this.draggedElement, insertBeforeNode);
      else editorEl.appendChild(this.draggedElement);
    ...
    if (content) {
      ...
      if (newElement) {
        if (appendToEnd) editorEl.appendChild(newElement);
        else if (insertBeforeNode) editorEl.insertBefore(newElement, insertBeforeNode);
        else editorEl.appendChild(newElement);
        this.onContentChange();
      } else {
  ...
  replacePlaceholders(content: string): string {
    ...
    processed = processed.replace(new RegExp('{{professional_name}}', 'g'), this.professionalName || 'Profesional');
  ...
  async createDocument() {
    ...
    const file = new File([htmlBlob], `${this.contractTitle}.html`, { type: 'text/html' });

    try {
        await this.professionalService.uploadProfessionalDocument(this.professionalId, file, 'contract');
