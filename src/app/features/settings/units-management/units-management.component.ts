    ...
    @if (!loading) {
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-semibold text-gray-900 flex items-center gap-2">
              <i class="fas fa-globe"></i> Unidades del Sistema
            </h3>
            <button
              class="inline-flex items-center px-3 py-1.5 border border-gray-300 shadow-sm text-xs font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
              (click)="hideAllSystemUnits()"
              [disabled]="hidingAllGenericUnits"
              title="Ocultar todas las unidades del sistema"
            >
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @for (u of genericUnits; track u.id) {
              <div
                class="flex flex-col gap-3 border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow bg-gray-50"
                [class.opacity-60]="u.is_hidden"
              >
                <div class="flex-1">
                  <div class="flex justify-between items-start mb-2">
                    ...
                    <div class="flex gap-1">
                      @if (u.is_hidden) {
                        <span
                          class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 mr-1"
                          >Oculto</span
                        >
                      }
                      <span
                  ...
                  @if (u.description) {
                    <div class="text-sm text-gray-500">{{ u.description }}</div>
                  }
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-semibold text-gray-900 flex items-center gap-2">
              <i class="fas fa-building"></i> Unidades Personalizadas
            </h3>
            <button
          ...
          @if (companyUnits.length) {
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              @for (u of companyUnits; track u.id) {
                <div
                  class="flex flex-col gap-3 border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow"
                >
                  <div class="flex-1">
                    @if (editingId === u.id) {
                      <div class="space-y-3">
...
export class UnitsManagementComponent implements OnInit {
  ...
  async hideAllSystemUnits() {
    if (!confirm('¿Estás seguro de que quieres ocultar todas las unidades del sistema visibles?'))
      return;
