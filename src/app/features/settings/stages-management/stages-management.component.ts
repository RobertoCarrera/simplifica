...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, RouterLink, DragDropModule, TextContrastPipe],
  template: `
    <!-- Header -->
    <div class="mb-6">
      <div class="flex justify-between items-center mb-4">
        <button
          class="inline-flex items-center gap-2 px-3 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-semibold text-gray-900 flex items-center gap-2">
            <i class="fas fa-globe"></i> Estados del Sistema
          </h3>
          <button
        ...
        <p class="text-sm text-gray-600 bg-gray-50 rounded-lg p-3 mb-4 flex items-center gap-2">
          ...
          Estados predeterminados. Puedes reordenarlos arrastrando (solo los visibles; los ocultos
          quedan al final) y ocultarlos si no las necesitas.
        </p>
        <div
        ...
        >
          @for (stage of visibleGenericStages; track stage.id) {
            ...
            >
              <div
                class="cursor-grab p-2 flex items-center justify-center transition-colors hover:text-blue-600 active:cursor-grabbing"
                cdkDragHandle
                [style.color]="stage.color | textContrast"
              >
              ...
              <ng-template cdkDragPreview>
                <div
                  class="flex items-center gap-3 p-4 rounded-lg border border-gray-200 transition-all hover:shadow-md drag-preview"
                  [style.background-color]="stage.color"
                  [style.color]="stage.color | textContrast"
                >
                  <div
              ...
              <ng-template cdkDragPlaceholder>
                <div
                  class="flex items-center gap-3 p-4 rounded-lg border border-gray-200 transition-all hover:shadow-md placeholder bg-transparent border-dashed border-2 border-blue-500 opacity-0 min-h-[84px] scale-95 transition-all duration-200"
                ></div>
              </ng-template>
        ...
        @if (hiddenGenericStages.length > 0) {
          <div
            class="my-6 border-t border-gray-100 pt-4 flex items-center gap-2 text-sm font-medium text-gray-500"
          >
            <button
              class="inline-flex items-center px-3 py-1.5 border border-gray-300 shadow-sm text-xs font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
              ...
              [style.color]="
                btnHoverById['hidden-toggle']
                  ? (newStage.color || '#6366f1' | textContrast) === '#000'
                    ? '#fff'
                    : '#000'
                  : (newStage.color || '#6366f1' | textContrast)
              "
              [style.background-color]="
                btnHoverById['hidden-toggle']
                  ? (newStage.color || '#6366f1' | textContrast) === '#000'
            ...
            >
              <i
                class="fas mr-1"
                [class.fa-chevron-down]="hiddenGenericsCollapsed"
                [class.fa-chevron-up]="!hiddenGenericsCollapsed"
              ></i>
          ...
          @if (!hiddenGenericsCollapsed) {
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 hidden-list">
              @for (stage of hiddenGenericStages; track stage.id) {
                <div
                  class="flex items-center gap-3 p-4 rounded-lg border border-gray-200 transition-all hover:shadow-md opacity-60 bg-gray-50"
                  [style.background-color]="stage.color"
                  [style.color]="stage.color | textContrast"
                >
                  <div class="flex-1 min-w-0">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-semibold text-gray-900 flex items-center gap-2">
            <i class="fas fa-building"></i> Estados Personalizados
          </h3>
          <button
          ...
          <div class="border border-gray-200 rounded-lg p-4 mb-4 bg-white shadow-sm">
            ...
            <form (ngSubmit)="createStage()" class="space-y-4">
              <div class="flex flex-col gap-1">
                <label for="newName" class="text-sm font-medium text-gray-700"
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="flex flex-col gap-1">
                  <label for="newPosition" class="text-sm font-medium text-gray-700"
                    >Posición *</label
                  >
                  <input
                    type="number"
              ...
              <div class="flex flex-col gap-1">
                <label for="newWorkflowCategory" class="text-sm font-medium text-gray-700"
                  >Categoría de flujo *</label
                >
                <select
                ...
                <small class="text-xs text-gray-500"
                  >Define cómo cuenta este estado en las estadísticas</small
                >
              </div>
              ...
              <div class="flex gap-2 justify-end pt-2">
                ...
                >
                  <i
                    class="fas mr-1"
                    [class.fa-spinner]="creating"
                    [class.fa-spin]="creating"
                    [class.fa-save]="!creating"
                  ></i>
        ...
        @if (companyStages.length === 0) {
          ...
          <div
            class="grid grid-cols-1 md:grid-cols-2 gap-4 relative"
            cdkDropList
            (cdkDropListDropped)="onDropCompany($event)"
          >
            @for (stage of companyStages; track stage.id) {
              <div
                class="flex items-center gap-3 p-4 rounded-lg border border-gray-200 transition-all hover:shadow-md bg-white"
                cdkDrag
                [style.background-color]="stage.color"
                [style.color]="stage.color | textContrast"
              >
                  ...
                  <form (ngSubmit)="saveEdit()" class="flex flex-col gap-3 w-full">
                    ...
                    <div class="flex gap-2 justify-end">
                      <button
                        ...
                        [style.background-color]="
                          btnHoverById[stage.id + '-save']
                            ? (editStage.color || stage.color | textContrast) === '#000'
                              ? '#fff'
                              : '#000'
                            : 'transparent'
                        "
          ...
          @if (showingReassignPicker && pendingDeleteStageId) {
            <div class="border border-gray-200 rounded-lg p-4 mt-4 bg-white shadow-sm">
              ...
              <p class="text-sm text-gray-600 mb-4">
                Este estado tiene tickets asociados. Selecciona a qué estado moverlos para poder
                eliminarlo.
              </p>
              <div class="flex flex-col gap-1 mb-4">
                <label for="reassignTarget" class="text-sm font-medium text-gray-700"
                  >Mover tickets a</label
                >
                <select
                  id="reassignTarget"
                  class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                ...
                <small class="text-xs text-gray-500"
                  >No puedes reasignar a un estado genérico oculto</small
                >
              </div>
              <div class="flex gap-2 justify-end">
                <button
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-lg p-6">
      ...
      <ul class="list-disc pl-5 space-y-2 text-blue-900">
        <li>
          <strong>Estados del Sistema:</strong> Son comunes para todas las empresas y no se pueden
          modificar ni eliminar, pero puedes ocultarlos si no las necesitas.
        </li>
        <li>
          <strong>Ocultar Estados:</strong> Los estados del sistema que ocultes no aparecerán en tus
          listas y formularios, pero seguirán disponibles para otras empresas.
        </li>
        <li>
          <strong>Estados Personalizados:</strong> Puedes crear estados específicos para tu empresa
          que complementen los del sistema.
        </li>
        <li>
          <strong>Posición:</strong> Determina el orden en que aparecen los estados en las listas y
          tableros.
        </li>
...
export class StagesManagementComponent implements OnInit {
  ...
  async hideAllSystemStages() {
    if (!confirm('¿Estás seguro de que quieres ocultar todos los estados del sistema visibles?'))
      return;
    ...
    try {
      ...
      this.toast.success(
        ...
        'Todos los estados del sistema visibles han sido ocultados',
      );
      await this.loadStages();
    } catch (e: any) {
      this.errorMessage =
        'Error al ocultar todos los estados: ' + (e?.message || 'Error desconocido');
    } finally {
  ...
  async createStage() {
    ...
    try {
      ...
      if (
        msg.includes('ux_ticket_stages_company_final') ||
        msg.includes('workflow_category = "final"')
      ) {
        this.errorMessage = 'Ya existe un estado "Final" en tu empresa. Solo puede haber uno.';
      } else if (
        msg.includes('ux_ticket_stages_company_cancel') ||
        msg.includes('workflow_category = "cancel"')
      ) {
        this.errorMessage =
          'Ya existe un estado de "Cancelación" en tu empresa. Solo puede haber uno.';
      } else if (msg.includes('debe existir al menos un estado de la categoría')) {
        this.errorMessage =
          'No puedes eliminar o mover el último estado de una categoría requerida. Asegúrate de mantener al menos uno por categoría.';
      } else {
  ...
  async deleteStage(stageId: string) {
    ...
    try {
      ...
      if (result.error) {
        ...
        if (
          ...
          (code === 'REASSIGN_REQUIRED' ||
            ...
            msg.includes('asignar') ||
            msg.includes('tickets referenciando'))
        ) {
          this.pendingDeleteStageId = stageId;
          this.reassignTargetId = null;
          this.showingReassignPicker = true;
          this.toast.info(
            'Este estado tiene tickets',
            'Selecciona un estado destino para reasignar los tickets.',
          );
          return;
        }
        ...
        if (
          status === 409 &&
          (code === 'COVERAGE_BREAK' || msg.includes('categor') || msg.includes('coverage'))
        ) {
          this.errorMessage =
            'No puedes eliminar este estado porque dejaría una categoría sin cobertura. Muestra un estado del sistema equivalente o crea otro antes de eliminar.';
      ...
      const msg = (error?.message || '').toLowerCase();
      if (
        msg.includes('debe existir al menos un estado de la categoría') ||
        msg.includes('coverage') ||
        msg.includes('categoría requerida')
      ) {
        this.errorMessage =
          'No puedes eliminar este estado porque dejaría una categoría sin cobertura. Muestra un estado del sistema equivalente o crea otro antes de eliminar.';
      } else {
        this.errorMessage =
          'Error al eliminar el estado: ' + (error.message || 'Error desconocido');
      }
    }
  }
  ...
  getReassignOptions(excludeStageId: string): TicketStage[] {
    ...
    return [...visibleGenerics, ...company].sort(
  ...
  async confirmReassignAndDelete() {
    ...
    try {
      ...
      let res = await this.stagesService.hideGenericStageWithReassign(
        this.pendingDeleteStageId,
        this.reassignTargetId,
      );
      if (res.error && Number(res.error.status || 0) >= 400) {
        res = await this.stagesService.deleteStageWithReassign(
          this.pendingDeleteStageId,
          this.reassignTargetId,
        );
      }
      ...
      if (
        ...
        this.errorMessage =
          'No se pudo completar porque se rompería la cobertura de categorías. Revisa los estados visibles.';
      } else {
        this.errorMessage =
          'No se pudo completar la reasignación: ' + (e?.message || 'Error desconocido');
      }
    }
  }
  ...
  async hideStage(stage: TicketStage) {
    ...
    try {
      ...
      if (result.error) {
        ...
        if (
          status === 409 &&
          (code === 'REASSIGN_REQUIRED' || msg.includes('reassign') || msg.includes('asignar'))
        ) {
          ...
          this.toast.info(
            'Este estado tiene tickets',
            'Selecciona un estado destino para reasignar los tickets y vuelve a ocultar.',
          );
          return;
        }
        if (
  ...
  async onDropCompany(event: CdkDragDrop<TicketStage[]>) {
    ...
    try {
      ...
      this.toast.success(
        'Orden actualizado',
        'El orden de los estados se ha actualizado correctamente',
      );
    } catch (error: any) {
      this.errorMessage = 'Error al actualizar el orden: ' + (error.message || 'Error desconocido');
