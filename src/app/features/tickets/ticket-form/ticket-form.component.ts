...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, TagManagerComponent, TranslocoPipe, ServiceTranslatePipe],
...
export class TicketFormComponent implements OnChanges, OnDestroy {
  ...
  addServiceToTicket(service: Service, variant?: ServiceVariant) {
    const existing = this.selectedServices.find(
      (s) =>
  ...
  updateServiceQuantity(serviceId: string, quantity: number, variantId?: string) {
    const item = this.selectedServices.find(
      (s) => s.service.id === serviceId && (variantId ? s.variant?.id === variantId : !s.variant),
    );
  ...
  private async getTopUsedProducts(): Promise<any[]> {
    // RPC Mock or call
    if (this.companyId) {
      const { data } = await (this.simpleSupabase.getClient() as any).rpc('get_top_used_products', {
        target_company_id: this.companyId,
        limit_count: 3,
      });
  ...
  filterProducts() {
    ...
    this.filteredProducts = this.availableProducts.filter(
      (p) => p.name.toLowerCase().includes(text) || p.brand?.toLowerCase().includes(text),
    );
  }
  ...
  onBrandSearchChange() {
    this.filteredBrands = this.availableBrands.filter((b) =>
      b.name.toLowerCase().includes(this.brandSearchText.toLowerCase()),
    );
  }

  onCategorySearchChange() {
    this.filteredCategories = this.availableCategories.filter((c) =>
      c.name.toLowerCase().includes(this.categorySearchText.toLowerCase()),
    );
  }

  hasExactBrandMatch() {
    return this.availableBrands.some(
      (b) => b.name.toLowerCase() === this.brandSearchText.toLowerCase(),
    );
  }
  getExactBrandMatch() {
    return this.availableBrands.find(
      (b) => b.name.toLowerCase() === this.brandSearchText.toLowerCase(),
    );
  }
  selectExistingBrandMatch() {
    const m = this.getExactBrandMatch();
    if (m) this.selectBrand(m);
  }

  hasExactCategoryMatch() {
    return this.availableCategories.some(
      (c) => c.name.toLowerCase() === this.categorySearchText.toLowerCase(),
    );
  }
  getExactCategoryMatch() {
    return this.availableCategories.find(
      (c) => c.name.toLowerCase() === this.categorySearchText.toLowerCase(),
    );
  }
  selectExistingCategoryMatch() {
    const m = this.getExactCategoryMatch();
    if (m) this.selectCategory(m);
  }

  async createNewBrand() {
    try {
      const brand = await this.productMetadataService.createBrand(
  ...
  async createNewCategory() {
    try {
      const cat = await this.productMetadataService.createCategory(
  ...
  async loadCustomerDevices() {
    ...
    try {
      ...
      this.customerDevices = (devices || []).filter(
  ...
  filterCustomerDevices() {
    ...
    this.filteredCustomerDevices = this.customerDevices.filter(
      (d) =>
  ...
  isDeviceSelected(id: string) {
    return this.selectedDevices.some((d) => d.id === id);
  }
  ...
  getTotalEstimatedHours() {
    return this.selectedServices.reduce(
      (sum, s) => sum + s.service.estimated_hours * s.quantity,
      0,
    );
  }
  ...
  async saveTicket() {
    ...
    try {
      ...
      if (this.editingTicket) {
        ...
        await this.ticketsService.replaceTicketServices(
        ...
        await this.ticketsService.replaceTicketProducts(
          this.editingTicket.id,
          this.companyId,
          productItems,
        );
        ...
        savedInfo = await this.ticketsService.createTicketWithItems(
          ticketData,
          serviceItems,
          productItems,
        );
  ...
  async syncTicketDevices(ticketId: string, devices: Device[]) {
    ...
    const { data: currentRels } = await (client as any)
      .from('ticket_devices')
      .select('device_id')
      .eq('ticket_id', ticketId);
    ...
    for (const id of toRemove) {
      await (client as any)
        .from('ticket_devices')
        .delete()
        .match({ ticket_id: ticketId, device_id: id });
    }
  }
