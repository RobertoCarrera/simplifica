...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ClientGdprPanelComponent, FormsModule, TranslocoPipe],
  template: `
    <div class="max-w-4xl mx-auto p-6 space-y-8">
      <div class="flex items-center justify-between">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ 'portal.settings.titulo' | transloco }}</h1>
      </div>

      <div *ngIf="isLoading" class="text-center py-10">
        <i class="fas fa-spinner fa-spin text-3xl text-primary-500"></i>
        <p class="mt-2 text-gray-500">{{ 'portal.settings.cargandoInfo' | transloco }}</p>
      </div>
      ...
      <div *ngIf="!isLoading && customer" class="space-y-8 animate-fadeIn">
        ...
        <div class="bg-white dark:bg-gray-800 shadow rounded-xl p-6 border border-gray-200 dark:border-gray-700">
          <h2 class="text-xl font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2">
            ...
            {{ 'portal.settings.datosFact' | transloco }}
          </h2>
          <p class="text-gray-500 dark:text-gray-400 mb-6 text-sm">
            {{ 'portal.settings.datosFact_desc' | transloco }}
          </p>
          ...
          <form (ngSubmit)="saveBilling()" class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="form-group">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ 'portal.settings.razonSocial' | transloco }}</label>
                ...
                <p class="text-xs text-gray-400 mt-1">{{ 'portal.settings.contactaSoporte' | transloco }}</p>
            </div>

            <div class="form-group">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ 'portal.settings.cifNif' | transloco }}</label>
            ...
            <!-- Email -->
            <div class="form-group">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ 'portal.settings.emailFacturacion' | transloco }}</label>
            ...
            <!-- Payment -->
            <div class="form-group">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ 'portal.settings.metodoPago' | transloco }}</label>
                <select [(ngModel)]="customer.payment_method" name="payment_method" class="form-select w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                  <option value="">{{ 'portal.settings.seleccionar' | transloco }}</option>
                  <option value="transfer">{{ 'portal.settings.transferencia' | transloco }}</option>
                  <option value="direct_debit">{{ 'portal.settings.domiciliacion' | transloco }}</option>
                  <option value="card">{{ 'portal.settings.tarjeta' | transloco }}</option>
            ...
            <div class="form-group">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ 'portal.settings.iban' | transloco }}</label>
            ...
            <div class="form-group">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ 'portal.settings.swiftBic' | transloco }}</label>
            ...
            <div class="col-span-1 md:col-span-2 flex justify-end pt-4 border-t dark:border-gray-700">
                <button type="submit" [disabled]="isSaving" class="px-6 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-lg font-medium transition-colors flex items-center gap-2">
                    ...
                    {{ isSaving ? ('portal.settings.guardando' | transloco) : ('portal.settings.guardarFacturacion' | transloco) }}
                </button>
            </div>
          </form>
        </div>
