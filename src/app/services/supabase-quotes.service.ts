...
export class SupabaseQuotesService {
  ...
  private async executeGetQuote(id: string): Promise<Quote> {
    ...
    const { data, error } = await client
      ...
      .select('id, full_quote_number, quote_number, year, sequence_number, title, status, quote_date, valid_until, total_amount, subtotal, tax_amount, discount_amount, currency, language, client_id, company_id, notes, terms_conditions, created_at, updated_at, is_anonymized, retention_until, client:clients(id, name, surname, email, phone, business_name, address, cif_nif), items:quote_items(id, quote_id, company_id, description, quantity, unit_price, tax_rate, tax_amount, subtotal, total, discount_percent, discount_amount, service_id, variant_id, line_number, notes, billing_period, created_at, updated_at, service:services(id,name,description), variant:service_variants(id,variant_name,pricing)), invoice:invoices!quotes_invoice_id_fkey(*)')
      .eq('id', id)
      .single();
