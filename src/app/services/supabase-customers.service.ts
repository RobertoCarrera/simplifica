...
export class SupabaseCustomersService {
  ...
  private async callUpsertClientRpc(customer: CreateCustomerDev | UpdateCustomer): Promise<Customer> {
    ...
    const payload: any = {
      ...
      // CRM & Billing Fields
      status: (customer as any).status ?? 'lead',
      ...
      iban: (customer as any).iban ?? null,
      bic: (customer as any).bic ?? null,
      currency: (customer as any).currency ?? 'EUR',
      tax_region: (customer as any).tax_region ?? null,
      billing_email: (customer as any).billing_email ?? null,
