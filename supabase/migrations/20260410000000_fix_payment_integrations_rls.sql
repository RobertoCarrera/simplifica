-- Fix critical cross-tenant data leak in payment_integrations
-- Previous policies did not verify that the user belonged to the same company as the integration.
...
-- 2. Create secure policies enforcing company_id match
-- We use TO authenticated because anonymous users should never access these.
...
USING (
  EXISTS (
    ...
    WHERE u.auth_user_id = auth.uid()
      AND u.company_id = payment_integrations.company_id -- CRITICAL CHECK
