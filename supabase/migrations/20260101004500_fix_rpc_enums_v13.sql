-- Migration to fix RPC with correct enum values and schema
-- Created on 2026-01-01-0020
-- V13 consolidated: 
-- 1. FIX: invoice_status 'draft' instead of 'pending'
-- 2. FIX: invoice_type 'normal' explicit assignment
-- 3. FIX: quote_number format YYYY-XXXXX
-- 4. FIX: invoice_number from DB sequence
-- 5. FIX: All column name corrections (source_quote_id, tax_rate, etc.)
...
BEGIN
    ...
    -- 6. Create Quote
    -- STATUS: 'accepted' is valid for quote_status enum
    ...
    -- 7. Create Invoice
    -- STATUS: 'draft' is valid for invoice_status enum ('pending' was NOT)
    -- TYPE: 'normal' is valid for invoice_type enum
