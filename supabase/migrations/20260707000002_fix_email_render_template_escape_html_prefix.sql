-- Hotfix: prefix all escape_html() calls inside email_render_template()
-- with `public.` so they resolve under SET search_path = ''.
--
-- Background: the migration `20260706000001_preview_email_template_rpc.sql`
-- defines the function with `SET search_path = ''` (a Supabase SECDEF
-- hardening requirement) but inside the CASE branches the per-type HTML
-- builders call `escape_html(...)` WITHOUT the schema prefix. With an
-- empty search_path, Postgres cannot resolve unqualified function names —
-- hence the runtime error `function escape_html(text) does not exist`
-- (SQLSTATE 42883) when `preview_email_template` is invoked from PostgREST.
--
-- This migration re-creates email_render_template with every `escape_html(...)`
-- call rewritten as `public.escape_html(...)`. The interpolation helper
-- itself (`public.interpolate_safe`) already calls `public.escape_html`
-- correctly, so no changes needed there.
--
