...
-- Create unique indexes to ensure only one stage per type per company
-- We use partial unique indexes for this
CREATE UNIQUE INDEX IF NOT EXISTS unique_review_stage_per_company 
ON public.project_stages (company_id) 
WHERE (is_review = true);

CREATE UNIQUE INDEX IF NOT EXISTS unique_default_stage_per_company 
ON public.project_stages (company_id) 
WHERE (is_default = true);

CREATE UNIQUE INDEX IF NOT EXISTS unique_landing_stage_per_company 
ON public.project_stages (company_id) 
WHERE (is_landing = true);

-- Function to handle auto-move logic
...
BEGIN
    ...
    -- Check if current stage is the review stage
    SELECT is_review INTO v_is_review_stage
    ...
    IF v_incomplete_tasks_count = 0 THEN
        IF v_review_stage_id IS NOT NULL AND (v_current_stage_id IS DISTINCT FROM v_review_stage_id) THEN
            ...
            -- Using IS DISTINCT FROM handles nulls safely, though stage_id shouldn't be null usually.
    ...
    -- ONLY if currently in Review Stage
    IF v_incomplete_tasks_count > 0 THEN
        IF v_is_review_stage = true AND v_default_stage_id IS NOT NULL THEN
             UPDATE public.projects 
...
-- Create Triggers
DROP TRIGGER IF EXISTS trigger_auto_move_on_task_change ON public.project_tasks;

CREATE TRIGGER trigger_auto_move_on_task_change
AFTER INSERT OR UPDATE OF is_completed OR DELETE
ON public.project_tasks
FOR EACH ROW
EXECUTE FUNCTION public.handle_project_auto_move();
