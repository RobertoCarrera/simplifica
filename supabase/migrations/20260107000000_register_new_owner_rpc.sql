-- Migration: Create RPC for registering a new owner from an invitation
-- Updated to support Multi-Tenancy (company_members)
...
begin
  ...
  if v_user_id is not null then
    -- Update existing user (set legacy columns for now, but primarily identify via company_members)
    update public.users
    set 
      company_id = v_new_company_id, -- Legacy/Default context
      role = 'owner',                -- Legacy/Default context
      name = p_user_name,
