-- Migration to add contract_service_rpc
-- Created on 2025-12-31-2000
...
BEGIN
    ...
    IF p_variant_id IS NOT NULL THEN
        ...
        IF v_variant_record.id IS NOT NULL THEN
            v_variant_name := COALESCE(v_variant_record.variant_name, v_variant_record.name); -- name is likely variant_name alias in record? No, checking schema again implies variant_name is the column. 'name' might not exist on variant record if explicit. Schema said 'variant_name'.
            v_title := v_service_record.name || ' - ' || COALESCE(v_variant_record.variant_name, '');
    ...
    INSERT INTO public.quotes (
        ...
        full_quote_number,
        title,
        status, -- 'accepted' for direct contract
        quote_date,
        valid_until,
        
        subtotal,
        discount, -- global discount, usually 0 if line item discount used
        tax_amount,
        total_amount,
        ...
        v_client_record.id,
        v_quote_number::integer, -- cast if needed, assumig get_next returns numeric string or int
        TO_CHAR(v_year, 'FM0000') || '/' || LPAD(v_quote_number::text, 4, '0'), -- Format: YYYY/0001
        v_title,
        'accepted', -- Contracted immediately
        CURRENT_DATE,
        ...
        CASE WHEN v_is_recurring THEN 
             ...
             END
        ELSE 'none' END,
        1, -- Interval count (1 month, 1 year, etc) - simplified
        CURRENT_DATE,
        CASE WHEN v_is_recurring THEN CURRENT_DATE + INTERVAL '1 month' ELSE NULL END, -- Simplified next run
        v_auth_user_id
    ...
    -- We can call the existing `convert_quote_to_invoice` function if it exists and works.
    -- However, to be safe and avoid "owner_id" bugs seen in previous sessions, let's look for it or manual insert.
    -- Given the constraint "fix bugs", manual insert might be safer if `convert_quote_to_invoice` is flaky.
    -- But `convert_quote_to_invoice` handles series and everything.
    -- Let's try manual insert for speed and reliability in this specific controlled flow.
    ...
    v_invoice_number := public.get_next_invoice_number(v_client_record.company_id, v_series_id, v_year);
    ...
    INSERT INTO public.invoices (
        ...
        invoice_number,
        full_invoice_number,
        status, -- 'pending' until paid OR 'draft'
        issue_date,
        due_date,
        subtotal,
        tax_amount,
        ...
        COALESCE(v_invoice_series, '') || CASE WHEN v_invoice_series IS NOT NULL THEN '-' ELSE '' END || v_invoice_number,
        'draft', -- Start as draft/pending. 'pending' is usually for sent invoices.
        CURRENT_DATE,
    ...
    -- Require payment selection if:
    -- 1. There are payment methods available
    -- 2. AND (More than 1 method OR (1 method but we want user to confirm/click pay))
    -- Actually, if there is ANY payment method, we usually should prompt user "Pay with X".
    -- "requires_payment_selection" -> TRUE means Frontend should show modal.
    -- If FALSE, Frontend assumes "Order Created" (Pay Later / Cash / Wire Transfer manually).
...
GRANT EXECUTE ON FUNCTION "public"."contract_service_rpc" TO "authenticated";
