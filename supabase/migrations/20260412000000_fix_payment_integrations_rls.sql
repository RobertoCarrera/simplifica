-- Fix RLS for payment_integrations
-- CRITICAL SECURITY FIX: Previously missing company_id check allowed cross-tenant access for admins.
-- This migration restricts access to only users belonging to the same company as the payment integration.
