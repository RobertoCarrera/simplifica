...
ALTER SCHEMA "verifactu" OWNER TO "postgres";
...
DECLARE
  ...
  v_cif_nif text;
  v_trade_name text;
  v_legal_rep_name text;
  v_legal_rep_dni text;
  v_mercantile_data jsonb;
  v_metadata jsonb;
  v_address_id uuid;
  v_full_name text;
...
BEGIN
  -- 1. Resolve Company
  SELECT company_id INTO v_company_id FROM users WHERE auth_user_id = v_auth_user_id LIMIT 1;
  IF v_company_id IS NULL THEN
    RAISE EXCEPTION 'User not associated with a company (ID: %)', v_auth_user_id;
  END IF;

  -- 2. Extract
  v_client_id := (payload->>'id')::uuid;
  v_name := upper(trim(payload->>'name'));
  v_apellidos := upper(trim(payload->>'apellidos'));
  v_email := lower(trim(payload->>'email'));
  v_phone := trim(payload->>'phone');
  v_dni := upper(trim(payload->>'dni'));
  v_client_type := lower(trim(coalesce(payload->>'client_type', 'individual')));
  v_business_name := upper(trim(payload->>'business_name'));
  v_cif_nif := upper(trim(payload->>'cif_nif'));
  v_trade_name := upper(trim(payload->>'trade_name'));
  v_legal_rep_name := upper(trim(payload->>'legal_representative_name'));
  v_legal_rep_dni := upper(trim(payload->>'legal_representative_dni'));
  v_mercantile_data := payload->'mercantile_registry_data';
  v_metadata := payload->'metadata';
  v_address_id := (payload->>'direccion_id')::uuid;

  -- Validation
  IF v_client_type = 'business' THEN
     IF v_business_name IS NULL OR v_cif_nif IS NULL OR v_email IS NULL THEN
       RAISE EXCEPTION 'Missing business fields';
     END IF;
     v_cif_nif := coalesce(v_cif_nif, 'B99999999');
  ELSE
     IF v_name IS NULL OR v_email IS NULL THEN
       -- Can't raise here if updating? TS checked it. 
       -- If inserting, yes.
       -- If updating, we might lack name if not passed? 
       -- But payload normally contains everything from form.
       -- PL/PGSQL coalesce(v_name, NULL) will be NULL if missing from payload.
       NULL; 
     END IF;
     v_dni := coalesce(v_dni, '99999999X');
  END IF;

  v_full_name := trim(regexp_replace(coalesce(v_name, '') || ' ' || coalesce(v_apellidos, ''), '\s+', ' ', 'g'));
  IF v_full_name = '' THEN v_full_name := NULL; END IF;

  -- 3. Update or Insert
  IF v_client_id IS NOT NULL THEN
     -- Update
     SELECT company_id INTO v_existing_comp_id FROM clients WHERE id = v_client_id;
     IF v_existing_comp_id IS NULL OR v_existing_comp_id != v_company_id THEN
     ...
     UPDATE clients SET
       ...
       client_type = coalesce(v_client_type, client_type),
       business_name = coalesce(v_business_name, business_name),
       cif_nif = coalesce(v_cif_nif, cif_nif),
       trade_name = coalesce(v_trade_name, trade_name),
       legal_representative_name = coalesce(v_legal_rep_name, legal_representative_name),
       legal_representative_dni = coalesce(v_legal_rep_dni, legal_representative_dni),
       mercantile_registry_data = coalesce(v_mercantile_data, mercantile_registry_data),
       metadata = coalesce(v_metadata, metadata),
       direccion_id = coalesce(v_address_id, direccion_id),
       updated_at = now()
  ...
  ELSE
     -- Insert
     IF EXISTS (SELECT 1 FROM clients WHERE company_id = v_company_id AND email = v_email AND deleted_at IS NULL) THEN
        RAISE EXCEPTION 'A client with this email already exists in your company';
     END IF;

     INSERT INTO clients (
       company_id, name, apellidos, email, phone, dni, client_type,
       business_name, cif_nif, trade_name, legal_representative_name,
       legal_representative_dni, mercantile_registry_data, metadata, direccion_id
     ) VALUES (
       v_company_id, v_full_name, v_apellidos, v_email, v_phone, v_dni, v_client_type,
       v_business_name, v_cif_nif, v_trade_name, v_legal_rep_name,
       v_legal_rep_dni, v_mercantile_data, coalesce(v_metadata, '{}'::jsonb), v_address_id
     )
     RETURNING to_jsonb(clients.*) INTO v_result;
  END IF;
...
CREATE OR REPLACE FUNCTION "verifactu"."compute_invoice_hash"("p_payload" "jsonb", "p_prev_hash" "text") RETURNS "text"
    LANGUAGE "plpgsql" IMMUTABLE
    SET "search_path" TO 'public', 'extensions', 'pg_temp'
    AS $$
begin
  -- Simple canonicalization: just stringify payload.
  -- In prod, ensure key sorting or use specific fields concatenation as per VeriFactu specs.
  return encode(extensions.digest(coalesce(p_prev_hash,'') || '|' || p_payload::text, 'sha256'), 'hex');
end;
$$;


ALTER FUNCTION "verifactu"."compute_invoice_hash"("p_payload" "jsonb", "p_prev_hash" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "verifactu"."compute_vat_breakdown"("p_invoice_id" "uuid") RETURNS "jsonb"
    LANGUAGE "plpgsql" STABLE
    AS $_$
declare
  rate_col text;
...
ALTER FUNCTION "verifactu"."compute_vat_breakdown"("p_invoice_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "verifactu"."get_next_invoice_number"("p_company_id" "uuid", "p_series" "text") RETURNS TABLE("number" bigint, "previous_hash" "text")
    LANGUAGE "plpgsql"
    AS $$
declare
  lock_key bigint;
...
ALTER FUNCTION "verifactu"."get_next_invoice_number"("p_company_id" "uuid", "p_series" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "verifactu"."handle_invoice_verifactu"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  -- REMOVED INSERT LOGIC to prevent auto-sending drafts.
  -- Events are now created explicitly by finalize_invoice().
...
ALTER FUNCTION "verifactu"."handle_invoice_verifactu"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "verifactu"."move_to_dlq"("p_event_id" "uuid", "p_reason" "text") RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$
declare
  v_evt verifactu.events%rowtype;
begin
  select * into v_evt from verifactu.events where id = p_event_id;
  if not found then return; end if;

  insert into verifactu.events_dlq(original_event_id, company_id, invoice_id, event_type, payload, error_reason)
  values (v_evt.id, v_evt.company_id, v_evt.invoice_id, v_evt.event_type, v_evt.payload, p_reason);

  delete from verifactu.events where id = p_event_id;
end;
$$;


ALTER FUNCTION "verifactu"."move_to_dlq"("p_event_id" "uuid", "p_reason" "text") OWNER TO "postgres";
...
CREATE OR REPLACE VIEW "analytics"."invoice_base" AS
 SELECT "id" AS "invoice_id",
    ...
    COALESCE("total", (0)::numeric) AS "total_amount",
    COALESCE("paid_amount", (0)::numeric) AS "paid_amount",
    (COALESCE("total", (0)::numeric) - COALESCE("paid_amount", (0)::numeric)) AS "pending_amount",
    "status",
    "invoice_type",
    "invoice_date",
    "due_date",
    "invoice_month" AS "period_month",
        CASE
            WHEN ("status" = 'paid'::"public"."invoice_status") THEN 0
            WHEN ("due_date" < CURRENT_DATE) THEN (- (CURRENT_DATE - "due_date"))
            ELSE ("due_date" - CURRENT_DATE)
        END AS "days_to_due",
    (("due_date" < CURRENT_DATE) AND ("status" <> ALL (ARRAY['paid'::"public"."invoice_status", 'cancelled'::"public"."invoice_status", 'draft'::"public"."invoice_status"]))) AS "is_overdue"
   FROM "public"."invoices" "i"
  WHERE ("deleted_at" IS NULL);


ALTER VIEW "analytics"."invoice_base" OWNER TO "postgres";
...
CREATE MATERIALIZED VIEW "analytics"."mv_invoice_kpis_monthly" AS
 SELECT "company_id",
    ...
    "count"(*) FILTER (WHERE ("status" = 'paid'::"public"."invoice_status")) AS "paid_count",
    "count"(*) FILTER (WHERE ("status" = ANY (ARRAY['sent'::"public"."invoice_status", 'partial'::"public"."invoice_status"]))) AS "pending_count",
    "count"(*) FILTER (WHERE (("status" = 'overdue'::"public"."invoice_status") OR "is_overdue")) AS "overdue_count",
    "count"(*) FILTER (WHERE ("status" = 'cancelled'::"public"."invoice_status")) AS "cancelled_count",
    "count"(*) FILTER (WHERE ("status" = 'draft'::"public"."invoice_status")) AS "draft_count",
    "sum"("subtotal") AS "subtotal_sum",
    "sum"("tax_amount") AS "tax_sum",
    "sum"("total_amount") AS "total_sum",
    "sum"("paid_amount") AS "collected_sum",
    "sum"("pending_amount") AS "pending_sum",
    "sum"("total_amount") FILTER (WHERE ("status" = 'paid'::"public"."invoice_status")) AS "paid_total_sum",
    "sum"("total_amount") FILTER (WHERE (("status" = ANY (ARRAY['sent'::"public"."invoice_status", 'partial'::"public"."invoice_status", 'overdue'::"public"."invoice_status"])) OR "is_overdue")) AS "receivable_sum",
    "avg"("total_amount") AS "avg_invoice_value",
    ("sum"("paid_amount") / NULLIF("sum"("total_amount"), (0)::numeric)) AS "collection_rate"
   FROM "analytics"."invoice_base" "ib"
  GROUP BY "company_id", "created_by", "period_month"
  WITH NO DATA;


ALTER MATERIALIZED VIEW "analytics"."mv_invoice_kpis_monthly" OWNER TO "postgres";
...
CREATE OR REPLACE VIEW "analytics"."quote_base" AS
 SELECT "id" AS "quote_id",
    ...
    "conversion_status",
        CASE
            WHEN ("quote_date" IS NOT NULL) THEN (("quote_date")::timestamp without time zone)::timestamp with time zone
            ELSE "created_at"
        END AS "quote_ts",
    "quote_month" AS "period_month",
    (
        CASE
            WHEN ("accepted_at" IS NOT NULL) THEN "date_part"('day'::"text", ("accepted_at" - COALESCE(("quote_date")::timestamp with time zone, "created_at")))
            ELSE NULL::double precision
        END)::integer AS "days_to_accept"
   FROM "public"."quotes" "q";


ALTER VIEW "analytics"."quote_base" OWNER TO "postgres";
...
CREATE MATERIALIZED VIEW "analytics"."mv_quote_cube" AS
 SELECT "company_id",
    ...
    "status",
    "conversion_status",
    (((((GROUPING("company_id") << 4) + (GROUPING("created_by") << 3)) + (GROUPING("period_month") << 2)) + (GROUPING("status") << 1)) + GROUPING("conversion_status")) AS "group_id",
    "count"(*) AS "quotes_count",
    "sum"("subtotal") AS "subtotal_sum",
    "sum"("tax_amount") AS "tax_sum",
    "sum"("total_amount") AS "total_sum"
   FROM "analytics"."quote_base" "qb"
  GROUP BY CUBE("company_id", "created_by", "period_month", "status", "conversion_status")
  WITH NO DATA;


ALTER MATERIALIZED VIEW "analytics"."mv_quote_cube" OWNER TO "postgres";
...
CREATE MATERIALIZED VIEW "analytics"."mv_quote_kpis_monthly" AS
 SELECT "company_id",
    ...
    "sum"("tax_amount") AS "tax_sum",
    "sum"("total_amount") AS "total_sum",
    "avg"("days_to_accept") AS "avg_days_to_accept",
    (("sum"(
        CASE
            WHEN ("conversion_status" = 'accepted'::"text") THEN 1
            ELSE 0
        END))::numeric / (NULLIF("count"(*), 0))::numeric) AS "conversion_rate"
   FROM "analytics"."quote_base" "qb"
  GROUP BY "company_id", "created_by", "period_month"
  WITH NO DATA;


ALTER MATERIALIZED VIEW "analytics"."mv_quote_kpis_monthly" OWNER TO "postgres";
...
ALTER VIEW "analytics"."quote_item_base" OWNER TO "postgres";


CREATE MATERIALIZED VIEW "analytics"."mv_quote_top_items_monthly" AS
 WITH "agg" AS (
         SELECT "qib"."company_id",
            "qib"."created_by",
            "qib"."period_month",
            "qib"."item_id",
 ...
 SELECT "company_id",
    ...
    "subtotal_sum",
    "total_sum",
    "row_number"() OVER (PARTITION BY "company_id", "created_by", "period_month" ORDER BY "total_sum" DESC) AS "rn_by_amount",
    "row_number"() OVER (PARTITION BY "company_id", "created_by", "period_month" ORDER BY "qty_sum" DESC) AS "rn_by_qty"
   FROM "agg" "a"
  WITH NO DATA;


ALTER MATERIALIZED VIEW "analytics"."mv_quote_top_items_monthly" OWNER TO "postgres";
...
CREATE OR REPLACE VIEW "analytics"."ticket_base" AS
 SELECT "t"."id" AS "ticket_id",
    ...
    COALESCE(("ts"."workflow_category" = ANY (ARRAY['final'::"public"."workflow_category", 'cancel'::"public"."workflow_category"])), ("ts"."stage_category" = 'completed'::"public"."stage_category")) AS "is_completed",
    (("t"."due_date" IS NOT NULL) AND ("t"."due_date" < CURRENT_TIMESTAMP) AND (NOT COALESCE(("ts"."workflow_category" = ANY (ARRAY['final'::"public"."workflow_category", 'cancel'::"public"."workflow_category"])), ("ts"."stage_category" = 'completed'::"public"."stage_category")))) AS "is_overdue",
        CASE
            WHEN COALESCE(("ts"."workflow_category" = ANY (ARRAY['final'::"public"."workflow_category", 'cancel'::"public"."workflow_category"])), ("ts"."stage_category" = 'completed'::"public"."stage_category")) THEN 0
            WHEN ("t"."due_date" IS NULL) THEN NULL::integer
            WHEN ("t"."due_date" < CURRENT_DATE) THEN (- (CURRENT_DATE - "t"."due_date"))
            ELSE ("t"."due_date" - CURRENT_DATE)
        END AS "days_to_due",
        CASE
            WHEN COALESCE(("ts"."workflow_category" = ANY (ARRAY['final'::"public"."workflow_category", 'cancel'::"public"."workflow_category"])), ("ts"."stage_category" = 'completed'::"public"."stage_category")) THEN (EXTRACT(epoch FROM ("t"."updated_at" - "t"."created_at")) / 86400.0)
            ELSE NULL::numeric
...
ALTER VIEW "analytics"."ticket_base" OWNER TO "postgres";
...
CREATE MATERIALIZED VIEW "analytics"."mv_ticket_kpis_monthly" AS
 SELECT "company_id",
    ...
    "count"(*) FILTER (WHERE ((("priority")::"text" = 'critical'::"text") OR (("priority")::"text" = 'urgent'::"text"))) AS "critical_count",
    "count"(*) FILTER (WHERE (("priority")::"text" = 'high'::"text")) AS "high_priority_count",
    "count"(*) FILTER (WHERE (("priority")::"text" = 'normal'::"text")) AS "normal_priority_count",
    "count"(*) FILTER (WHERE (("priority")::"text" = 'low'::"text")) AS "low_priority_count",
    "count"(*) FILTER (WHERE (("workflow_category" = 'waiting'::"public"."workflow_category") OR (("workflow_category" IS NULL) AND ("stage_category" = 'open'::"public"."stage_category")))) AS "open_count",
    "count"(*) FILTER (WHERE (("workflow_category" = ANY (ARRAY['analysis'::"public"."workflow_category", 'action'::"public"."workflow_category"])) OR (("workflow_category" IS NULL) AND ("stage_category" = 'in_progress'::"public"."stage_category")))) AS "in_progress_count",
    "count"(*) FILTER (WHERE "is_completed") AS "completed_count",
    "count"(*) FILTER (WHERE ("is_completed" AND (("date_trunc"('month'::"text", "updated_at"))::"date" = "period_month"))) AS "completed_this_month",
    "count"(*) FILTER (WHERE "is_overdue") AS "overdue_count",
    "sum"("total_amount") AS "total_amount_sum",
    "sum"("total_amount") FILTER (WHERE "is_completed") AS "invoiced_amount_sum",
    "avg"("resolution_days") FILTER (WHERE ("resolution_days" IS NOT NULL)) AS "avg_resolution_days",
    "min"("resolution_days") FILTER (WHERE ("resolution_days" IS NOT NULL)) AS "min_resolution_days",
    "max"("resolution_days") FILTER (WHERE ("resolution_days" IS NOT NULL)) AS "max_resolution_days",
    (("count"(*) FILTER (WHERE "is_completed"))::numeric / NULLIF(("count"(*))::numeric, (0)::numeric)) AS "resolution_rate"
   FROM "analytics"."ticket_base" "tb"
  GROUP BY "company_id", "period_month"
  WITH NO DATA;


ALTER MATERIALIZED VIEW "analytics"."mv_ticket_kpis_monthly" OWNER TO "postgres";
...
ALTER TABLE "analytics"."refresh_log" OWNER TO "postgres";


CREATE SEQUENCE IF NOT EXISTS "analytics"."refresh_log_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER SEQUENCE "analytics"."refresh_log_id_seq" OWNER TO "postgres";


ALTER SEQUENCE "analytics"."refresh_log_id_seq" OWNED BY "analytics"."refresh_log"."id";
...
CREATE TABLE IF NOT EXISTS "verifactu"."events" (
    ...
    "response" "jsonb",
    CONSTRAINT "events_event_type_check" CHECK (("event_type" = ANY (ARRAY['alta'::"text", 'anulacion'::"text", 'rectificacion'::"text", 'resumen'::"text"]))),
    CONSTRAINT "events_status_check" CHECK (("status" = ANY (ARRAY['pending'::"text", 'sending'::"text", 'accepted'::"text", 'rejected'::"text"])))
);


ALTER TABLE "verifactu"."events" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "verifactu"."events_dlq" (
    ...
    "status" "text" DEFAULT 'dlq'::"text" NOT NULL,
    "replayed_at" timestamp with time zone,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "events_dlq_event_type_check" CHECK (("event_type" = ANY (ARRAY['alta'::"text", 'anulacion'::"text", 'rectificacion'::"text", 'resumen'::"text"]))),
    CONSTRAINT "events_dlq_status_check" CHECK (("status" = ANY (ARRAY['dlq'::"text", 'replayed'::"text"])))
);


ALTER TABLE "verifactu"."events_dlq" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "verifactu"."invoice_meta" (
    ...
    "issue_time" timestamp with time zone DEFAULT "now"() NOT NULL,
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "invoice_meta_status_check" CHECK (("status" = ANY (ARRAY['pending'::"text", 'sent'::"text", 'accepted'::"text", 'rejected'::"text", 'void'::"text"])))
);


ALTER TABLE "verifactu"."invoice_meta" OWNER TO "postgres";
...
ALTER TABLE "verifactu"."invoice_sequence" OWNER TO "postgres";
...
ALTER VIEW "verifactu"."vw_ledger" OWNER TO "postgres";


ALTER TABLE ONLY "analytics"."refresh_log" ALTER COLUMN "id" SET DEFAULT "nextval"('"analytics"."refresh_log_id_seq"'::"regclass");
...
ALTER TABLE ONLY "analytics"."refresh_log"
...
ALTER TABLE ONLY "verifactu"."events_dlq"
    ADD CONSTRAINT "events_dlq_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "verifactu"."events"
    ADD CONSTRAINT "events_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "verifactu"."invoice_meta"
    ADD CONSTRAINT "invoice_meta_company_id_series_number_key" UNIQUE ("company_id", "series", "number");
...
ALTER TABLE ONLY "verifactu"."invoice_sequence"
    ADD CONSTRAINT "invoice_sequence_pkey" PRIMARY KEY ("company_id", "series");



CREATE UNIQUE INDEX "ux_mv_invoice_kpis_monthly" ON "analytics"."mv_invoice_kpis_monthly" USING "btree" ("company_id", "created_by", "period_month");



CREATE UNIQUE INDEX "ux_mv_quote_cube" ON "analytics"."mv_quote_cube" USING "btree" ("company_id", "created_by", "period_month", "status", "conversion_status", "group_id");



CREATE UNIQUE INDEX "ux_mv_quote_kpis_monthly" ON "analytics"."mv_quote_kpis_monthly" USING "btree" ("company_id", "created_by", "period_month");



CREATE UNIQUE INDEX "ux_mv_quote_top_items_monthly" ON "analytics"."mv_quote_top_items_monthly" USING "btree" ("company_id", "created_by", "period_month", "item_id");



CREATE UNIQUE INDEX "ux_mv_ticket_kpis_monthly" ON "analytics"."mv_ticket_kpis_monthly" USING "btree" ("company_id", "period_month");
...
CREATE INDEX "idx_invoice_meta_company_status" ON "verifactu"."invoice_meta" USING "btree" ("company_id", "status");



CREATE INDEX "idx_verifactu_events_company_status" ON "verifactu"."events" USING "btree" ("company_id", "status");



CREATE INDEX "idx_verifactu_events_dlq_company_status" ON "verifactu"."events_dlq" USING "btree" ("company_id", "status");



CREATE INDEX "idx_verifactu_events_dlq_failed_at" ON "verifactu"."events_dlq" USING "btree" ("failed_at" DESC);



CREATE INDEX "idx_verifactu_events_dlq_invoice" ON "verifactu"."events_dlq" USING "btree" ("invoice_id");



CREATE INDEX "idx_verifactu_events_invoice" ON "verifactu"."events" USING "btree" ("invoice_id");



CREATE UNIQUE INDEX "uq_verifactu_events_invoice_type" ON "verifactu"."events" USING "btree" ("invoice_id", "event_type");
...
CREATE OR REPLACE TRIGGER "trg_audit_verifactu_events" AFTER INSERT OR UPDATE ON "verifactu"."events" FOR EACH ROW EXECUTE FUNCTION "public"."gdpr_audit_trigger"();



CREATE OR REPLACE TRIGGER "trg_audit_verifactu_meta" AFTER INSERT OR UPDATE ON "verifactu"."invoice_meta" FOR EACH ROW EXECUTE FUNCTION "public"."gdpr_audit_trigger"();



CREATE OR REPLACE TRIGGER "trg_verifactu_voiding" AFTER INSERT OR UPDATE ON "verifactu"."events" FOR EACH ROW EXECUTE FUNCTION "public"."handle_verifactu_voiding"();
...
ALTER TABLE ONLY "verifactu"."events_dlq"
    ADD CONSTRAINT "events_dlq_invoice_id_fkey" FOREIGN KEY ("invoice_id") REFERENCES "public"."invoices"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "verifactu"."events_dlq"
    ADD CONSTRAINT "events_dlq_original_event_id_fkey" FOREIGN KEY ("original_event_id") REFERENCES "verifactu"."events"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "verifactu"."events"
    ADD CONSTRAINT "events_invoice_id_fkey" FOREIGN KEY ("invoice_id") REFERENCES "public"."invoices"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "verifactu"."invoice_meta"
    ADD CONSTRAINT "invoice_meta_invoice_id_fkey" FOREIGN KEY ("invoice_id") REFERENCES "public"."invoices"("id") ON DELETE CASCADE;
...
CREATE POLICY "Allow read access for authenticated users" ON "public"."tickets_tags" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "Allow write access for authenticated users" ON "public"."tickets_tags" TO "authenticated" USING (true) WITH CHECK (true);
...
CREATE POLICY "Enable delete for authenticated users" ON "public"."services_tags" FOR DELETE USING (("auth"."role"() = 'authenticated'::"text"));



CREATE POLICY "Enable insert for authenticated users" ON "public"."services_tags" FOR INSERT WITH CHECK (("auth"."role"() = 'authenticated'::"text"));



CREATE POLICY "Enable read access for all users" ON "public"."services_tags" FOR SELECT USING (true);



CREATE POLICY "Enable read access for authenticated users" ON "public"."services_tags" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "Enable read access for authenticated users" ON "public"."tag_scopes" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "Enable write access for authenticated users" ON "public"."services_tags" TO "authenticated" USING (true);



CREATE POLICY "Enable write access for authenticated users" ON "public"."tag_scopes" TO "authenticated" USING (true);
...
CREATE POLICY "global_tags_company_only" ON "public"."global_tags" USING (("company_id" = "public"."get_user_company_id"())) WITH CHECK (("company_id" = "public"."get_user_company_id"()));
...
CREATE POLICY "Users can view own company dlq" ON "verifactu"."events_dlq" FOR SELECT USING (("company_id" = "public"."current_company_id"()));
...
CREATE POLICY "evt_dlq_mod" ON "verifactu"."events_dlq" USING (("company_id" = "public"."current_company_id"())) WITH CHECK (("company_id" = "public"."current_company_id"()));



CREATE POLICY "evt_dlq_sel" ON "verifactu"."events_dlq" FOR SELECT USING (("company_id" = "public"."current_company_id"()));



CREATE POLICY "evt_mod" ON "verifactu"."events" USING (("company_id" = "public"."current_company_id"())) WITH CHECK (("company_id" = "public"."current_company_id"()));



CREATE POLICY "evt_sel" ON "verifactu"."events" FOR SELECT USING (("company_id" = "public"."current_company_id"()));
...
CREATE POLICY "meta_mod" ON "verifactu"."invoice_meta" USING (("company_id" = "public"."current_company_id"())) WITH CHECK (("company_id" = "public"."current_company_id"()));



CREATE POLICY "meta_sel" ON "verifactu"."invoice_meta" FOR SELECT USING (("company_id" = "public"."current_company_id"()));



CREATE POLICY "seq_mod" ON "verifactu"."invoice_sequence" USING (("company_id" = "public"."current_company_id"())) WITH CHECK (("company_id" = "public"."current_company_id"()));



CREATE POLICY "seq_sel" ON "verifactu"."invoice_sequence" FOR SELECT USING (("company_id" = "public"."current_company_id"()));
...
ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."ticket_stages";
...
ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "verifactu"."events";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "verifactu"."invoice_meta";
...
GRANT USAGE ON SCHEMA "verifactu" TO "anon";
GRANT USAGE ON SCHEMA "verifactu" TO "authenticated";
GRANT USAGE ON SCHEMA "verifactu" TO "service_role";
GRANT USAGE ON SCHEMA "verifactu" TO "authenticator";
...
GRANT ALL ON TABLE "verifactu"."events" TO "service_role";
GRANT SELECT,INSERT,UPDATE ON TABLE "verifactu"."events" TO "authenticated";



GRANT ALL ON TABLE "verifactu"."events_dlq" TO "service_role";
GRANT SELECT,INSERT,UPDATE ON TABLE "verifactu"."events_dlq" TO "authenticated";



GRANT ALL ON TABLE "verifactu"."invoice_meta" TO "service_role";
GRANT SELECT,INSERT,UPDATE ON TABLE "verifactu"."invoice_meta" TO "authenticated";



GRANT ALL ON TABLE "verifactu"."invoice_sequence" TO "service_role";
GRANT SELECT,INSERT,UPDATE ON TABLE "verifactu"."invoice_sequence" TO "authenticated";



GRANT ALL ON TABLE "verifactu"."vw_ledger" TO "service_role";
GRANT SELECT,INSERT,UPDATE ON TABLE "verifactu"."vw_ledger" TO "authenticated";
...
-- FIX: Create missing verifactu_certificates table
CREATE TABLE IF NOT EXISTS "public"."verifactu_certificates" (
    ...
    "cert_len" integer,
    "key_len" integer,
    "pass_present" boolean DEFAULT false,
    CONSTRAINT "verifactu_certificates_pkey" PRIMARY KEY ("id")
);

ALTER TABLE "public"."verifactu_certificates" OWNER TO "postgres";
GRANT ALL ON TABLE "public"."verifactu_certificates" TO "anon";
GRANT ALL ON TABLE "public"."verifactu_certificates" TO "authenticated";
GRANT ALL ON TABLE "public"."verifactu_certificates" TO "service_role";


-- FIX: Create missing ticket_stages table
CREATE TABLE IF NOT EXISTS "public"."ticket_stages" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "name" "text" NOT NULL,
    "position" integer NOT NULL,
    "color" "text" NOT NULL,
    "company_id" "uuid",
    "stage_category" "public"."stage_category" DEFAULT 'open'::public.stage_category NOT NULL,
    "workflow_category" "public"."workflow_category" DEFAULT 'ticket'::public.workflow_category NOT NULL,
...
CREATE TABLE IF NOT EXISTS "public"."hidden_stages" (
    ...
    "hidden_by" "uuid",
    CONSTRAINT "hidden_stages_pkey" PRIMARY KEY ("id"),
    CONSTRAINT "hidden_stages_company_id_fkey" FOREIGN KEY ("company_id") REFERENCES "public"."companies"("id") ON DELETE CASCADE,
    CONSTRAINT "hidden_stages_stage_id_fkey" FOREIGN KEY ("stage_id") REFERENCES "public"."ticket_stages"("id") ON DELETE CASCADE,
    CONSTRAINT "hidden_stages_unique_company_stage" UNIQUE ("company_id", "stage_id")
);
...
CREATE POLICY "Enable all for authenticated users" ON "public"."hidden_stages" TO "authenticated" USING (true) WITH CHECK (true);

-- Functions for Stage Configuration
...
BEGIN
  IF p_hide THEN
    INSERT INTO public.hidden_stages (company_id, stage_id, hidden_by)
    VALUES (v_company_id, p_stage_id, auth.uid())
    ON CONFLICT (company_id, stage_id) DO NOTHING;
  ELSE
    DELETE FROM public.hidden_stages
...
CREATE OR REPLACE FUNCTION "public"."get_top_tags"("search_scope" "text", "limit_count" integer) RETURNS SETOF "public"."global_tags"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  v_company_id uuid := public.get_user_company_id();
BEGIN
  IF search_scope = 'tickets' THEN
    RETURN QUERY
    SELECT gt.*
    FROM public.global_tags gt
    JOIN (
        SELECT tag_id, count(*) as usage_count
        FROM public.tickets_tags
        GROUP BY tag_id
    ) usage ON gt.id = usage.tag_id
    WHERE gt.company_id = v_company_id
    ORDER BY usage.usage_count DESC
    LIMIT limit_count;
  ELSIF search_scope = 'clients' THEN
    RETURN QUERY
    SELECT gt.*
    FROM public.global_tags gt
    JOIN (
        SELECT tag_id, count(*) as usage_count
        FROM public.clients_tags
        GROUP BY tag_id
    ) usage ON gt.id = usage.tag_id
    WHERE gt.company_id = v_company_id
    ORDER BY usage.usage_count DESC
    LIMIT limit_count;
  ELSIF search_scope = 'services' THEN
    RETURN QUERY
    SELECT gt.*
    FROM public.global_tags gt
    JOIN (
        SELECT tag_id, count(*) as usage_count
        FROM public.services_tags
        GROUP BY tag_id
    ) usage ON gt.id = usage.tag_id
    WHERE gt.company_id = v_company_id
    ORDER BY usage.usage_count DESC
...
ALTER FUNCTION "public"."get_top_tags"("search_scope" "text", "limit_count" integer) OWNER TO "postgres";
GRANT ALL ON FUNCTION "public"."get_top_tags"("search_scope" "text", "limit_count" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_top_tags"("search_scope" "text", "limit_count" integer) TO "service_role";
...
-- FIX: Create get_payment_integrations function
CREATE OR REPLACE FUNCTION "public"."get_payment_integrations"("p_company_id" "uuid") RETURNS "jsonb"
    LANGUAGE "sql" STABLE SECURITY DEFINER
    AS $$
  SELECT COALESCE(
    jsonb_agg(
...
-- FIX: Create get_verifactu_cert_status function
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'configured', (cert_pem_enc IS NOT NULL),
    'mode', 'encrypted'
  ) INTO v_settings
  ...
  FROM public.verifactu_certificates
  WHERE company_id = p_company_id;
...
-- MIGRATION: RPCs for Admin Module Management (Replacing Edge Functions)

-- 1. admin_list_user_modules
CREATE OR REPLACE FUNCTION "public"."admin_list_user_modules"("p_owner_id" "uuid" DEFAULT NULL) RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  v_users jsonb;
  ...
  v_company_id uuid;
BEGIN
  -- Logic: If p_owner_id is provided, limit to that company? 
  -- For now, let's list all users if admin, or filter by caller's company if needed.
  -- Assuming system admin calls this.
...
ALTER FUNCTION "public"."admin_list_user_modules"("p_owner_id" "uuid") OWNER TO "postgres";
GRANT ALL ON FUNCTION "public"."admin_list_user_modules"("p_owner_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."admin_list_user_modules"("p_owner_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."admin_list_user_modules"("p_owner_id" "uuid") TO "service_role";


-- 2. admin_set_user_module
CREATE OR REPLACE FUNCTION "public"."admin_set_user_module"("p_target_user_id" "uuid", "p_module_key" "text", "p_status" "text") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
BEGIN
  INSERT INTO public.user_modules (user_id, module_key, status)
  VALUES (p_target_user_id, p_module_key, p_status::module_status)
  ON CONFLICT (user_id, module_key)
  DO UPDATE SET status = EXCLUDED.status, updated_at = now();
...
-- FIX: Refresh MV to avoid 500 error on first access (as it is created WITH NO DATA)
-- We wrap in a block to ignore errors if tables are empty
DO $$
BEGIN
  REFRESH MATERIALIZED VIEW "analytics"."mv_ticket_kpis_monthly";
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Could not refresh mv_ticket_kpis_monthly: %', SQLERRM;
END $$;
