-- Migration: Create create_ticket RPC
-- Description: Replaces create-ticket Edge Function with a secure Postgres function.
-- Handles: Permission checks (Staff vs Client), Ticket Creation, Services/Products insertion, Initial Comment, Device linking.
...
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER -- Runs with privileges of the creator (postgres/admin), bypassing RLS to perform cross-checks using clean logic
...
BEGIN
  ...
  IF v_final_stage_id IS NULL THEN
    ...
    IF v_final_stage_id IS NULL THEN
      RAISE EXCEPTION 'No active ticket stages found';
    END IF;
  END IF;
  ...
  INSERT INTO public.tickets (
    ...
    updated_at,
    is_opened
    -- ticket_number is SERIAL/Sequence, automatically handled
  ) VALUES (
    p_company_id,
    p_client_id,
    p_title,
  ...
  IF p_initial_comment IS NOT NULL AND length(trim(p_initial_comment)) > 0 THEN
    ...
    -- NOTE: Clients might not have a public.users row.
    -- ticket_comments.user_id FK usually refers to public.users?
    -- If user is Client, user_id might need to be NULL if the table enforces FK to users.
    -- Let's check if we have a user_id from the staff check.
    -- If Client, pass NULL for user_id (if schema allows) or we might need a client_id column on comments?
    -- Current schema for ticket_comments likely relies on user_id -> users.
    -- If we pass NULL, it shows as "System" or similar?
    -- The Edge function fix was `userRow?.id || null`. We do the same here.
    ...
    INSERT INTO public.ticket_comments (
      ...
      v_ticket_id,
      v_full_comment,
      (SELECT id FROM public.users WHERE auth_user_id = v_auth_user_id LIMIT 1), -- Will be NULL if client
      p_company_id,
      false,
      now()
    );
