-- Migration to fix RPC with self-healing client assignment
-- Created on 2026-01-01-0060
-- V15 consolidated: 
-- 1. Adds auto-link logic: If matched client has NULL auth_user_id, update it to current user.
...
BEGIN
    ...
    -- Look up client via view (matches by email)
    ...
    -- SELF-HEALING: Ensure the client record is linked to this auth user
    -- This fixes cases where RLS blocks visibility because auth_user_id was NULL
    ...
    -- 7. Create Invoice
    -- Get default series ID AND CODE
    ...
    IF v_series_id IS NULL THEN
        RAISE EXCEPTION 'No active invoice series found for company';
    END IF;
