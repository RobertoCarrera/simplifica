-- Fix Owner/Admin default modules in get_effective_modules
-- Date: 2026-01-08 20:30:00
...
begin
    ...
    if p_input_company_id is not null then
        ...
        if v_public_user_id is not null then
            -- Try to infer role if possible from a default company? 
            -- For legacy calls, we might not have company_id, so we can't trust role. 
            -- But usually explicit company_id is sent now.
            -- If we are here, we might just assume default permissions or False if no config.
            v_membership_role := 'member'; -- Default assumption if not provided? Or keep null.
        end if;
            ...
            jsonb_build_object(
                ...
                'enabled', (
                    CASE 
                        ...
                        -- No record found: Default to TRUE for Owners/Admins, FALSE for others
