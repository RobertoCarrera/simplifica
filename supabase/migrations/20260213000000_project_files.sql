...
BEGIN
  ...
  EXECUTE 'CREATE POLICY "Give users access to bucket Select" ON storage.objects FOR SELECT TO authenticated USING (bucket_id = ''project-files'')';
  EXECUTE 'CREATE POLICY "Give users access to bucket Insert" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = ''project-files'')';
  EXECUTE 'CREATE POLICY "Give users access to bucket Delete" ON storage.objects FOR DELETE TO authenticated USING (bucket_id = ''project-files'')';
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'project_files storage: skipped — %', SQLERRM;
END $$;
