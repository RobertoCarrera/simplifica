-- Corregir la función create_rectification_quote para incluir company_id en quote_items
-- Esto soluciona el error 23502 (not_null_violation) al intentar rectificar una factura
...
BEGIN
  ...
  v_quote_number := 'P' || v_year || '-' || LPAD(v_sequence_number::TEXT, 4, '0');
  ...
  FOR v_item IN SELECT * FROM public.invoice_items WHERE invoice_id = p_invoice_id LOOP
    INSERT INTO public.quote_items (
      quote_id,
      company_id, -- IMPORTANTE: Este campo es obligatorio y faltaba en la versión anterior
      line_number,
      description,
      quantity,
      unit_price,
