...
BEGIN
  IF EXISTS (
    ...
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_user_modules_user_id ON public.user_modules (user_id)';
  ELSE
    RAISE NOTICE 'optimize_io: skipped idx_user_modules_user_id — table user_modules does not exist';
  END IF;
END $$;
