-- 1. Fix contract_service_rpc to include recurrence and service IDs
...
BEGIN
    ...
    IF v_client_id IS NULL THEN
        -- Fallback to email search (already implemented logic simplified here for brevity, keeping existing robust logic)
    ...
    -- Resolve Recurrence (weekly, monthly, quarterly, yearly)
...
-- 2. Data Repair Script for GEMMA (Quote 61dcc221-3a88-40f3-b85a-92694cdb3428)
-- We know it's "Posicionamiento Web - Esencial" which is 'monthly'
UPDATE public.quotes 
SET recurrence_type = 'monthly', 
    recurrence_interval = 1,
    next_run_at = created_at + interval '1 month'
WHERE id = '61dcc221-3a88-40f3-b85a-92694cdb3428';

-- Update quote_item to include service_id so portal can find it
UPDATE public.quote_items
SET service_id = (SELECT service_id FROM public.invoice_items WHERE invoice_id = (SELECT invoice_id FROM public.invoices WHERE source_quote_id = '61dcc221-3a88-40f3-b85a-92694cdb3428' LIMIT 1) LIMIT 1),
    variant_id = (SELECT id FROM public.service_variants WHERE variant_name = 'Esencial' AND service_id = (SELECT service_id FROM public.invoice_items WHERE invoice_id = (SELECT invoice_id FROM public.invoices WHERE source_quote_id = '61dcc221-3a88-40f3-b85a-92694cdb3428' LIMIT 1) LIMIT 1) LIMIT 1),
    billing_period = 'monthly'
WHERE quote_id = '61dcc221-3a88-40f3-b85a-92694cdb3428';
