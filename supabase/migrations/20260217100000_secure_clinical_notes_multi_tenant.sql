-- Migration: Secure Clinical Notes with Multi-Tenant RLS & RPC Checks (V2 - Fix Role Column)

-- 1. Enable RLS on client_clinical_notes (just to be safe)
ALTER TABLE public.client_clinical_notes ENABLE ROW LEVEL SECURITY;

-- 2. Drop existing legacy Policies
DROP POLICY IF EXISTS "Users can view notes for their company clients" ON public.client_clinical_notes;
DROP POLICY IF EXISTS "Users can create notes for their company clients" ON public.client_clinical_notes;
DROP POLICY IF EXISTS "Users can update their own notes" ON public.client_clinical_notes;
DROP POLICY IF EXISTS "Users can delete their own notes" ON public.client_clinical_notes;
-- Drop possibly created V1 policies if partial success (unlikely transaction)
DROP POLICY IF EXISTS "clinical_notes_select_policy" ON public.client_clinical_notes;
DROP POLICY IF EXISTS "clinical_notes_insert_policy" ON public.client_clinical_notes;
DROP POLICY IF EXISTS "clinical_notes_update_policy" ON public.client_clinical_notes;
DROP POLICY IF EXISTS "clinical_notes_delete_policy" ON public.client_clinical_notes;

-- 3. Create New Multi-Tenant Policies using company_members
-- SELECT: Active company members can view notes for clients of their company
...
-- INSERT: Active company members can create notes for clients of their company
...
-- Uses app_roles table via role_id since 'role' text column was removed
...
-- 4. Secure RPCs (Security Definer functions need explicit permission checks)
