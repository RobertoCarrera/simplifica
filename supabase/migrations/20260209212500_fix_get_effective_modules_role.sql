-- Fix: get_effective_modules RPC references cm.role which no longer exists
-- The company_members table now uses role_id (FK to app_roles) instead of a role text column.
-- This migration updates the function to join app_roles and use ar.name.
...
begin
    ...
    if p_input_company_id is not null then
        ...
        -- FIX: Use app_roles.name instead of cm.role which no longer exists
