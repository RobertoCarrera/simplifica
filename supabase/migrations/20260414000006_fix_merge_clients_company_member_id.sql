-- Fix: merge_clients referenced client_assignments.professional_id which does not exist.
-- The table client_assignments uses company_member_id (not professional_id).
...
BEGIN
  ...
  -- client_assignments: avoid duplicate (client_id, company_member_id) PK pairs
  ...
  -- clients_tags: avoid duplicate (client_id, tag_id) PK pairs
