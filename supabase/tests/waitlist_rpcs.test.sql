...
-- Force-exhaust both active entries for this slot so T16-09 idempotency check returns 0
UPDATE public.waitlist SET status = 'notified', notified_at = NOW()
WHERE id IN ('e4000000-0000-0000-0000-000000000004', 'e5000000-0000-0000-0000-000000000005');
...
SELECT ok(
  ...
  -- Both entries are now notified, so repeated call returns 0
...
-- (RESET role: notifications has no SELECT policy for authenticated — need postgres role to verify)
RESET role;
SELECT ok(
  (SELECT COUNT(*) > 0
...
WITH upsert AS (
  ...
  RETURNING last_notified_at > NOW() AS is_future
)
SELECT ok((SELECT is_future FROM upsert), 'T16-23: waitlist_rate_limits UPSERT updates last_notified_at correctly');
