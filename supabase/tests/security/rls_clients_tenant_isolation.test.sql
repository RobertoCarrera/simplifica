...
INSERT INTO auth.users (id, email) VALUES
  ('aaaaaaaa-aaaa-0000-0000-000000000001', 'admin-a@test.invalid');

-- Auth user for Company A (employee — read-only role)
INSERT INTO auth.users (id, email) VALUES
  ('aaaaaaaa-aaaa-0000-0000-000000000002', 'employee-a@test.invalid');
...
-- Employee A needs a client_assignment for Client A (required by can_view_client for non-admin members)
INSERT INTO public.client_assignments (client_id, company_member_id)
  SELECT 'cccccccc-0000-0000-0000-000000000001',
         cm.id
  FROM public.company_members cm
  WHERE cm.user_id = 'aaaaaaaa-aaaa-0000-0000-000000000002'
    AND cm.company_id = 'aaaaaaaa-0000-0000-0000-000000000001';
...
SELECT set_config('request.jwt.claim.sub', 'aaaaaaaa-aaaa-0000-0000-000000000001', true);SELECT set_config('request.jwt.claim.role', 'authenticated', true);
SELECT is(
...
-- ============================================================
SET LOCAL role = authenticated;
SELECT set_config('request.jwt.claim.sub', 'aaaaaaaa-aaaa-0000-0000-000000000002', true);
...
-- ============================================================
-- TEST 9: Employee A cannot update Company B client (cross-tenant isolation)
-- ============================================================
UPDATE public.clients SET phone = 'EMPLOYEE_HACK' WHERE id = 'cccccccc-0000-0000-0000-000000000002';
RESET role;
SELECT isnt(
  (SELECT phone FROM public.clients WHERE id = 'cccccccc-0000-0000-0000-000000000002'),
  'EMPLOYEE_HACK',
  'Employee A cannot update Company B client — cross-tenant isolation enforced'
);
...
-- ============================================================
SET LOCAL role = authenticated;
SELECT set_config('request.jwt.claim.sub', 'aaaaaaaa-aaaa-0000-0000-000000000001', true);
SELECT set_config('request.jwt.claim.role', 'authenticated', true);
