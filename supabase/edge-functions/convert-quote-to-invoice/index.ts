...
serve(async (req) => {
  ...
  try {
    ...
    // Allow conversion if accepted OR if it is a rectification quote (internal workflow) OR if it has negative total
    if (quote.status !== 'accepted' && !quote.rectifies_invoice_id && (quote.total_amount ?? 0) >= 0) {
