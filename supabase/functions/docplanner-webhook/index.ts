// @ts-nocheck
// ================================================================
// Edge Function: docplanner-webhook
// ================================================================
// Receives push notifications from DocPlanner (Doctoralia) when
// bookings are created, updated, moved, or cancelled.
//
// Auth: HMAC-SHA256 signature verification using a per-company
//       webhook_secret, or a shared token passed as ?token=xxx.
//       NO JWT required (DocPlanner sends server-to-server).
//
// POST /docplanner-webhook?company_id=xxx&token=yyy
//   Body: DocPlanner notification payload
//
// Events handled:
//   slot-booking       → upsert booking (pull full details from API)
//   booking-canceled   → mark booking cancelled
//   booking-moved      → update booking times
//   booking-confirmed  → update booking status
//   presence-marked    → update booking status
// ================================================================
...
/* ── AES-256-GCM helpers (shared with docplanner-api) ── */
...
async function getValidToken(
  ...
  // Store refreshed token (fire & forget — not critical for webhook processing)
...
async function syncBookingToGoogleCalendar(
  ...
  if (expiresAt.getTime() - Date.now() < 5 * 60 * 1000) {
    ...
    if (refreshed.error || !refreshed.access_token) {
      console.warn('[webhook][syncBookingToGoogleCalendar] Token refresh failed:', refreshed.error);
      return;
    }
  ...
  if (!gcalRes.ok) {
    ...
    console.warn(`[webhook][syncBookingToGoogleCalendar] API error for booking ${bookingId}:`, err);
    return;
  }
...
async function syncBookingToResourceCalendar(
  ...
  try {
    ...
    console.error('[webhook][syncBookingToResourceCalendar] error:', err);
  }
}
...
async function upsertBookingFromDP(
  ...
  const resourceId = effectiveStatus !== 'cancelled'
    ? await assignRoomForBooking(serviceClient, companyId, mapping.professional_id, startTime, endTime || startTime)
    : null;
  ...
  if (bookingId && effectiveStatus !== 'cancelled') {
    try {
      ...
      console.warn('[webhook][upsertBookingFromDP] Calendar sync failed (non-fatal):', e);
    }
  }
}
...
serve(async (req) => {
  ...
  // Load integration to verify webhook is expected
  const { data: integration, error: intErr } = await serviceClient
    .from('docplanner_integrations')
    .select('webhook_secret, is_active, facility_id, doctor_mappings')
    .eq('company_id', companyId)
    .single();
  ...
  // Verify authentication: HMAC signature OR token param
  ...
  if (signatureHeader && integration.webhook_secret) {
    ...
    // Fallback: verify shared token matches the webhook_secret
  ...
  try {
    ...
    // Determine which doctor mapping to use based on notification data
    ...
    const mapping = mappings.find(
      (m: any) => String(m.dp_doctor_id) === doctorId
    );
        ...
        case 'booking-moved': {
          ...
          if (bookingId && facilityId) {
            try {
              ...
              // Prefer webhook-provided addressId, then mapping's stored address
              ...
              if (fullBooking) {
                ...
                synced++;
                // Update mapping address if it was stale
                if (useAddrId && useAddrId !== mapping.address_id) {
                  mapping.address_id = useAddrId;
                  const updated = mappings.map((m: any) =>
                    String(m.dp_doctor_id) === String(mapping.dp_doctor_id) ? { ...m, address_id: useAddrId } : m
                  );
              ...
              errors.push(`Fetch booking ${bookingId}: ${String(e)}`);
              failed++;
            }
          } else {
            errors.push(`Missing booking_id or facility_id for ${eventName}`);
            failed++;
          }
          break;
        }
        ...
        case 'booking-canceled': {
          ...
          if (bookingId) {
            ...
            if (existing) {
              ...
              errors.push(`No local booking found for DP booking ${bookingId}`);
              failed++;
            }
          }
          break;
    ...
    errors.push(`Processing error: ${String(e)}`);
    failed++;
  }
