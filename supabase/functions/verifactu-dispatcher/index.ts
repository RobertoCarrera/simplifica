...
serve(async (req)=>{
  ...
  try {
    ...
    async function requireCompanyAccess(company_id) {
      ...
      if (profileError || !userProfile?.company_id) return { error: 'User has no company' };
      if (userProfile.company_id !== company_id) return { error: 'Access denied for this company', status: 403 };
