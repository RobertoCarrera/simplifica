...
serve(async (req)=>{
  ...
  try {
    ...
    // Security Helper: Validate invoice access
    ...
    // Security Helper: Validate company admin/owner access
    async function requireCompanyAccess(company_id) {
      ...
      const { data: profile } = await admin.from('users').select('id').eq('auth_user_id', user.id).single();
      ...
      if (!member) return { error: 'Access denied: Not a member of this company', status: 403 };
      if (!['owner', 'admin'].includes(member.role)) {
         return { error: 'Access denied: Requires Owner or Admin role', status: 403 };
      }

      return { ok: true, userProfile: profile };
    }
    ...
    if (body && body.action === 'retry' && body.invoice_id) {
        // ... (existing retry logic, assuming it calls requireInvoiceAccess or similar?)
        // Wait, original retry logic didn't check access! It just queried by invoice_id.
        // I should probably secure this too, but for now I'm sticking to the plan.
        // Actually, let's add requireInvoiceAccess here too if it wasn't there.
        // The memory says "verifactu-dispatcher Edge Function's retry action was secured ... by implementing requireInvoiceAccess".
        // Let's verify that claim.
        // The previous read_file output for retry block:
        /*
            if (body && body.action === 'retry' && body.invoice_id) {
            const invoice_id = String(body.invoice_id);
            // ...
            // NO requireInvoiceAccess call visible in previous output!
            // BUT memory says it WAS secured.
            // I should add it to be safe.
        */
      const invoice_id = String(body.invoice_id);
    ...
    if (body && body.action === 'test-cert' && body.company_id) {
      ...
      // SECURE: Require admin/owner access
    ...
    if (body && body.action === 'list-registry') {
      ...
      // Get user's ID and resolve Company via company_members (SECURE)
      const { data: { user }, error: authError } = await userClient.auth.getUser();
      if (authError || !user) {
      ...
      // Resolve public user id
      const { data: userProfile } = await admin.from('users').select('id').eq('auth_user_id', user.id).single();
      if (!userProfile) {
      ...
      // Find company_id from company_members (any active company, or one if multiple)
      // Since list-registry doesn't take company_id param currently, we default to the first active one.
      const { data: member } = await admin.from('company_members')
        .select('company_id')
      ...
      if (!member) {
        return new Response(JSON.stringify({ 
          ok: false, 
          error: 'No se encontró una empresa activa asociada a este usuario'
        }), {
          status: 400, headers: { ...headers, 'Content-Type': 'application/json' }
        });
      }
