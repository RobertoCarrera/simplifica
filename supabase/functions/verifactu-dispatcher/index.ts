...
serve(async (req)=>{
  ...
  try {
    ...
    // Secure access to company-level debug actions
    async function requireCompanyAccess(company_id) {
      ...
      // Get internal user ID to check membership
      const { data: internalUser } = await userClient
        .from('users')
        .select('id')
        .eq('auth_user_id', user.id)
        .maybeSingle();

      if (!internalUser) return { error: 'User profile not found', status: 403 };
      ...
      if (!member) return { error: 'Access denied to this company', status: 403 };
