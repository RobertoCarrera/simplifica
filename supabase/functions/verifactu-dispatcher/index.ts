...
serve(async (req)=>{
  ...
  try {
    ...
    async function requireCompanyAccess(company_id) {
       ...
       // Verify membership using RLS on company_members
    ...
    // SECURITY: Dangerous debug endpoints removed (debug-test-update, debug-env, debug-last-event, debug-aeat-process)
    ...
    if (body && body.action === 'test-cert' && body.company_id) {
      ...
      // SECURITY CHECK: Verify access to company
