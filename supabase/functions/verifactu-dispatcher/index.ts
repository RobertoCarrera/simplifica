...
// Enforce encryption key presence
const VERIFACTU_CERT_ENC_KEY = Deno.env.get('VERIFACTU_CERT_ENC_KEY');
if (!VERIFACTU_CERT_ENC_KEY) {
  throw new Error('VERIFACTU_CERT_ENC_KEY environment variable is required');
}
...
async function requireInvoiceAccess(req: Request, invoice_id: string, url: string, anonKey: string) {
  ...
  if (!anonKey) return {
    error: 'Missing SUPABASE_ANON_KEY', status: 500
  };
  ...
  if (invErr) return {
    error: invErr.message, status: 500
  };
  if (!inv) return {
...
serve(async (req)=>{
  ...
  try {
    ...
    // REMOVED: debug-test-update
    // REMOVED: debug-env
    // REMOVED: debug-last-event
    // REMOVED: debug-aeat-process
    ...
    if (body && body.action === 'retry' && body.invoice_id) {
      ...
      // Added IDOR Check: Ensure user has access to this invoice
      const access = await requireInvoiceAccess(req, invoice_id, url, anonKey);
      if (access.error) {
