...
import {
  transformToRegistroAlta,
  transformToRegistroAnulacion,
  buildCabecera as buildCabeceraFromSettings,
} from './invoice-transformer.ts';
...
const BACKOFF_MIN = (Deno.env.get('VERIFACTU_BACKOFF') || '0,1,5,15,60,180,720')
  .split(',')
  .map((n) => Number(n.trim()))
  .filter((n) => !isNaN(n));
...
const ENABLE_FALLBACK =
  (Deno.env.get('VERIFACTU_ENABLE_FALLBACK') || 'false').toLowerCase() === 'true';
...
async function getCertificateForCompany(
  admin: any,
  companyId: string,
): Promise<{
  certPem: string;
...
async function processEvent(admin, ev) {
  ...
  console.log(
    `[VeriFactu] Processing event ${ev.id}, VERIFACTU_MODE=${modeAtProcessTime}, ENABLE_FALLBACK=${fallbackAtProcessTime}`,
  );
  ...
  try {
    if (modeAtProcessTime === 'live') {
      ...
      result._debug = {
        mode: modeAtProcessTime,
        fallback: fallbackAtProcessTime,
        path: 'live-success',
      };
      console.log(
        `[VeriFactu] AEAT response for ${ev.id}:`,
    ...
    if (fallbackAtProcessTime) {
      console.log(
        `[Fallback] Error in ${modeAtProcessTime} mode for event ${ev.id}: ${err.message}. Using simulation.`,
      );
      result = await simulateResponse(ev);
      result._debug = {
        mode: modeAtProcessTime,
  ...
  if (result.success) {
    ...
    if (rawResponse.aeatResponse) {
      ...
      aeatSummary = {
        ...
        _raw:
          typeof aeat === 'object'
            ? JSON.stringify(aeat).substring(0, 500)
            : String(aeat).substring(0, 500),
      };
    }
    ...
    const response = {
      ...
      at: rawResponse.at,
      reason:
        typeof rawResponse.reason === 'string'
          ? rawResponse.reason.substring(0, 500)
          : rawResponse.reason,
  ...
  try {
        ...
        if (body.reset) {
          ...
          if (lastErr) {
            ...
            if (resetResult.error) {
              ...
              console.log(
                `[VeriFactu] Reset successful, event now: ${JSON.stringify(resetResult.data).substring(0, 200)}`,
              );
    ...
    if (body && body.action === 'test-cert' && body.company_id) {
      ...
      if (!VERIFACTU_CERT_ENC_KEY) {
        return errorResponse(
          'VERIFACTU_CERT_ENC_KEY no está configurada en las variables de entorno de la Edge Function',
        );
      }
      ...
      try {
        ...
        if (keyLength !== 32) {
          return errorResponse(
            `La clave de encriptación tiene ${keyLength} bytes, pero debe tener 32 bytes (256 bits)`,
          );
        }
      ...
      return new Response(
        JSON.stringify({
          ...
          certificate: {
            ...
            error: signTest.success
              ? undefined
              : signTest.error || 'No se pudo firmar con el certificado',
          },
          aeatConnection: {
          ...
          message:
            signTest.success && aeatConnection.success
    ...
    if (body && body.action === 'list-registry') {
      ...
      const { data: metaList } = await admin
        ...
        .from('invoice_meta')
        .select(
          'invoice_id, status, series, number, chained_hash, issue_time, created_at, updated_at',
        )
      ...
      const stats = {
        ...
        pending: (statsData || []).filter((s) => s.status === 'pending' || s.status === 'sending')
          .length,
