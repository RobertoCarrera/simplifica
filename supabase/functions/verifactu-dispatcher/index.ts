...
serve(async (req)=>{
  ...
  try {
    ...
    // Uses authenticated user's company. Returns certificate status without exposing sensitive data.
    if (body && body.action === 'test-cert') {
      const authHeader = req.headers.get('authorization') || '';
      const token = (authHeader.match(/^Bearer\s+(.+)$/i) || [])[1];
      if (!token) {
        return new Response(JSON.stringify({ ok: false, error: 'Missing Bearer token' }), { status: 401, headers: { ...headers, 'Content-Type': 'application/json' } });
      }
      ...
      if (authError || !user) {
        return new Response(JSON.stringify({ ok: false, error: 'Invalid token' }), { status: 401, headers: { ...headers, 'Content-Type': 'application/json' } });
      }
      ...
      if (profileError || !userProfile?.company_id) {
        return new Response(JSON.stringify({ ok: false, error: 'User has no company' }), { status: 400, headers: { ...headers, 'Content-Type': 'application/json' } });
      }
