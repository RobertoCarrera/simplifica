...
serve(async (req)=>{
  ...
  try {
    ...
    // Secure helper: Validates that the user belongs to the requested company_id using RLS
    async function requireCompanyAccess(company_id) {
      ...
      // Verify user has access to company via RLS on users table
      const { data: user, error: authError } = await userClient.auth.getUser();
      if (authError || !user?.user) return { error: 'Invalid token', status: 401 };
      ...
      if (!profile) return { error: 'Access denied for this company', status: 403 };
