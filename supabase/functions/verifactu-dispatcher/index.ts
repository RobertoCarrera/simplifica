...
serve(async (req)=>{
  ...
  try {
    ...
    // NEW: Helper for company access (Replaces insecure debug access)
    async function requireCompanyAccess(company_id) {
      ...
      // Check membership in the company via RLS
      ...
      if (error || !data) {
         return { error: 'Acceso denegado a los recursos de la empresa', status: 403 };
      }
      return { ok: true };
    }
    
    // DEBUG endpoints removed for security (IDOR prevention)
