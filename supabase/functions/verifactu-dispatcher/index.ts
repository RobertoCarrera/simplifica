...
serve(async (req)=>{
  ...
  try {
    ...
    // Helper to check if the caller belongs to the target company (prevents IDOR)
    async function requireCompanyAccess(company_id) {
      ...
      // Verify user has access to this company via RLS
      ...
      if (error || !data) {
        return { error: 'Unauthorized: You do not have access to this company', status: 403 };
      }
      return { ok: true };
    }
    ...
    if (body && body.action === 'retry' && body.invoice_id) {
      ...
      // Require invoice access for retry too
