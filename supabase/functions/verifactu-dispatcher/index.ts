...
// Helper: For actions that require validating the caller against RLS (per-invoice access)
// create a user-scoped client from the Authorization header and ensure the invoice exists for them.
async function requireInvoiceAccess(authHeader: string, supabaseUrl: string, supabaseAnonKey: string, invoice_id: string) {
...
serve(async (req)=>{
  ...
  try {
    ...
    if (body && body.action === 'retry' && body.invoice_id) {
      ...
      // SECURITY: Require access to the invoice before allowing retry
      const authHeader = req.headers.get('authorization') || '';
      const access = await requireInvoiceAccess(authHeader, url, anonKey, invoice_id);
