...
serve(async (req)=>{
  ...
  try {
    ...
    // Validate caller access to company
    async function requireCompanyAccess(company_id) {
      const authHeader = req.headers.get('authorization') || '';
      const token = (authHeader.match(/^Bearer\s+(.+)$/i) || [])[1];
      if (!token) return { error: 'Missing Bearer token', status: 401 };
      
      // Use Deno.env.get directly to avoid closure dependency/scope issues
      ...
      // Ensure strict string comparison to avoid type mismatch (e.g. UUID vs string)
      if (String(userProfile.company_id) !== String(company_id)) {
        return { error: 'Unauthorized access to company', status: 403 };
      }
