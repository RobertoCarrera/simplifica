...
serve(async (req)=>{
  ...
  try {
    ...
    // Requires authentication. Returns certificate status without exposing sensitive data.
    if (body && body.action === 'test-cert') {
      ...
      if (profileError || !userProfile?.company_id) {
        return new Response(JSON.stringify({ ok: false, error: 'No se pudo determinar la empresa del usuario' }), {
