...
serve(async (req)=>{
  ...
  try {
    ...
    // Secure access check for company-level actions
    async function requireCompanyAccess(company_id) {
      ...
      // 1. Verify token and get Auth User ID
      const { data: { user }, error: authError } = await userClient.auth.getUser();
      if (authError || !user) return { error: 'Invalid token', status: 401 };

      // 2. Resolve Public User ID (since it differs from Auth ID)
      ...
      // 3. Check membership (Primary company OR company_members table)
      let isMember = false;

      if (publicUser.company_id === company_id) {
          isMember = true;
      } else {
      ...
      if (!isMember) return { error: 'Unauthorized: You do not have access to this company', status: 403 };
