...
serve(async (req)=>{
  ...
  try {
    ...
    async function requireCompanyAccess(company_id) {
      ...
      // Use company_members to verify access (RLS ensures user can only see their own memberships)
      ...
      if (!member) return { error: 'Access denied: You are not a member of this company', status: 403 };
      if (member.status !== 'active') return { error: 'Access denied: Your membership is not active', status: 403 };
