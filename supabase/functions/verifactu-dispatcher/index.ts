...
serve(async (req)=>{
  ...
  try {
    ...
    // 1. HELPER: Require Invoice Access (User Scope)
    ...
    // 2. HELPER: Require Company Membership (User Scope)
    async function requireCompanyMembership(company_id) {
      ...
      // Verify membership via RLS on company_members
      ...
      if (memberError || !member) return { error: 'Access denied to company', status: 403 };

      return { ok: true, role: member.role, user };
    }
    
    // DEBUG: Test update operation on events (PROTECTED)
    if (body && body.action === 'debug-test-update' && body.company_id) {
      const access = await requireCompanyMembership(body.company_id);
      if (access.error) return new Response(JSON.stringify({ ok: false, error: access.error }), { status: access.status || 400, headers: { ...headers, 'Content-Type': 'application/json' } });
      ...
      if (getErr || !lastEvent) {
        return new Response(JSON.stringify({ ok: false, error: getErr?.message || 'No event found' }), { status: 400, headers: { ...headers, 'Content-Type': 'application/json' } });
      }
      ...
      const { data: updateData, error: updateErr } = await admin
        .schema('verifactu')
        .from('events')
        .update({ attempts: testAttempts, last_error: testError })
        .eq('id', lastEvent.id)
        .select()
        .single();
    ...
    // DEBUG: Get last event for a company (PROTECTED)
    if (body && body.action === 'debug-last-event' && body.company_id) {
      ...
      return new Response(JSON.stringify({ ok: true, event: lastEvent, error: evErr?.message }), { status: 200, headers: { ...headers, 'Content-Type': 'application/json' } });
    }
    
    // DEBUG: Test AEAT process steps for a specific company (PROTECTED)
    if (body && body.action === 'debug-aeat-process' && body.company_id) {
      ...
      try {
        ...
        if (!cert) {
          return new Response(JSON.stringify({ ok: false, error: 'Certificate not found for company', steps }), { status: 400, headers: { ...headers, 'Content-Type': 'application/json' } });
        }
        steps.certOk = true;
        ...
        if (evErr) return new Response(JSON.stringify({ ok: false, error: `Event query error: ${evErr.message}`, steps }), { status: 400, headers: { ...headers, 'Content-Type': 'application/json' } });
        if (!events || events.length === 0) return new Response(JSON.stringify({ ok: false, error: 'No pending events for this company', steps }), { status: 400, headers: { ...headers, 'Content-Type': 'application/json' } });
        ...
        return new Response(JSON.stringify({ ok: true, result, updatedEvent: updatedEv, steps, config: { VERIFACTU_MODE, ENABLE_FALLBACK } }), { status: 200, headers: { ...headers, 'Content-Type': 'application/json' } });
        
      } catch (err) {
        return new Response(JSON.stringify({ ok: false, error: err.message, stack: err.stack, steps }), { status: 500, headers: { ...headers, 'Content-Type': 'application/json' } });
      }
    }
    
    // Safe manual retry: reset last rejected event to pending for an invoice (PROTECTED via requireInvoiceAccess)
    if (body && body.action === 'retry' && body.invoice_id) {
      const invoice_id = String(body.invoice_id);

      // Check invoice access first
    ...
    // Expose non-sensitive dispatcher configuration (PUBLIC OK)
    if (body && body.action === 'config') {
      return new Response(JSON.stringify({ ok: true, maxAttempts: MAX_ATTEMPTS, backoffMinutes: BACKOFF_MIN, mode: VERIFACTU_MODE, fallbackEnabled: ENABLE_FALLBACK, certEncKeyConfigured: !!VERIFACTU_CERT_ENC_KEY }), { status: 200, headers: { ...headers, 'Content-Type': 'application/json' } });
    }

    // Test certificate (PROTECTED)
    if (body && body.action === 'test-cert' && body.company_id) {
      const company_id = String(body.company_id);
      const access = await requireCompanyMembership(company_id);
      if (access.error) return new Response(JSON.stringify({ ok: false, error: access.error }), { status: access.status || 400, headers: { ...headers, 'Content-Type': 'application/json' } });
      ...
      if (!VERIFACTU_CERT_ENC_KEY) return errorResponse('VERIFACTU_CERT_ENC_KEY no está configurada en las variables de entorno de la Edge Function');
      
      let keyLength = 0;
      try {
        const keyBytes = Uint8Array.from(atob(VERIFACTU_CERT_ENC_KEY), c => c.charCodeAt(0));
        keyLength = keyBytes.length;
        if (keyLength !== 32) return errorResponse(`La clave de encriptación tiene ${keyLength} bytes, pero debe tener 32 bytes (256 bits)`);
      ...
      try {
        ...
        return errorResponse(`Error al desencriptar (formato: ${certFormat}, len: ${certLen}): ${decryptErr.message}. Posible causa: la clave VERIFACTU_CERT_ENC_KEY no coincide con la usada al subir el certificado.`);
      }
      ...
      try {
        ...
        const aeatEndpoint = env === 'prod' ? 'https://www1.agenciatributaria.gob.es/wlpl/TIKE-CONT/ws/SistemaFacturacion/VerifactuSOAP' : 'https://prewww1.aeat.es/wlpl/TIKE-CONT/ws/SistemaFacturacion/VerifactuSOAP';
        const startTime = Date.now();
        const aeatResp = await fetch(aeatEndpoint, { method: 'GET', headers: { 'Accept': 'text/html' } });
        const responseTime = Date.now() - startTime;
        aeatConnection = { success: aeatResp.status > 0, endpoint: aeatEndpoint, httpStatus: aeatResp.status, responseTime, error: null };
      } catch (aeatErr: any) {
    ...
    // Health summary (PROTECTED - requires valid token)
    if (body && body.action === 'health') {
      ...
      return new Response(JSON.stringify({ ok: true, pending, lastEventAt, lastAcceptedAt, lastRejectedAt }), { status: 200, headers: { ...headers, 'Content-Type': 'application/json' } });
    }

    // Secure proxy: per-invoice VeriFactu metadata (PROTECTED via requireInvoiceAccess)
    ...
    // Secure proxy: per-invoice VeriFactu events (PROTECTED via requireInvoiceAccess)
    if (body && body.action === 'events' && body.invoice_id) {
      ...
      const { data: events, error: evErr } = await admin.schema('verifactu').from('events').select('*').eq('invoice_id', invoice_id).order('created_at', { ascending: false }).limit(limit);
      if (evErr) return new Response(JSON.stringify({ ok: false, error: evErr.message }), { status: 400, headers: { ...headers, 'Content-Type': 'application/json' } });
      return new Response(JSON.stringify({ ok: true, events: events || [] }), { status: 200, headers: { ...headers, 'Content-Type': 'application/json' } });
    }

    // List all invoices with VeriFactu status (PROTECTED via User Client)
    if (body && body.action === 'list-registry') {
      ...
      for (const ev of (eventsList || [])) {
        if (!eventsByInvoice.has(ev.invoice_id)) eventsByInvoice.set(ev.invoice_id, ev);
      }
    ...
    // Diagnostic: verify access to verifactu schema (PROTECTED - requires valid token)
    ...
    // Default: Poll for pending events (Background Process)
    // This is safe to leave publicly triggerable as it only processes existing pending records
    // which are inserted by authorized users.
    // However, we could restrict it to cron calls or authenticated users if desired.
    // For now, leaving as-is but checking error handling.
