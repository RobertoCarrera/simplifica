...
serve(async (req)=>{
  ...
  try {
    ...
    async function verifyCompanyAccess(targetCompanyId) {
      const authHeader = req.headers.get('authorization') || '';
      const token = (authHeader.match(/^Bearer\s+(.+)$/i) || [])[1];
      ...
      if (String(userProfile.company_id) !== String(targetCompanyId)) {
         return { error: 'Unauthorized: Company mismatch', status: 403 };
      }
    ...
    if (body && body.action === 'test-cert' && body.company_id) {
      ...
      const access = await verifyCompanyAccess(company_id);
      if (access.error) {
        return new Response(JSON.stringify({
          ok: false,
          error: access.error
