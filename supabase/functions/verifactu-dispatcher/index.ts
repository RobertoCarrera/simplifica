...
serve(async (req)=>{
  ...
  try {
    ...
    // Helper: Validate caller is a member of the company (IDOR protection)
    async function requireCompanyAccess(company_id) {
      ...
      if (!member) return { error: 'User is not a member of this company', status: 403 };
    ...
    if (body && body.action === 'debug-test-update' && body.company_id) {
      ...
      if (access.error) return new Response(JSON.stringify(access), { status: access.status || 401, headers: { ...headers, 'Content-Type': 'application/json' } });
    ...
    if (body && body.action === 'debug-env') {
      // Require valid company member to access debug info
      if (!body.company_id) return new Response(JSON.stringify({ error: 'company_id required for auth' }), { status: 400, headers: { ...headers, 'Content-Type': 'application/json' } });
      const access = await requireCompanyAccess(body.company_id);
      if (access.error) return new Response(JSON.stringify(access), { status: access.status || 401, headers: { ...headers, 'Content-Type': 'application/json' } });
    ...
    // Diagnostic: verify access to verifactu schema objects & sample data (scoped to company)
    if (body && body.action === 'diag') {
      if (!body.company_id) return new Response(JSON.stringify({ error: 'company_id required for diag' }), { status: 400, headers: { ...headers, 'Content-Type': 'application/json' } });
      const access = await requireCompanyAccess(body.company_id);
      if (access.error) return new Response(JSON.stringify(access), { status: access.status || 401, headers: { ...headers, 'Content-Type': 'application/json' } });
      ...
      // Test events table (scoped)
      const evTest = await admin.schema('verifactu').from('events')
      ...
      // Test invoice_meta table (scoped)
      const metaTest = await admin.schema('verifactu').from('invoice_meta')
      ...
      // Count pending events (head query) - Scoped
      const pendingHead = await admin.schema('verifactu').from('events')
