...
serve(async (req)=>{
  ...
  try {
    ...
    // NOTE: Debug endpoints (debug-test-update, debug-env, debug-last-event, debug-aeat-process)
    // have been removed for security (IDOR prevention).
    ...
    if (body && body.action === 'retry' && body.invoice_id) {
      ...
      // SECURITY: Verify user has access to this invoice (INLINED)
      ...
      if (!anonKey) {
        return new Response(JSON.stringify({ok:false, error:'Missing SUPABASE_ANON_KEY'}), {
    ...
    if (body && body.action === 'test-cert' && body.company_id) {
      ...
      // SECURITY: Verify user is admin/owner of this company
      const authHeader = req.headers.get('authorization') || '';
      const token = (authHeader.match(/^Bearer\s+(.+)$/i) || [])[1];
      if (!token) {
        return new Response(JSON.stringify({ok:false, error:'Missing Bearer token'}), {
      ...
      // Check membership (owner or admin)
      ...
      if (!member) {
         return new Response(JSON.stringify({ok:false, error:'Unauthorized: You are not an admin of this company'}), {
    ...
    // SECURED: Reduced information leakage. Only return generic OK unless auth provided?
    // For now, removing detailed stats.
