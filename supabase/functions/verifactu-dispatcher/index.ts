...
serve(async (req)=>{
  ...
  try {
    ...
    // Secure Check for Company Access (Admin/Owner)
    async function requireCompanyAccess(company_id) {
      ...
      if (!['owner', 'admin'].includes(member.role)) return { error: 'Insufficient privileges', status: 403 };
    ...
    if (body && body.action === 'retry' && body.invoice_id) {
      ...
      // SECURITY CHECK: Verify user access to invoice
    ...
    if (body && body.action === 'test-cert' && body.company_id) {
      ...
      // SECURITY CHECK: Verify user is admin/owner of company
      const access = await requireCompanyAccess(company_id);
      if (access.error) {
        return new Response(JSON.stringify({
          ok: false,
          error: access.error
        }), { status: access.status || 403, headers: { ...headers, 'Content-Type': 'application/json' } });
      }
