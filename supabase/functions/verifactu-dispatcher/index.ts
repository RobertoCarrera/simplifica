...
serve(async (req)=>{
  ...
  try {
    ...
    // DEBUG endpoints removed for security (2026-04-21)
    ...
    if (body && body.action === 'retry' && body.invoice_id) {
      ...
      // >>> SECURITY FIX: Require invoice access
    ...
    if (body && body.action === 'test-cert' && body.company_id) {
      ...
      // >>> SECURITY FIX: Verify access to company
      const authHeader = req.headers.get('authorization') || '';
      const token = (authHeader.match(/^Bearer\s+(.+)$/i) || [])[1];
      ...
      // Check access by trying to read verifactu_settings for this company
      // This works because verifactu_settings RLS restricts access to company owners/admins
      ...
      if (accessErr || !accessCheck) {
         return new Response(JSON.stringify({ ok: false, error: 'Access denied to company settings' }), { status: 403, headers: { ...headers, 'Content-Type': 'application/json' } });
      }
      // <<< END SECURITY FIX
