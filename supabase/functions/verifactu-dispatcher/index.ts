...
serve(async (req)=>{
  ...
  try {
    ...
    // Secure proxy: check if user belongs to company (via RLS)
    async function requireCompanyAccess(company_id) {
      ...
      // Check if user has access to company (via RLS on companies table)
      ...
      if (!co) return { error: 'Company not found or access denied', status: 403 };
      
      return { ok: true };
    }
    
    // NOTE: Debug endpoints (debug-test-update, debug-env, debug-last-event, debug-aeat-process, diag)
    // have been removed for security (March 2026 Audit).
    ...
    if (body && body.action === 'test-cert' && body.company_id) {
      ...
      // SECURITY FIX: Verify user belongs to the company
