...
serve(async (req)=>{
  ...
  try {
    ...
    // Helper to validate company access for actions that don't pass an invoice_id
    async function requireCompanyAccess(company_id) {
      ...
      // Check if user is a member of the company (using RLS on public.company_members)
      // We use limit(1) because we just need to know if any record exists for this company
      // visible to the user (either their own membership or others if admin).
