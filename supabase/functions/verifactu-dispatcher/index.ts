...
async function sendToAeat(admin: any, ev: any): Promise<{ success: boolean; response: any }> {
  ...
  // Build settings object for transformer
  const settings = {
...
async function requireInvoiceAccess(req, invoice_id) {
...
// NEW: Ensure caller belongs to the company (Employee/Owner)
async function requireCompanyAccess(req, admin, company_id) {
    const authHeader = req.headers.get('authorization') || '';
    const token = (authHeader.match(/^Bearer\s+(.+)$/i) || [])[1];
    ...
    const { data: userProfile } = await userClient.from('users').select('id').eq('auth_user_id', user.id).single();
    if (!userProfile) return { error: 'User profile not found', status: 403 };

    // Check membership using admin to allow finding the record (assuming RLS might hide other companies, which is good, but we want to be sure)
    ...
    if (!member) return { error: 'Access denied to company', status: 403 };
...
serve(async (req)=>{
  ...
  try {
    ...
    if (body && body.action === 'retry' && body.invoice_id) {
      ...
      // SECURITY CHECK: Ensure user has access to this invoice
      const access = await requireInvoiceAccess(req, invoice_id);
      if (access.error) {
    ...
    if (body && body.action === 'test-cert' && body.company_id) {
      ...
      // SECURITY CHECK: Ensure user belongs to the company
      const access = await requireCompanyAccess(req, admin, company_id);
      if (access.error) {
         return new Response(JSON.stringify({
            ok: false,
            error: access.error
