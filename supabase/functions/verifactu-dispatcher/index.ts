...
serve(async (req)=>{
  ...
  try {
    ...
    // Validate caller access to invoice (IDOR Check)
    ...
    // Validate caller access to company (IDOR Check)
    async function requireCompanyAccess(company_id) {
      ...
      // Verify if user belongs to the target company
      // This relies on RLS policies of public.users: users should be able to read their own profile
    ...
    // REMOVED INSECURE DEBUG ACTIONS: debug-test-update, debug-env, debug-last-event, debug-aeat-process
    ...
    if (body && body.action === 'test-cert' && body.company_id) {
      ...
      // SECURITY CHECK: Verify user belongs to the company
      const access = await requireCompanyAccess(company_id);
      if (access.error) {
        return new Response(JSON.stringify({
          ok: false,
          error: access.error,
          certificate: { valid: false, error: 'Acceso denegado' }
        }), {
    ...
    if (body && body.action === 'list-registry') {
      const access = await requireCompanyAccess(body.company_id || ''); // body.company_id not passed often for list, usually inferred.

      // But here the code was extracting company from user manually.
      // Let's refactor to use requireCompanyAccess?
      // Actually, list-registry inferred company_id from the token, rather than validating a passed company_id.
      // So I will keep the existing logic but use the logic from requireCompanyAccess style if possible,
      // OR better: keep existing logic as it was safe (derived from token).
      // The issue was `test-cert` accepting a company_id without verifying the token owner belongs to it.

      // I'll keep list-registry logic as is, but maybe clean it up?
      // No, "Changes should be minimal". I'll stick to removing debug actions and securing test-cert.

      // Get user from token to find their company (Original logic preserved)
      const authHeader = req.headers.get('authorization') || '';
      const token = (authHeader.match(/^Bearer\s+(.+)$/i) || [])[1];
      if (!token) {
