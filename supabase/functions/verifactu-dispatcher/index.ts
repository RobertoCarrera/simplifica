...
async function sendToAeat(admin: any, ev: any) {
  ...
  let { data: lines, error: linesErr } = await admin
    .from('invoice_items')
    .select('*')
  ...
  const aeatClient = await createAEATClient({
    environment: cert.environment,
    certificate: { pem: cert.certPem, privateKey: cert.keyPem, keyPassword: cert.keyPass },
    retryOnError: true,
    maxRetries: 2
  });
...
async function processEvent(admin, ev) {
  ...
  if (result.success) {
    ...
    const { error: updateErr } = await admin
      .schema('verifactu')
      .from('events')
      .update({ status: 'accepted', response: { ...result.response, _debug: result._debug } })
      .eq('id', ev.id);
    ...
    if (attempts >= MAX_ATTEMPTS) {
      await admin
        .schema('verifactu')
        .from('events')
        .update({ status: 'rejected', attempts, last_error: 'max_attempts', response })
        .eq('id', ev.id);
      ...
      const { error: retryErr } = await admin
        .schema('verifactu')
        .from('events')
        .update({ status: 'pending', attempts, last_error: 'retry', response })
        .eq('id', ev.id);
...
serve(async (req) => {
  ...
  try {
    ...
    // HELPER: Require user to have access to the invoice
    ...
    // HELPER: Require user to be member of the company
    async function requireCompanyAccess(company_id) {
      ...
      if (!publicUser) return { error: 'User profile not found' };
      ...
      if (!membership) {
        return { error: 'Access denied: You are not an active member of this company', status: 403 };
      }
    ...
    if (body && body.action === 'test-cert' && body.company_id) {
      ...
      if (!VERIFACTU_CERT_ENC_KEY) return errorResponse('VERIFACTU_CERT_ENC_KEY no está configurada');
      ...
      try {
        ...
        return errorResponse(`Error al desencriptar: ${decryptErr.message}`);
      }
      ...
      if (!certValid || !keyValid) {
        return new Response(JSON.stringify({
          ...
          certificate: { valid: false, error: 'Formato PEM inválido' },
      ...
      return new Response(JSON.stringify({
        ...
        decryption: { success: true, certLength: certPem.length, keyLength: keyPem.length, hasPassphrase: !!keyPass },
        certificate: { valid: certValid && keyValid && signTest.success, subject: 'Ver detalles en el certificado original', error: signTest.success ? undefined : signTest.error },
        aeatConnection: { success: aeatConnection.success, endpoint: aeatConnection.endpoint, httpStatus: aeatConnection.httpStatus, responseTime: aeatConnection.responseTime, error: aeatConnection.error },
        config: { environment: settings.environment, issuerNif: settings.issuer_nif, softwareCode: settings.software_code || '' },
        message: signTest.success && aeatConnection.success ? '🎉 Certificado OK' : '⚠️ Problemas con el certificado'
      }), { status: 200, headers: { ...headers, 'Content-Type': 'application/json' } });
    }

    // Meta: Secured (Invoice Access)
    ...
    // List Registry: Secured (Token based)
    if (body && body.action === 'list-registry') {
      ...
      const { data: userProfile } = await userClient.from('users').select('company_id').eq('auth_user_id', user.id).single();
      if (!userProfile?.company_id) return new Response(JSON.stringify({ ok: false, error: 'No company found' }), { status: 400, headers: { ...headers, 'Content-Type': 'application/json' } });
      ...
      const { data: invoices, error: listErr, count } = await admin.from('invoices').select(`id, full_invoice_number, invoice_date, status, total, currency, created_at, client:clients!invoices_client_id_fkey(name, apellidos, business_name)`, { count: 'exact' }).eq('company_id', companyId).order('created_at', { ascending: false }).range(offset, offset + pageSize - 1);
      ...
      const { data: metaList } = await admin.schema('verifactu').from('invoice_meta').select('*').in('invoice_id', invoiceIds);
      const { data: eventsList } = await admin.schema('verifactu').from('events').select('*').in('invoice_id', invoiceIds).order('created_at', { ascending: false });
    ...
    if (body && body.action === 'retry' && body.invoice_id) {
      ...
      if (evErr) return new Response(JSON.stringify({ ok: false, error: evErr.message }), { status: 400, headers: { ...headers, 'Content-Type': 'application/json' } });
      if (!ev) return new Response(JSON.stringify({ ok: false, message: 'No rejected event to retry' }), { status: 200, headers: { ...headers, 'Content-Type': 'application/json' } });
    ...
    // Process Pending (Internal cron - could be secured but relied on no body actions)
