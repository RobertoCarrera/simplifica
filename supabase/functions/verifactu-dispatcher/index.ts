...
serve(async (req)=>{
  ...
  try {
    ...
    // Helper to check if the user belongs to the target company (IDOR protection)
    async function requireCompanyAccess(targetCompanyId) {
      ...
      const anonKey = Deno.env.get('SUPABASE_ANON_KEY') || '';
      const { data: { user }, error: authErr } = await createClient(url, anonKey, {
      ...
      const { data: publicUser } = await admin.from('users').select('id').eq('auth_user_id', user.id).single();
      ...
      if (!member) return { error: 'Unauthorized: Access denied for this company', status: 403 };
      return { ok: true };
    }
