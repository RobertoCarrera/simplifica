...
// Helper: Check if caller has access to invoice (RLS via Anon Key + Auth Header)
async function checkInvoiceAccess(req, invoiceId, url, anonKey) {
  const authHeader = req.headers.get('authorization');
  if (!authHeader) return { error: 'Missing Bearer token', status: 401 };
...
serve(async (req)=>{
  ...
  try {
    ...
    if (body && body.action === 'retry' && body.invoice_id) {
      ...
      // Security Check: Ensure caller has access to this invoice
      const access = await checkInvoiceAccess(req, invoice_id, url, anonKey);
      if (access.error) {
