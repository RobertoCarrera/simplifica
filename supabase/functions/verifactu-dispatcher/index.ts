...
serve(async (req)=>{
  ...
  try {
    ...
    // SECURITY HELPER: Ensure the caller is a member of the company
    async function requireCompanyAccess(company_id) {
      ...
      // Map auth_user_id to internal public.users id
      ...
      if (userErr || !userData) return { error: 'User not found', status: 403 };
      ...
      if (memberErr) return { error: memberErr.message, status: 500 };
    ...
    // REMOVED FOR SECURITY (exposed env vars)
    ...
    if (body && body.action === 'retry' && body.invoice_id) {
      ...
      // Validate access to the invoice (RLS equivalent)
