...
serve(async (req) => {
  ...
  try {
    ...
    const CLIENT_PORTAL_URL = Deno.env.get('CLIENT_PORTAL_URL') ?? 'https://portal.simplificacrm.es';
    ...
    const effectivePortalUrl = requestedPortalUrl && ALLOWED_PORTAL_ORIGINS.includes(requestedPortalUrl) ? requestedPortalUrl : CLIENT_PORTAL_URL;
    ...
    const portalAdmin = CLIENT_PORTAL_SUPABASE_URL && CLIENT_PORTAL_SERVICE_ROLE_KEY ? createClient(CLIENT_PORTAL_SUPABASE_URL, CLIENT_PORTAL_SERVICE_ROLE_KEY, {
      auth: {
        autoRefreshToken: false,
        persistSession: false
      }
    ...
    const { data: userData, error: userErr } = await supabaseAdmin.from('users').select('id, active, app_role:app_roles(name)').eq('auth_user_id', authUserId).single();
    ...
    let membershipQuery = supabaseAdmin.from('company_members').select('company_id, role_data:app_roles(name)').eq('user_id', userData.id).eq('status', 'active');
    ...
    let validMemberships = allMemberships.filter((m)=>{
    ...
    let existingPendingInviteQuery = supabaseAdmin.from('company_invitations').select('id, token, send_count').eq('email', email).eq('status', 'pending').order('created_at', {
      ascending: false
    }).limit(1);
    if (role === 'owner' && isSuperAdmin) {
      existingPendingInviteQuery = existingPendingInviteQuery.is('company_id', null).eq('invited_by_user_id', currentUser.id);
    } else {
      existingPendingInviteQuery = existingPendingInviteQuery.eq('company_id', currentUser.company_id);
    }
    const { data: existingPendingInvite, error: existingPendingInviteErr } = await existingPendingInviteQuery.maybeSingle();
    ...
    if (existingPendingInvite) {
      if (!isResend) {
        console.log('[send-company-invite] Resuming existing pending invite (was not marked as resend)', { email, existingRole: existingPendingInvite.role || 'unknown', newRole: role });
      }
      const { data: updated, error: updErr } = await supabaseAdmin.from('company_invitations').update({
        role: role,
        status: 'pending',
      ...
      const { data: created, error: createErr } = await supabaseAdmin.from('company_invitations').insert({
      ...
      if (createErr) {
        ...
        if (createErr.code === '23505') {
          let existingQuery = supabaseAdmin.from('company_invitations').select('id, token, send_count').eq('email', email);
          if (role === 'owner' && isSuperAdmin) {
            existingQuery = existingQuery.is('company_id', null).eq('invited_by_user_id', currentUser.id);
          } else {
    ...
    const safeRedirectUrl = isClientInvite ? `${effectivePortalUrl}/invite?token=${inviteToken}` : `${redirectBase}/invite?token=${inviteToken}`;
    ...
    const inviteLink = isClientInvite ? `${effectivePortalUrl}/invite?token=${inviteToken}` : `${redirectBase}/invite?token=${inviteToken}`;
    ...
    if (brandedEmailResult.success) {
      ...
      if (!emailSent) {
        ...
        try {
          const { error: otpErr } = await withTimeout(
            inviteAdminClient.auth.signInWithOtp({
              email,
              options: {
        ...
        if (isClientInvite && portalAdmin && !portalAuthUserId) {
          ...
          try {
            const { data: existingRow } = await portalAdmin.from('client_portal_users').select('auth_user_id').eq('email', email).maybeSingle();
            portalAuthUserId = existingRow?.auth_user_id ?? null;
            if (portalAuthUserId) {
              console.log('send-company-invite: portalAuthUserId from existing client_portal_users', {
                portalAuthUserId
              });
            }
          } catch (e) {
          ...
          if (!portalAuthUserId) {
            try {
              const { data: listData } = await withTimeout(
                inviteAdminClient.auth.admin.listUsers({ page: 1, perPage: 500 }),
                10000, 'listUsers'
              );
              const matchedUser = listData?.users?.find((u)=>u.email?.toLowerCase() === email.toLowerCase());
              portalAuthUserId = matchedUser?.id ?? null;
              if (portalAuthUserId) {
                console.log('send-company-invite: portalAuthUserId from listUsers', {
                  portalAuthUserId
                });
              } else {
                console.warn('send-company-invite: user not found in portal auth.users', {
                  email
                });
              }
            } catch (lookupErr) {
      ...
      if (isClientInvite && portalAdmin) {
        try {
          ...
          if (portalAuthUserId) {
            ...
            const { data: clientRow } = await supabaseAdmin.from('clients').select('id').eq('email', email).eq('company_id', currentUser.company_id).maybeSingle();
            const clientId = clientRow?.id ?? null;
            if (!clientId) {
              console.log('send-company-invite: no clients record found for invited email — proceeding with client_id=null', {
                email,
                company_id: currentUser.company_id
              });
            }
            // Portal database
            const { error: portalUpsertErr } = await portalAdmin.from('client_portal_users').upsert({
            ...
            const { error: crmUpsertErr } = await supabaseAdmin.from('client_portal_users').upsert({
              auth_user_id: portalAuthUserId,
              client_id: clientId,
              company_id: currentUser.company_id,
              email: email,
      ...
      // ── Response ────────────────────────────────────────────────────────────
      if (!emailSent) {
        console.warn('send-company-invite: email delivery failed, returning invite link for manual sharing', {
          invitationId,
    ...
    console.log('[send-company-invite] END — returning success');
    return new Response(JSON.stringify({
      success: true,
      invitation_id: invitationId || null,
      email_sent: true
