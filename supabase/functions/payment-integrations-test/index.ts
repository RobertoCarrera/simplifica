...
serve(async (req) => {
  ...
  try {
    ...
    // Get user profile (id and active status only)
    ...
    if (!userData || !userData.active) {
      return new Response(JSON.stringify({ error: "User inactive or not found" }), {
        status: 403,
        headers: corsHeaders,
      });
    }

    // Verify company membership and role via company_members
