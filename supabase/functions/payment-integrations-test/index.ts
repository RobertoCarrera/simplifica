...
async function testPayPal(
  ...
  try {
    ...
    if (!tokenRes.ok) {
      ...
      console.error('[payment-integrations-test] PayPal auth failed', {
...
serve(async (req) => {
  ...
  const rl = await checkRateLimit(`payment-integrations-test:${ip}`, 10, 60000);
  if (!rl.allowed) {
    ...
    if (provider === 'paypal') {
      if (!credentials.clientId || !credentials.clientSecret) {
        return new Response(
          JSON.stringify({
            error:
              'Credenciales PayPal incompletas. Asegúrate de haber guardado Client ID y Client Secret.',
            details: {
