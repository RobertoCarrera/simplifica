...
serve(async (req) => {
  ...
  try {
    ...
    // company_id → first owner user  (take last entry to avoid duplicates; one owner per company)
    const ownerByCompany = new Map(
      (members || []).map((m) => [m.company_id as string, userById.get(m.user_id as string)]),
    );

    // 6. Send one SES email per company owner
    ...
    for (const [companyId, { logIds, clients }] of byCompany) {
      ...
      const html = `
        <div style="font-family:Arial,sans-serif;font-size:14px;color:#111;max-width:600px;margin:0 auto">
          ...
          <p>Podés reactivar cualquier cliente desde el módulo de <strong>Clientes</strong> en Simplifica.</p>
          <p style="color:#888;font-size:12px;margin-top:32px;border-top:1px solid #eee;padding-top:12px">
            Este es un mensaje automático del sistema Simplifica. Si tenés alguna consulta, respondé a este email.
          </p>
        </div>
      `;
    ...
    if (notifiedLogIds.length > 0) {
      ...
      if (updateErr) {
        // Non-fatal: log will be retried on the next run
