...
serve(
  withCsrf(async (req: Request) => {
    ...
    if (req.method === 'GET') {
      try {
        console.log('import-customers GET health', { origin });
      } catch {}
      return new Response(JSON.stringify({ ok: true, name: 'import-customers' }), {
        status: 200,
    ...
    try {
      ...
      if (!userData.user.email_confirmed_at && !userData.user.confirmed_at) {
        return new Response(
          JSON.stringify({
            error: 'Email not confirmed. Please verify your email before importing customers.',
          }),
      ...
      if (!authoritativeCompanyId) {
        return new Response(
          JSON.stringify({ error: 'Authenticated user has no associated company (forbidden)' }),
          { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
        );
      }
      ...
      console.log('import-customers: begin', {
        count: rows.length,
      ...
      const localityCandidates: Record<
        string,
        { postal_code: string; name: string; province: string | null; country: string | null }
      > = {};
      const parsedAddressByIndex: Record<
        number,
        {
          raw: string;
      ...
      const VIA_REGEX = new RegExp(
        `^(?:${VIA_ALIASES.map((v) => v.replace(/\./g, '\.')).join('|')})$`,
        'i',
      );
      ...
      function parseAddress(raw?: string | null) {
        ...
        const postalMatch = joinedTokens.match(POSTAL_REGEX);
        if (postalMatch) postal = postalMatch[1];
        let street_type: string | null = null,
        ...
        if (firstClauseParts.length) {
          if (VIA_REGEX.test(firstClauseParts[0])) {
            ...
            if (
              firstClauseParts.length > 2 &&
              /\d+[A-Za-z0-9/-]?$/i.test(firstClauseParts[firstClauseParts.length - 1])
            )
              number = firstClauseParts.pop()!;
            street_name = firstClauseParts.slice(1).join(' ') || null;
          } else {
            const numIdx = firstClauseParts.findIndex((p) => /\d+[A-Za-z0-9/-]?$/i.test(p));
            if (numIdx >= 0) {
              street_name = firstClauseParts.slice(0, numIdx).join(' ') || null;
              number = firstClauseParts[numIdx];
            } else {
              street_name = firstClauseParts.join(' ') || null;
            }
          }
        }
        if (tokens.length > 1) {
          for (let i = 1; i < tokens.length; i++) {
            ...
            if (postal && tNoWS.includes(postal)) {
              const locCandidate = tNoWS
                .replace(postal, '')
                .replace(/[()]/g, '')
                .replace(/^[,\s\-–—]+|[,\s\-–—]+$/g, '')
                .trim();
              if (!locality && locCandidate) {
                locality = locCandidate;
                continue;
              }
            }
            ...
            if (!number) {
              const numMatch = t.match(
                /\b(?:nº|no|num|numero|número|num\.|otr|portal|porta)?\s*(\d+[A-Za-z0-9/-]?)/i,
              );
            ...
            complements.push(t);
            if (!locality) locality = t;
            else if (!province) province = t;
            else if (!country) country = t;
          }
        }
        if (locality && postal)
          locality =
        ...
        const parsed = {
          raw: original,
          street_type: street_type ? street_type.toUpperCase() : null,
          street_name: street_name ? street_name.toUpperCase() : null,
          number: number ? number.toUpperCase() : null,
          postal_code: postal,
          locality: locality ? locality.toUpperCase() : null,
          province: province ? province.toUpperCase() : null,
          country: country ? country.toUpperCase() : null,
          composed: composed || original.toUpperCase(),
          complements,
        } as any;
        try {
          console.log('import-customers parseAddress', {
            raw: original,
      ...
      const findAnyField = (obj: any, patterns: RegExp[]) => {
        ...
        for (const p of patterns) {
          for (const k of Object.keys(obj)) {
            if (p.test(k) && obj[k] != null && String(obj[k]).toString().trim() !== '')
              return obj[k];
          }
        }
        ...
        for (const p of patterns) {
          for (const k of Object.keys(lowMap)) {
            if (p.test(k) && lowMap[k] != null && String(lowMap[k]).toString().trim() !== '')
              return lowMap[k];
          }
        }
        return null;
      ...
      const extractAddressParts = (row: any) => {
        const g = (patterns: RegExp[], fallback: any = null) =>
          findAnyField(row, patterns) ?? fallback;
      ...
      const isValidCIF = (val?: string | null) => {
        ...
        return (
          /^[ABCDEFGHJKLMNPQRSUVW]\d{7}[0-9A-J]$/.test(v) || /^[ABCDEFGHJKLMNPQRSUVW]\d{8}$/.test(v)
        );
      };
      ...
      const detectLegalId = (
        val?: string | null,
      ): { type: 'CIF' | 'DNI' | 'NIE' | 'UNKNOWN'; normalized: string | null } => {
        const v = cleanLegalId(val);
      ...
      const BUSINESS_KEYWORD_REGEX =
        /\b(S\.?L\.?U?|S\.?A\.?|SLU|SL|SA|SAS|SRL|GMBH|LTD|LIMITED|INC|CORP|LLC|LLP|PLC|BV|NV|AG|OY|AB|AS|COOP|COOPERATIVA|SCOOP|SOCIEDAD\s+LIMITADA|SOCIEDAD\s+ANONIMA|SLL|SLNE|SC|SNC|EURL|SARL|SASU|UTE|U\.T\.E\.)\b/i;
      ...
      const detectClientType = (
        ...
        const businessName = String(
          row.business_name ||
            findAnyField(row, [/business.*name/i, /razon.*social/i, /company/i, /empresa/i]) ||
            '',
        );
        const cif = row.cif_nif || row.cif || row.nif_empresa || null;
        const probablePersonalCompany =
          businessName && normalizeStr(businessName) === normalizeStr(`${name} ${surname}`);
        if (isValidCIF(cif) && !probablePersonalCompany) return { type: 'business', by: 'cif' };
        const scan = scanRowForBizKeyword(row);
        if (scan && !probablePersonalCompany)
          return { type: 'business', by: 'keyword', keyword: scan.keyword, field: scan.field };
      ...
      for (const r of rows) {
        ...
        let email =
          r.email ||
          r.correo ||
          findAnyField(r, [/(:|\b)email$/i, /^email$/i, /:correo$/i, /correo$/i]) ||
          null;
        let name =
          r.name ||
          r.nombre ||
          findAnyField(r, [/(:|\b)first_name$/i, /(:|\b)name$/i, /first_name$/i, /name$/i]) ||
          '';
        let surname =
          r.surname ||
          r.apellidos ||
          r.last_name ||
          findAnyField(r, [/(:|\b)last_name$/i, /(:|\b)last$/i, /last_name$/i, /apellidos$/i]) ||
          '';
        let phone =
          r.phone ||
          r.telefono ||
          findAnyField(r, [/(:|\b)phone$/i, /telefono$/i, /tel$/i]) ||
          null;
        ...
        const legalRaw =
          r.legal ||
          r['bill_to:legal'] ||
          findAnyField(r, [/(:|\b)legal$/i, /\b(cif|dni|nif|nie)\b/i]) ||
          null;
        ...
        const businessName =
          r.business_name ||
          findAnyField(r, [/business.*name/i, /razon.*social/i, /company/i, /empresa/i]) ||
          null;
        const cifNif =
          r.cif_nif ||
          r.cif ||
          r.nif_empresa ||
          (legalParsed.type === 'CIF' ? legalParsed.normalized : null) ||
          null;
        ...
        if (clientType === 'business') countBusiness++;
        ...
        if (clientType === 'individual') {
          ...
          const hasTradeIndicators =
            tradeName &&
            /(autonom|freelanc|consultor|asesor|studio|estudio|taller)/i.test(String(tradeName));
          const businessMatchesPersonal =
            businessName && norm(businessName) === norm(`${name} ${surname}`);
          if (hasTradeIndicators || businessMatchesPersonal) {
            person_subtype = 'self_employed';
            countSelfEmployed++;
          } else {
            person_subtype = 'consumer';
            countConsumer++;
          }
        }
        ...
        if (clientType === 'business') {
          const finalBusinessName = businessName
            ? sanitizeString(String(businessName))
            : 'Empresa importada';
          row.business_name = finalBusinessName.toUpperCase();
          // Only set cif_nif when we actually have one
          const cNorm =
            cleanLegalId(cifNif) || (legalParsed.type === 'CIF' ? legalParsed.normalized : null);
          ...
          const dNorm =
            legalParsed.type === 'DNI' || legalParsed.type === 'NIE'
              ? legalParsed.normalized
              : cleanLegalId(dni);
        ...
        const hasSomeIdentification =
          !!(row.dni && String(row.dni).trim() !== '') ||
          !!(row.cif_nif && String(row.cif_nif).trim() !== '');
        ...
        if (extracted && (extracted.raw || (!rawAddress && (extracted.cp || extracted.locality)))) {
          ...
          rawAddress =
            extracted.raw ||
            [extracted.cp, extracted.locality, extracted.province, extracted.country]
              .filter(Boolean)
              .join(', ');
        }
        if (!row.direccion_id) {
          if (rawAddress) {
            ...
            if (parsed) {
              if (!parsed.postal_code && extracted.cp)
                parsed.postal_code = String(extracted.cp).replace(/\D+/g, '').slice(0, 5) || null;
              if (!parsed.locality && extracted.locality)
                parsed.locality = String(extracted.locality).toUpperCase();
              if (!parsed.province && extracted.province)
                parsed.province = String(extracted.province).toUpperCase();
              if (!parsed.country && extracted.country)
                parsed.country = String(extracted.country).toUpperCase();
            }
            try {
              console.log('import-customers address-extract', {
                idx: prepared.length,
                rawProvided: !!(
                  r.direccion ||
                  r.address ||
                ...
                composedRaw: extracted.raw ? String(extracted.raw).slice(0, 120) : null,
                cp: parsed?.postal_code || (extracted.cp ? String(extracted.cp) : null),
                locality:
                  parsed?.locality ||
                  (extracted.locality ? String(extracted.locality).toUpperCase() : null),
              });
            } catch {}
            ...
            const cpSeed =
              parsed?.postal_code ||
              (extracted.cp ? String(extracted.cp).replace(/\D+/g, '').slice(0, 5) : null);
            if (cpSeed) {
              const nameSeed =
                parsed?.locality ||
                (extracted.locality ? String(extracted.locality).toUpperCase() : cpSeed);
              if (!localityCandidates[cpSeed]) {
                localityCandidates[cpSeed] = {
                  postal_code: cpSeed,
                  name: nameSeed,
                  province: (parsed?.province ||
                    (extracted.province ? String(extracted.province).toUpperCase() : null)) as
                    | string
                    | null,
                  country: (parsed?.country ||
                    (extracted.country ? String(extracted.country).toUpperCase() : null)) as
                    | string
                    | null,
                };
              }
            ...
            const minimal: any = {
              ...
              street_name: null,
              number: null,
              postal_code: extracted.cp
                ? String(extracted.cp).replace(/\D+/g, '').slice(0, 5)
                : null,
            ...
            parsedAddressByIndex[prepared.length] =
              minimal.postal_code || minimal.locality ? minimal : null;
            if (minimal.postal_code && !localityCandidates[minimal.postal_code]) {
              localityCandidates[minimal.postal_code] = {
                postal_code: minimal.postal_code,
                name: minimal.locality || minimal.postal_code,
                province: minimal.province,
                country: minimal.country,
              };
            }
            try {
              console.log('import-customers address-extract (no raw)', {
                idx: prepared.length,
                cp: minimal.postal_code,
                locality: minimal.locality,
                province: minimal.province,
              });
            } catch {}
          }
        ...
        meta.classification = {
          ...
          field: detected.field || null,
          idType: legalParsed.type,
        };
      ...
      const localityPayloads = Object.values(localityCandidates)
        .map((l) => ({
          ...
          province: l.province ? l.province.toUpperCase() : null,
          country: l.country ? l.country.toUpperCase() : null,
          postal_code: (l.postal_code || '').toString().replace(/\D+/g, ''),
        }))
        .filter((p) => p.postal_code && p.name);
      try {
        console.log('import-customers localityCandidates', {
          count: localityPayloads.length,
          sample: localityPayloads[0],
        });
      } catch {}
        ...
        for (const lp of localityPayloads) {
          ...
          try {
            ...
            if (!rpcErr && rpcData) {
              ...
              console.warn(
                'import-customers RPC insert_or_get_locality error',
                rpcErr.message || rpcErr,
              );
            }
          } catch (e: any) {
            console.warn(
              'import-customers RPC insert_or_get_locality exception',
              e?.message || String(e),
            );
          }
          ...
          if (!gotId) {
            try {
              const { data: upData, error: upErr } = await supabaseAdmin
                .from('localities')
                .upsert(
                  { name: lp.name, province: lp.province, country: lp.country, postal_code: cp },
                  { onConflict: 'postal_code' },
                )
                .select()
                .single();
              if (!upErr && upData?.id) gotId = upData.id;
              else if (upErr)
                console.warn('import-customers locality upsert error', upErr.message || upErr);
            } catch (e: any) {
              console.warn('import-customers locality upsert exception', e?.message || String(e));
            }
          }
      ...
      for (let idx = 0; idx < prepared.length; idx++) {
        ...
        if (!locality_id && parsed.locality) {
          try {
            ...
            const { data: eqByName } = await supabaseAdmin
              .from('localities')
              .select('id,name,province')
              .eq('name', parsed.locality)
              .limit(2);
            if (Array.isArray(eqByName) && eqByName.length === 1 && eqByName[0]?.id) {
              ...
              let q = supabaseAdmin
                .from('localities')
                .select('id,name,province')
                .ilike('name', `%${escapeLike(parsed.locality)}%`)
                .limit(5);
              ...
              if (Array.isArray(likeData) && likeData.length === 1 && likeData[0]?.id) {
                ...
                if (parsed.province) {
                  const exactProv = likeData.find(
                    (l) => (l.province || '').toUpperCase() === parsed.province,
                  );
            ...
            if (!locality_id && parsed.locality) {
              try {
                ...
                if (!insErr && insLoc?.id) {
                  ...
                  try {
                    console.log('import-customers created locality without postal', {
                      idx,
                      name: parsed.locality,
                      province: parsed.province || null,
                    });
                  ...
                  console.warn(
                    'import-customers locality insert (no postal) error',
                    insErr.message || insErr,
                  );
                }
              } catch (eIns: any) {
                console.warn(
                  'import-customers locality insert (no postal) exception',
                  eIns?.message || String(eIns),
                );
              }
            }
            if (!locality_id) {
              try {
                console.log('import-customers skip address: locality not found by name', {
                  idx,
                  locality: parsed.locality,
                  province: parsed.province,
                });
            ...
            console.warn(
              'import-customers locality lookup by name exception',
              e?.message || String(e),
            );
          }
        }
        // If we resolved by name (no cp), count it as processed locality for visibility
        if (!parsed.postal_code && locality_id) {
          localitiesProcessed++;
        }
        if (!locality_id) continue;
        indexToLocalityId[idx] = locality_id;
        const direccion = parsed.composed;
        const numero = parsed.number || null;
        if (direccion && !prepared[idx].direccion_id)
          ...
          try {
            ...
            try {
              ...
              if (!rpcErr && rpcData) {
                ...
                console.warn(
                  'import-customers RPC insert_or_get_address error',
                  rpcErr.message || rpcErr,
                );
              }
            } catch (e: any) {
              console.warn(
                'import-customers RPC insert_or_get_address exception',
                e?.message || String(e),
              );
            }
            ...
            if (rowAddr?.id && (!rowAddr.company_id || rowAddr.company_id !== ap.company_id)) {
              try {
                const { data: upd, error: updErr } = await supabaseAdmin
                  .from('addresses')
                  .update({ company_id: ap.company_id })
                  .eq('id', rowAddr.id)
                  .select()
                  .single();
                ...
                .upsert(
                  {
                    ...
                    numero: ap.numero,
                    locality_id: ap.locality_id,
                    usuario_id: ap.usuario_id,
                    company_id: ap.company_id,
                  },
                  { onConflict: 'direccion,locality_id,usuario_id' },
                )
                .select();
              if (!upErr && upData) {
                rowAddr = Array.isArray(upData) ? upData[0] : upData;
              } else if (upErr) {
                ...
                if (noUnique) {
                  const { data: existing, error: selErr } = await supabaseAdmin
                    .from('addresses')
                    .select('*')
                    .eq('direccion', ap.direccion)
                    .eq('locality_id', ap.locality_id)
                    .eq('company_id', ap.company_id)
                    .maybeSingle();
                  if (!selErr && existing) rowAddr = existing;
                  else {
                  ...
                  console.warn('import-customers address upsert error', upErr.message || upErr);
                }
              }
            }
            if (rowAddr?.id) {
              for (let idx = 0; idx < prepared.length; idx++) {
                ...
                if (!parsed) continue;
                const locIdForIdx =
                  indexToLocalityId[idx] ||
                  (parsed.postal_code ? postalToLocalityId[parsed.postal_code] : undefined);
                if (
                  !prepared[idx].direccion_id &&
                  parsed.composed === ap.direccion &&
                  locIdForIdx === ap.locality_id
                ) {
            ...
            console.warn(
              'import-customers address creation exception',
              addrE?.message || String(addrE),
            );
          }
        }
      }
      ...
      for (let i = 0; i < prepared.length; i += chunkSize) {
        ...
        try {
          ...
          if (error) {
            console.warn('import-customers: batch insert failed, fallback per-row', {
              i,
              count: chunk.length,
            ...
            for (const row of chunk) {
              try {
                ...
                if (oneErr) {
                  ...
                  const code =
                    (oneErr as any)?.code ||
                    (oneErr as any)?.details ||
                    (oneErr as any)?.message ||
                    '';
                  ...
                  if (isDup && row.email) {
                    ...
                    if (!fetchErr && Array.isArray(existing) && existing.length) {
                      ...
                      const isPlaceholder = (v: any) =>
                        typeof v === 'string' &&
                        (/^empresa importada$/i.test(v) || /^cliente importado$/i.test(v));
                      if (
                        row.client_type &&
                        (!current.client_type || current.client_type !== row.client_type)
                      )
                        patch.client_type = row.client_type;
                      if (
                        row.business_name &&
                        (isPlaceholder(current.business_name) || !current.business_name)
                      )
                        patch.business_name = row.business_name;
                      if (row.trade_name && !current.trade_name) patch.trade_name = row.trade_name;
                      if (row.cif_nif && (!current.cif_nif || /^B99999999$/i.test(current.cif_nif)))
                        patch.cif_nif = row.cif_nif;
                      if (row.name && (isPlaceholder(current.name) || !current.name))
                        patch.name = row.name;
                      if (
                        row.apellidos &&
                        (!current.apellidos || /^Apellidos$/i.test(current.apellidos))
                      )
                        patch.apellidos = row.apellidos;
                      if (row.metadata)
                        patch.metadata = { ...(current.metadata || {}), ...(row.metadata || {}) };
                      if (Object.keys(patch).length) {
                        const { data: upd, error: updErr } = await supabaseAdmin
                          ...
                          .limit(1);
                        const final =
                          !updErr && Array.isArray(upd) && upd.length ? upd[0] : current;
                        inserted.push(final);
                      } else {
                    ...
                    errors.push({ error: oneErr.message || oneErr, row });
                    errorsMap[oneErr.message || String(oneErr)] =
                      (errorsMap[oneErr.message || String(oneErr)] || 0) + 1;
                  }
          ...
          console.warn('import-customers: chunk exception, fallback per-row', {
            i,
            count: chunk.length,
          ...
          for (const row of chunk) {
            try {
              ...
              if (oneErr) {
                ...
                if (isDup && row.email) {
                  ...
                  if (!fetchErr && Array.isArray(existing) && existing.length) {
                    ...
                    if (row.name && (isPlaceholder(current.name) || !current.name))
                      patch.name = row.name;
                    if (row.surname && (!current.surname || /^Apellidos$/i.test(current.surname)))
                      patch.surname = row.surname;
                    if (row.metadata)
                      patch.metadata = { ...(current.metadata || {}), ...(row.metadata || {}) };
                    if (Object.keys(patch).length) {
                      const { data: upd, error: updErr } = await supabaseAdmin
              ...
              errorsMap[ee?.message || String(ee)] =
                (errorsMap[ee?.message || String(ee)] || 0) + 1;
            }
          }
        }
      }
      ...
      try {
        console.log('import-customers: done', {
          inserted: inserted.length,
          errors: errors.length,
          localitiesProcessed,
          addressesProcessed,
          addressResolutionMs: addrSpentMs,
        });
      } catch {}
      return new Response(
        JSON.stringify({
          ...
          summary: {
            inserted: inserted.length,
            errors: errors.length,
            errorTypes: errorsMap,
            classification: {
              business: countBusiness,
              individual: countIndividual,
              self_employed: countSelfEmployed,
