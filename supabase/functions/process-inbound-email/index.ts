    ...
    try {
      ...
      if (data.attachments && Array.isArray(data.attachments) && data.attachments.length > 0) {
        for (const att of data.attachments) {
          try {
            const fileName = `${crypto.randomUUID()}_${att.filename || 'attachment'}`;
            ...
            if (!uploadErr) {
              ...
              if (lastMsg) {
                await supabaseClient.from('mail_attachments').insert({
            ...
            console.warn('Failed to save attachment:', att.filename, attErr);
          }
        }
      }
