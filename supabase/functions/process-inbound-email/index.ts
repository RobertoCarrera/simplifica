...
serve(async (req) => {
    ...
    try {
        ...
        if (action === 'reprocess' && s3_key) {
            ...
            data = {
                to: parsed.to?.value[0]?.address || extractEmail(Deno.env.get('AWS_DEFAULT_RECIPIENT') || ''), // Fallback
                from: {
        ...
        try {
            ...
            await supabaseClient.from('inbound_email_audit').insert({
                ...
                subject: subject,
                status: auditStatus,
                error_message: auditError ? (auditError + (action === 'reprocess' ? ' [Recovery Attempt]' : '')) : (action === 'reprocess' ? '[Recovery Attempt]' : null)
            });
        }
