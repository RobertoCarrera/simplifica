...
// booking transitions to status='confirmed' + payment_status='paid'.
//
// Flow:
...
//  5. Create Holded salesreceipt document
//  6. Update bookings.holded_invoice_id (idempotency lock)
//
...
serve(async (req) => {
  ...
  try {
    ...
    if (booking.status !== 'confirmed' || booking.payment_status !== 'paid') {
      console.log('[holded-booking-invoice] Booking not paid+confirmed, skipping');
      return new Response(JSON.stringify({ skipped: true, reason: 'not paid+confirmed' }), { status: 200, headers });
    }
    ...
    /* ── 8. Create salesreceipt in Holded ────────────────── */
    const holdedDoc = await holdedPost('/documents/salesreceipt', docPayload, apiKey);
    ...
    /* ── 9. Persist holded_invoice_id on the booking ────── */
    ...
    console.log(`[holded-booking-invoice] ✓ Created salesreceipt ${holdedId} for booking ${booking_id}`);
