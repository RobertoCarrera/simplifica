...
serve(async (req: Request) => {
  ...
  const payload: any = {
    ...
    description: (
      ...
      (body.p_initial_attachment_url && /^https?:\/\/.{1,2000}$/.test(body.p_initial_attachment_url)
  ...
  try {
    ...
    const merged = new Map<
      string,
      { service_id: string; variant_id: string | null; quantity: number; unit_price: number | null }
    >();
