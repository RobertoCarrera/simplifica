...
serve(
  withCsrf(async (req: Request) => {
    ...
    const payload: any = {
      ...
      description: (
        (body.p_description || '').toString().trim().substring(0, 10000) +
        (body.p_initial_attachment_url &&
        /^https?:\/\/.{1,2000}$/.test(body.p_initial_attachment_url)
          ? `\n\n![Adjunto](${body.p_initial_attachment_url})`
          : '')
      ).trim(),
    ...
    try {
      ...
      if (rawServices.length > 500) throw new Error('Too many services');
        ...
        .map((s: any) => ({
          ...
          unit_price:
            typeof s?.unit_price === 'number'
              ? Math.min(999999.99, Math.max(0, s.unit_price))
              : null,
        }))
      ...
      if (rawProducts.length > 500) throw new Error('Too many products');
        ...
        .map((p: any) => ({
          ...
          unit_price:
            typeof p?.unit_price === 'number'
              ? Math.min(999999.99, Math.max(0, p.unit_price))
              : null,
        }))
      ...
      if (services.length === 0 && products.length === 0) {
        return jsonResponse(
          400,
          { error: 'At least one service or product is required', code: 'no_line_items' },
          origin || '*',
        );
      }
      ...
      if (!isStaff && !actingClientId) {
        console.warn(
          `[${FUNCTION_NAME}] User is neither Staff nor Client for company ${payload.company_id}`,
        );
      ...
      if (!isStaff && actingClientId) {
        if (payload.client_id !== actingClientId) {
          return jsonResponse(
            403,
            { error: 'Clients can only create tickets for themselves', code: 'client_mismatch' },
            origin || '*',
          );
        }
      }
      ...
      if (services.length > 0) {
        ...
        async function tryInsertTicketServices(
          baseRows: any[],
      ...
      if (products.length > 0) {
        ...
        async function tryInsertTicketProducts(
          baseRows: any[],
        ): Promise<{ ok: boolean; err?: any }> {
      ...
      if (body.p_initial_comment) {
        try {
          ...
          if (commentContent) {
            const { error: commentErr } = await supabaseAdmin.from('ticket_comments').insert({
              ticket_id: inserted.id,
      ...
      if (body.p_device_id) {
        try {
          const { error: devErr } = await supabaseAdmin.from('ticket_devices').insert({
            ticket_id: inserted.id,
