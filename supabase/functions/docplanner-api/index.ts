...
//
// Actions:
//   save-credentials   — Encrypt & store client_id + client_secret, test connection
//   test-connection     — Verify stored credentials work
//   disconnect          — Remove integration
//   get-facilities      — List facilities from DocPlanner
//   get-doctors         — List doctors for a facility
//   get-addresses       — List addresses (locations) for a doctor
//   get-bookings        — Get bookings for a doctor/address in date range
//   save-config         — Save facility, doctor mappings, sync settings
//   sync-bookings       — Pull bookings from DocPlanner → Simplifica
//
// Security:
...
async function getValidToken(
  ...
  // Need to refresh — decrypt credentials first
 ...
 * belongs to the doctor — using another doctor's address yields a 403.
 */
...
async function handleSaveCredentials(
  ...
  try {
    ...
    return jsonResponse(400, {
      error: 'Invalid credentials — could not authenticate with DocPlanner',
      detail: String(e),
    });
  }
...
async function handleSyncBookings(
  ...
  if (!integration.facility_id) {
    return jsonResponse(400, { error: 'No facility selected — configure the integration first' });
  }
  ...
  // NOTE: do NOT auto-activate professionals here — some may be intentionally inactive
...
/* ── Google Calendar sync for DocPlanner bookings ────────── */
/**
...
/* ── Company owner OAuth token helper ───────────────── */
...
/* ── Booking upsert from DocPlanner ──────────────────────── */
async function upsertBookingFromDP(
  ...
  // ── Client deduplication (cascade: email → phone → name) ──────────────
  // Note: DocPlanner booking API does NOT return patient.id, so we deduplicate
  // by email → phone → create new. We store dp-dedup key in docplanner_patient_id
  ...
  if (hasPatientData) {
    ...
    // Composite dedup key: phone digits → email → name|surname (mirrors handleImportPatients)
...
/* ── Import patients from DocPlanner ────────────────────── */
/**
...
async function handleImportPatients(
  ...
  const mappings = integration.doctor_mappings || [];
  if (!mappings.length) {
    return jsonResponse(400, { error: 'No doctor mappings configured — import doctors first' });
  }

  const token = await getValidToken(serviceClient, companyId);
  const tagId = await ensureDocplannerTag(serviceClient, companyId);

  // Build monthly date chunks: past 2 years → future 6 months
  const now = new Date();
  ...
  for (const mapping of mappings) {
    if (!mapping.dp_doctor_id) {
      diagnostics.push(`Mapping sin dp_doctor_id (professional_id=${mapping.professional_id}) — omitido`);
      skippedMappings++;
      continue;
    }
    ...
    if (!addressId) {
      try {
        addressId = await refreshMappingAddress(
          serviceClient, companyId, token, integration.facility_id, mappings, mapping.dp_doctor_id);
      } catch (_) { /* ignore — will be flagged below */ }
    }

    if (!addressId) {
      diagnostics.push(`Doctor ${mapping.dp_doctor_id} sin address_id (no se pudo resolver) — omitido`);
      skippedMappings++;
      continue;
    }
  ...
  return jsonResponse(200, {
    ...
    errors: diagnostics,
    message: `${imported} paciente(s) importado(s), ${tagged} etiquetado(s) de ${seen.size} únicos encontrados (${totalBookingsFetched} reservas escaneadas)`,
  });
}

/* ── Resolve correct addresses for all mapped doctors ────── */
/**
  ...
  for (const mapping of mappings) {
    ...
    try {
      ...
      if (newAddr === mapping.address_id) {
        ...
        details.push(`${mapping.dp_doctor_name || mapping.dp_doctor_id}: ${mapping.address_id || '(empty)'} → ${newAddr}`);
        mapping.address_id = newAddr;
        resolved++;
      }
    } catch (e) {
      details.push(`${mapping.dp_doctor_name || mapping.dp_doctor_id}: Error — ${String(e)}`);
      failed++;
    }
  }
...
/* ── Import doctors from DocPlanner ──────────────────────── */
  ...
  for (const doctor of doctors) {
    ...
    if (existingByDpId) {
      ...
      if (byName) {
        ...
        // Step 3: Create new professional (user_id null — external import)
...
serve(async (req) => {
  ...
  // Use service role to bypass RLS on company_members — JWT was already verified above
  const serviceClient = createClient(SUPABASE_URL, SERVICE_ROLE_KEY);

  // Resolve internal user ID (public.users.id ≠ auth.uid() — auth UUID lives in auth_user_id)
  ...
  if (!roleName || !['owner', 'admin'].includes(roleName)) {
    ...
    return jsonResponse(403, { error: 'Insufficient permissions — requires owner or admin role' });
  }
