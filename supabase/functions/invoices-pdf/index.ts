...
function computeLine(item: any, settings: TaxSettings) {
  ...
  const taxRate = settings.iva_enabled
    ? Number(item?.tax_rate ?? item?.vat_rate ?? settings.iva_rate ?? 0)
    : 0;
...
async function generateInvoicePdf(payload: {
  invoice: any;
  items: any[];
  client: any;
  company: any;
  ...
  const companyNif =
    company?.nif || company?.vat_number || company?.tax_id || company?.cif || company?.vat || null;
  if (companyNif) {
  ...
  const clientNif =
    client?.nif || client?.vat_number || client?.tax_id || client?.cif || client?.vat || null;
  if (clientNif) {
    clientInfo.push({ text: `NIF: ${clientNif}`, fontSize: 9, color: TEXT_LIGHT });
  }

  const clientAddr =
    client?.address?.line1 || client?.address?.street || client?.address_text || '';
  if (clientAddr) {
  ...
  // Generar QR code
  // Siempre que tengamos datos suficientes, PRIORIZAMOS el QR VeriFactu oficial (URL AEAT)
  let qrText: string | undefined;
  const nifForQr = company?.nif || companyNif || undefined;
  const hashForQr = meta?.chained_hash || invoice?.verifactu_hash || meta?.hash || undefined;
  ...
  if (!qrText) {
    ...
    if (seriesForQr && numberForQr && hashForQr) {
      ...
      qrText =
        meta?.qr_payload || `SERIE:${seriesForQr}|NUM:${numberForQr}|HASH:${hashForQr || 'N/A'}`;
    }
  }
      ...
      // Totales
      {
        columns: [
          { width: '*', text: '' },
          {
            ...
            stack: [
              {
                canvas: [{ type: 'rect', x: 0, y: 0, w: 220, h: 90, r: 4, color: SECONDARY_COLOR }],
              },
              {
                margin: [12, -85, 12, 10],
                stack: [
                  ...
                  discountTotalNet > 0
                    ? {
                        columns: [
                          ...
                          {
                            text: formatMoney(subtotalBeforeDiscount, invoice?.currency),
                            fontSize: 9,
                            alignment: 'right',
                            color: TEXT_LIGHT,
                          },
                  ...
                  discountTotalNet > 0
                    ? {
                        columns: [
                          ...
                          {
                            text: `- ${formatMoney(discountTotalNet, invoice?.currency)}`,
                            fontSize: 9,
                            alignment: 'right',
                            color: '#2E8B57',
                          },
                  ...
                  },
                  {
                    columns: [
                      {
                        text: `IVA (${isFinite(invoice?.iva_rate) ? invoice.iva_rate : settings.iva_rate}%):`,
                        fontSize: 10,
                        color: TEXT_DARK,
                      },
                      {
                  ...
                  irpfToShow > 0
                    ? {
                        columns: [
                          {
                            text: `IRPF (${isFinite(invoice?.irpf_rate) ? invoice.irpf_rate : settings.irpf_rate}%):`,
                            fontSize: 10,
                            color: TEXT_DARK,
                          },
                          {
...
serve(async (req) => {
  ...
  try {
    ...
    const series = (meta?.series || invoice?.invoice_series || 'SER').toString();
    const number = (meta?.number || invoice?.invoice_number || '00001').toString();
    const companyId = invoice.company_id;

    const { data: appSettings } = await admin.from('app_settings').select('*').maybeSingle();
    ...
    const settings: TaxSettings = {
      ...
      iva_enabled: Boolean(
        companySettings?.iva_enabled ?? appSettings?.default_iva_enabled ?? true,
      ),
      iva_rate: Number(companySettings?.iva_rate ?? appSettings?.default_iva_rate ?? 21),
      irpf_enabled: Boolean(
        companySettings?.irpf_enabled ?? appSettings?.default_irpf_enabled ?? false,
      ),
      irpf_rate: Number(companySettings?.irpf_rate ?? appSettings?.default_irpf_rate ?? 0),
    };
    ...
    if (!force) {
      ...
      if (
        (exists || []).find(
          (f) =>
            f.name === `${series}-${number}.pdf` || f.name === `${series}/${series}-${number}.pdf`,
        )
      ) {
