...
function generateQuotePdf(ctx: {
  ...
  (items || []).forEach((it, idx) => {
    ...
    tableBody.push([
      ...
      },
      {
        ...
        alignment: 'center',
        fontSize: 9,
        color: discountShow !== '-' ? ACCENT_COLOR : TEXT_LIGHT,
        fillColor: idx % 2 === 0 ? LIGHT_GRAY : null,
      },
      {
  ...
  const taxPercent =
    subtotal > 0 ? ((taxAmount / subtotal) * 100).toFixed(1) : String(settings.iva_rate);
                ...
                stack: [
                  ...
                  discountTotalNet > 0
                    ? {
                        columns: [
                          ...
                          {
                            text: money(subtotalBeforeDiscount, quote?.currency),
                            fontSize: 9,
                            alignment: 'right',
                            color: TEXT_LIGHT,
                          },
                  ...
                  discountTotalNet > 0
                    ? {
                        columns: [
                          ...
                          {
                            text: `- ${money(discountTotalNet, quote?.currency)}`,
                            fontSize: 9,
                            alignment: 'right',
                            color: ACCENT_COLOR,
                          },
                    ...
                    : {},
                  {
                    columns: [
                      ...
                      {
                        text: money(subtotal, quote?.currency),
                        fontSize: 10,
                        alignment: 'right',
                        color: TEXT_DARK,
                        bold: discountTotalNet > 0,
                  ...
                  },
                  {
                    columns: [
                      ...
                      {
                        text: money(taxAmount, quote?.currency),
                        fontSize: 10,
                        alignment: 'right',
                        color: TEXT_DARK,
                      },
                  ...
                  settings.irpf_enabled
                    ? {
                        columns: [
                          {
                            text: `IRPF (${settings.irpf_rate}%):`,
                            fontSize: 10,
                            color: TEXT_DARK,
                          },
                          {
                            text: `- ${money(irpfAmount, quote?.currency)}`,
                            fontSize: 10,
                            alignment: 'right',
                            color: TEXT_DARK,
                          },
                  ...
                  },
                  {
                    columns: [
                      ...
                      {
                        text: money(totalAmount, quote?.currency),
                        fontSize: 15,
                        bold: true,
                        alignment: 'right',
                        color: ACCENT_COLOR,
...
serve(async (req) => {
  ...
  try {
    ...
    const settings: TaxSettings = {
      pricesIncludeTax: Boolean(
        companySettings?.pricesIncludeTax ??
    ...
    const year = (quote?.quote_date || new Date().toISOString()).substring(0, 4);
    ...
    // quotes/<companyName>/<year>/<fileName>
    const path = `${safeCompanyName}/${year}/${fileName}`;

    // Check if PDF already exists (unless force=1)
    if (!force) {
      const { data: exists } = await admin.storage
        .from(bucket)
        .list(`${safeCompanyName}/${year}`, { search: fileName });
