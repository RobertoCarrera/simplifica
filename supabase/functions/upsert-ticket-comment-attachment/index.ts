...
function validateInput(body: Record<string, any>) {
  ...
  if (invalid.length) {
    return {
      ok: false,
      status: 400,
      error: `Only canonical p_* keys are allowed. Invalid: ${invalid.join(', ')}`,
    };
  }
...
serve(async (req) => {
  ...
  const rl = await checkRateLimit(`upsert-ticket-comment-attachment:${ip}`, 30, 60000);
  if (!rl.allowed) {
    return json(
      429,
      { error: 'Too many requests' },
      new Headers({ ...Object.fromEntries(cors), ...getRateLimitHeaders(rl) }),
    );
  }
