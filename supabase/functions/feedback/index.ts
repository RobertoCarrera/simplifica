...
serve(async (req) => {
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // FIND SUPERADMIN TO DELIVER FEEDBACK TO
  // ALL feedback goes to the global superadmin — no exceptions
  // ─────────────────────────────────────────────────────────────────────────────
  const SUPER_ADMIN_ROLE_ID = '99da6ecd-ffab-4a2a-84ac-3fd21ea66197';

  // Find the global superadmin (company_id = null)
  const { data: globalSuperadmin, error: globalError } = await supabaseAdmin
    .from('users')
    .select('id, email, app_role_id')
    .is('company_id', null)
    .eq('app_role_id', SUPER_ADMIN_ROLE_ID)
    .limit(1)
    .maybeSingle();

  if (globalError || !globalSuperadmin) {
    console.error('[feedback] No global superadmin found');
    await supabaseAdmin.from('company_feedback').update({ status: 'failed' }).eq('id', feedbackData.id);
    return new Response(JSON.stringify({ error: 'No superadmin configured. Cannot deliver feedback.' }), {
      status: 500,
      headers: { ...getCorsHeaders(req), 'Content-Type': 'application/json' },
    });
  }

  const { data: superadminAcc, error: superadminAccError } = await supabaseAdmin
    .from('mail_accounts')
    .select('id, email, user_id, provider')
    .eq('user_id', globalSuperadmin.id)
    .eq('is_active', true)
    .maybeSingle();

  if (superadminAccError || !superadminAcc) {
    console.error('[feedback] Superadmin has no active mail_account:', superadminAccError);
    await supabaseAdmin.from('company_feedback').update({ status: 'failed' }).eq('id', feedbackData.id);
    return new Response(JSON.stringify({ error: 'Superadmin has no webmail account configured.' }), {
      status: 500,
      headers: { ...getCorsHeaders(req), 'Content-Type': 'application/json' },
    });
  }

  const superadminMailAccount = superadminAcc;

  // Fallback: find any active SES mail_account (prefer non-sender accounts)
  if (!superadminMailAccount) {
    ...
    if (!fallbackError && fallbackAccounts && fallbackAccounts.length > 0) {
      // Filter out sender's own account
      const others = fallbackAccounts.filter((a: any) => a.user_id !== userId);
      if (others.length > 0) {
        superadminMailAccount = others[0];
      } else if (fallbackAccounts.length > 0) {
        // Fallback to any if all are the sender
        superadminMailAccount = fallbackAccounts[0];
      }
    }
  }

  if (!superadminMailAccount) {
    console.error('[feedback] No suitable mail_account found for delivery');
    await supabaseAdmin.from('company_feedback').update({ status: 'failed' }).eq('id', feedbackData.id);
    return new Response(JSON.stringify({ error: 'No webmail account configured for superadmin. Please configure a webmail account.' }), {
      status: 400,
      headers: { ...getCorsHeaders(req), 'Content-Type': 'application/json' },
    });
  }

  // Find inbox folder for the superadmin's mail_account
  const { data: inboxFolder, error: inboxError } = await supabaseAdmin
    .from('mail_folders')
    .select('id')
    .eq('account_id', superadminMailAccount.id)
    .eq('system_role', 'inbox')
    .single();

  if (inboxError || !inboxFolder) {
    console.error('[feedback] Superadmin inbox not found:', inboxError);
    await supabaseAdmin.from('company_feedback').update({ status: 'failed' }).eq('id', feedbackData.id);
  ...
  // Create thread in superadmin's account
  ...
  const { data: thread, error: threadError } = await supabaseAdmin
    .from('mail_threads')
    .insert({
      account_id: superadminMailAccount.id,
  ...
  // Insert mail_message into the superadmin's inbox
  // FROM: the actual sender's userEmail (so superadmin sees who sent it)
  // TO: the superadmin's mail_account email
  // reply_to: the sender's email so superadmin can reply directly
  const { data: mailMessage, error: mailMsgError } = await supabaseAdmin
    ...
    .insert({
      ...
      thread_id: thread.id,
      from: { name: userEmail.split('@')[0], email: userEmail },
      to: [{ email: superadminMailAccount.email, name: superadminMailAccount.email.split('@')[0] }],
      subject: threadSubject,
      body_text: textBody,
      body_html: htmlBody,
      snippet: description.trim().substring(0, 100),
