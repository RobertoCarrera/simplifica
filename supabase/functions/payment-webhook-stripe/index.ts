...
serve(async (req) => {
  ...
  try {
    ...
    // SECURITY: Fail closed if secret or signature is missing
    if (!integration?.webhook_secret_encrypted || !stripeSignature) {
      console.error("[stripe-webhook] Missing webhook secret or signature");
      return new Response(JSON.stringify({ error: "Unauthorized: Missing signature or configuration" }), { status: 401, headers });
    }
