...
async function tryEmitToVerifactu(
  ...
  try {
    ...
    if (!settings?.is_active || !settings?.auto_emit) {
      console.log(
        '[stripe-webhook] Verifactu not enabled or auto_emit off for company:',
        companyId,
      );
      return;
    }
...
serve(async (req) => {
  ...
  const rateLimit = await checkRateLimit(`payment-webhook-stripe:${ip}`, 30, 60000);
  ...
  try {
    ...
    if (invErr || !invoice) {
      console.error(
        '[stripe-webhook] Invoice not found for token:',
    ...
    // SECURITY: Always verify webhook signature — reject if missing
    if (!stripeSignature) {
    ...
    if (!integration?.webhook_secret_encrypted) {
      console.error(
        '[stripe-webhook] No webhook_secret configured for company:',
        invoice.company_id,
      );
      return new Response(
        JSON.stringify({ error: 'Webhook secret not configured for this company' }),
        { status: 500, headers },
      );
    }
    {
      ...
      if (!webhookSecret) {
        ...
        return new Response(JSON.stringify({ error: 'Webhook secret decryption failed' }), {
          status: 500,
          headers,
        });
      }
    ...
    switch (event.type) {
      ...
      case 'payment_intent.succeeded': {
        ...
        if (invoice.total != null && Math.abs(amount - invoice.total) > 0.01) {
          console.error(
            `[stripe-webhook] Amount mismatch: received ${amount}, expected ${invoice.total} for invoice ${invoice.id}`,
          );
          return new Response(
