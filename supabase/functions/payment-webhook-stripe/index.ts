...
serve(async (req) => {
  ...
  try {
    ...
    if (!integration) {
      console.error("[stripe-webhook] No active integration for company:", invoice.company_id);
      return new Response(JSON.stringify({ error: "Integration not active" }), { status: 404, headers });
    }

    if (!integration.webhook_secret_encrypted) {
      console.error("[stripe-webhook] Missing webhook secret for company:", invoice.company_id);
