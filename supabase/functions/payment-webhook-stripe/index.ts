...
if (!ENCRYPTION_KEY) {
  console.error("[payment-webhook-stripe] Missing ENCRYPTION_KEY");
}
...
serve(async (req) => {
  ...
  try {
    ...
    // FAIL CLOSED: Verify webhook signature
    // If integration missing, secret missing, or signature missing -> REJECT
    if (!integration) {
      console.error("[stripe-webhook] No active Stripe integration found for company:", invoice.company_id);
      return new Response(JSON.stringify({ error: "Integration not configured" }), { status: 403, headers });
    }
