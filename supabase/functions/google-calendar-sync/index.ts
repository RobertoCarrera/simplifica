...
serve(async (req)=>{
  ...
  try {
    ...
    // Skip silent CSV imports: bookings stamped with source='csv-doctoralia'
    // come from the bulk Doctoralia import wizard. We do NOT want those
    // to create events in Google Calendar, nor overwrite an existing
    // google_event_id. Short-circuit before any external API call.
    if (booking.source === 'csv-doctoralia') {
      console.log('Skipping Calendar sync for csv-doctoralia booking', booking.id);
      return new Response(JSON.stringify({
        message: 'Skipped: csv-doctoralia import'
      }), {
        headers: corsHeaders
      });
    }
    ...
    if (!professionalId) {
      ...
      return new Response(JSON.stringify({
        message: 'Skipped: No professional'
      }), {
        headers: corsHeaders
      });
    }
    // 1. Get Integration Tokens
    const { data: integration, error: intError } = await supabaseClient.from('integrations').select('*').eq('user_id', professionalId).eq('provider', 'google_calendar').maybeSingle();
    if (intError || !integration) {
      console.log('No Google Calendar integration found for professional', professionalId);
      return new Response(JSON.stringify({
        message: 'Skipped: No integration'
      }), {
        headers: corsHeaders
      });
    }
    ...
    const { data: bookingDetails } = await supabaseClient.from('bookings').select(`
                *,
