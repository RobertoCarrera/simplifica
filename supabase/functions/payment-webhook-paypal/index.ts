...
serve(async (req) => {
  ...
  try {
    ...
    if (!integration || !integration.webhook_secret_encrypted || !integration.credentials_encrypted) {
      console.error("[paypal-webhook] Integration not configured or secrets missing");
      return new Response(JSON.stringify({ error: "Integration configuration error" }), { status: 400, headers });
    }
