...
serve(async (req) => {
  ...
  try {
    ...
    if (!integration) {
      console.error("[paypal-webhook] Integration not found for company:", invoice.company_id);
    ...
    if (!integration.webhook_secret_encrypted || !integration.credentials_encrypted) {
      console.error("[paypal-webhook] Missing credentials/secrets for company:", invoice.company_id);
