...
const PAYMENT_COMPLETED_EVENTS = ['CHECKOUT.ORDER.APPROVED', 'PAYMENT.CAPTURE.COMPLETED'];

const PAYMENT_FAILED_EVENTS = ['PAYMENT.CAPTURE.DENIED', 'PAYMENT.CAPTURE.DECLINED'];

const REFUND_EVENTS = ['PAYMENT.CAPTURE.REFUNDED'];
 ...
 */
async function tryEmitToVerifactu(
  supabase: any,
  invoiceId: string,
  companyId: string,
): Promise<void> {
  try {
    ...
    if (!settings?.is_active || !settings?.auto_emit) {
      console.log(
        '[paypal-webhook] Verifactu not enabled or auto_emit off for company:',
        companyId,
      );
      return;
    }
...
serve(async (req) => {
  ...
  const rateLimit = await checkRateLimit(`payment-webhook-paypal:${ip}`, 30, 60000);
  ...
  try {
    ...
    if (invErr || !invoice) {
      console.error(
        '[paypal-webhook] Invoice not found for token:',
        paymentLinkToken?.slice(0, 8) + '...',
      );
    ...
    if (!integration?.webhook_secret_encrypted || !integration?.credentials_encrypted) {
      console.error(
        '[paypal-webhook] Missing webhook secret or credentials for company:',
        invoice.company_id,
      );
    ...
    if (PAYMENT_COMPLETED_EVENTS.includes(event.event_type)) {
      ...
      if (invoice.total != null && Math.abs(amount - invoice.total) > 0.01) {
        console.error(
          `[paypal-webhook] Amount mismatch: received ${amount}, expected ${invoice.total} for invoice ${invoice.id}`,
        );
        return new Response(
