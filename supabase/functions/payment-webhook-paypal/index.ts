...
serve(async (req) => {
  ...
  try {
    ...
    if (!integration?.webhook_secret_encrypted || !integration?.credentials_encrypted) {
      console.error("[paypal-webhook] Missing integration secrets or credentials");
      return new Response(JSON.stringify({ error: "Integration configuration error" }), { status: 500, headers });
    }
