...
/* ── Auth guard (v2: apikey | Bearer service_role; no user-JWT path — internal-only) ── */
function assertServiceRole(req: Request): Response | null {
  ...
  if (!authed) {
    return jsonResponse(401, { error: 'Unauthorized — service role or valid apikey required' });
  }
  return null;
}
...
serve(async (req: Request) => {
  ...
  const ip = req.headers.get('x-forwarded-for')?.split(',')[0]?.trim()
          ...
          || 'unknown';
  const rateCheck = await checkRateLimit(`send-budget-notification:${ip}`, 600, 60_000);
  if (!rateCheck.allowed) {
    return new Response(
      JSON.stringify({ error: 'Too many requests' }),
      { status: 429, headers: withSecurityHeaders({ ...getCorsHeaders(req), 'Content-Type': 'application/json', ...getRateLimitHeaders(rateCheck) }) }
    );
  }
