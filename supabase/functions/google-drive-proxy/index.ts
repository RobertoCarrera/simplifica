...
serve(async (req) => {
  ...
  const rl = await checkRateLimit(`google-drive-proxy:${ip}`, 20, 60000);
  if (!rl.allowed) {
  ...
  try {
    ...
    if (isGoogleWorkspaceType) {
      ...
      if (mimeType.includes('document'))
        computedMimeType = 'application/pdf'; // 'application/vnd.openxmlformats-officedocument.wordprocessingml.document';
      else if (mimeType.includes('spreadsheet'))
        computedMimeType = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'; // XLSX
      ...
      if (computedMimeType.includes('pdf') && !fileName.endsWith('.pdf'))
        computedFileName += '.pdf';
      if (computedMimeType.includes('spreadsheet') && !fileName.endsWith('.xlsx'))
        computedFileName += '.xlsx';
    }
