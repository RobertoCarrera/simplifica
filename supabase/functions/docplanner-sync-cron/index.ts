...
/* ── DocPlanner fetch helper ─────────────────────────── */
async function dpFetch(token: string, path: string): Promise<any> {
...
async function getGoogleAccessToken(
  ...
  if (expiresAt.getTime() - now.getTime() < 5 * 60 * 1000) {
    ...
    const encryptedNew = OAUTH_ENCRYPTION_KEY
      ? await encryptGoogleToken(tokens.access_token, OAUTH_ENCRYPTION_KEY)
      : tokens.access_token;
    await serviceClient
      .from('integrations')
      .update({ access_token: encryptedNew, expires_at: newExpiry.toISOString(), updated_at: now.toISOString() })
      .eq('id', integration.id);
...
async function syncBookingToGoogleCalendar(
  ...
  if (!professionalId) { console.warn('[gcal] syncBookingToGoogleCalendar: no professionalId, skipping', { bookingId }); return; }
  ...
  if (!prof?.google_calendar_id) { console.warn('[gcal] syncBookingToGoogleCalendar: professional has no google_calendar_id, skipping', { bookingId, professionalId }); return; }

  if (!ownerUserId) { console.warn('[gcal] syncBookingToGoogleCalendar: no owner found for company, skipping', { bookingId, companyId }); return; }

  console.log('[gcal] syncBookingToGoogleCalendar: fetching token for owner', { bookingId, ownerUserId });
  const accessToken = await getGoogleAccessToken(serviceClient, ownerUserId);
  if (!accessToken) { console.warn('[gcal] syncBookingToGoogleCalendar: no valid access token for owner, skipping', { bookingId, ownerUserId }); return; }

  const calendarId = prof.google_calendar_id;

  const attendees: { email: string }[] = [];
  if (bookingData.customer_email) attendees.push({ email: bookingData.customer_email });

  const eventBody = {
    summary: `${bookingData.customer_name} — ${prof.display_name}`,
    description: bookingData.notes || undefined,
    start: { dateTime: bookingData.start_time, timeZone: 'Europe/Madrid' },
    end: { dateTime: bookingData.end_time || bookingData.start_time, timeZone: 'Europe/Madrid' },
  ...
  try {
    ...
    if (existingGoogleEventId) {
      const res = await fetch(
        `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendarId)}/events/${encodeURIComponent(existingGoogleEventId)}?sendUpdates=none`,
        {
          method: 'PATCH',
...
/* ── Resource (room) Google Calendar sync ───────────── */
async function syncBookingToResourceCalendar(
  ...
  if (!bookingData.resource_id) { console.warn('[gcal] syncBookingToResourceCalendar: no resource_id, skipping', { bookingId }); return; }
  ...
  const { data: resource } = await serviceClient
    ...
    .maybeSingle();
  if (!resource?.google_calendar_id) { console.warn('[gcal] syncBookingToResourceCalendar: resource has no google_calendar_id, skipping', { bookingId, resourceId: bookingData.resource_id }); return; }

  if (!ownerUserId) { console.warn('[gcal] syncBookingToResourceCalendar: no owner found for company, skipping', { bookingId, companyId }); return; }

  console.log('[gcal] syncBookingToResourceCalendar: fetching token for owner', { bookingId, ownerUserId });
  const accessToken = await getGoogleAccessToken(serviceClient, ownerUserId);
  if (!accessToken) { console.warn('[gcal] syncBookingToResourceCalendar: no valid access token for owner, skipping', { bookingId, ownerUserId }); return; }
...
async function upsertBookingFromDP(
  ...
  const { data: existing } = await serviceClient
    .from('bookings')
    .select('id, resource_id, google_event_id, resource_google_event_id')
    .eq('company_id', companyId)
  ...
  if (existing) {
    ...
    resourceId = await assignRoomForBooking(
      serviceClient, companyId, mapping.professional_id,
...
async function processNotificationQueue(
  ...
  const { data: ownerRole } = await serviceClient
    ...
    .eq('name', 'owner')
    .maybeSingle();
  const { data: ownerMember } = ownerRole
    ? await serviceClient
        ...
        .maybeSingle()
    : { data: null };
  const ownerUserId: string | null = ownerMember?.user_id ?? null;
  ...
  try {
    ...
    if (notifications.length === 0) return { synced, failed, errors };
    ...
    for (const notification of notifications) {
      ...
      try {
        ...
        const mapping = mappings.find((m: any) => String(m.dp_doctor_id) === doctorId);

        if (!mapping) {
          // Skip notifications for unmapped doctors
          continue;
        }

        if (['slot-booking', 'booking-confirmed', 'booking-moved'].includes(eventName)) {
          ...
          if (bookingId && facilityId) {
            ...
            const fetchBooking = async (addressId: string) => {
                  ...
                  if (newAddrId) {
                    ...
                    if (retryBooking) {
                      ...
                      const updated = mappings.map((m: any) =>
                        String(m.dp_doctor_id) === String(mapping.dp_doctor_id) ? { ...m, address_id: newAddrId } : m
                      );
          ...
          if (bookingId) {
            const { data: existing } = await serviceClient
              ...
              .select('id')
              .eq('company_id', companyId)
              .eq('docplanner_booking_id', String(bookingId))
              .maybeSingle();
...
/* ── Full booking sync for a company ─────────────────── */
async function syncCompanyBookings(
  serviceClient: any,
  integration: any,
  token: string
): Promise<{ synced: number; failed: number; errors: string[] }> {
    ...
    if (!addressId) {
      try {
        ...
        if (addresses.length > 0) {
          ...
          const updated = mappings.map((m: any) =>
            String(m.dp_doctor_id) === String(mapping.dp_doctor_id) ? { ...m, address_id: addressId } : m
          );
          await serviceClient.from('docplanner_integrations')
            .update({ doctor_mappings: updated, updated_at: new Date().toISOString() })
            .eq('company_id', companyId);
      ...
      if (String(e).includes('403') && String(e).includes('address')) {
        ...
        try {
          ...
          if (newAddr && newAddr !== addressId) {
            ...
            const updated = mappings.map((m: any) =>
              String(m.dp_doctor_id) === String(mapping.dp_doctor_id) ? { ...m, address_id: newAddr } : m
            );
            await serviceClient.from('docplanner_integrations')
              .update({ doctor_mappings: updated, updated_at: new Date().toISOString() })
              .eq('company_id', companyId);
...
serve(async (req) => {
  ...
  if (!integrations || integrations.length === 0) {
    return new Response(JSON.stringify({ message: 'No active auto-sync integrations', processed: 0 }), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }

  const results: Array<{ company_id: string; synced: number; failed: number; errors: string[] }> = [];
  const now = new Date();
  ...
  for (const integration of integrations) {
    ...
    try {
      ...
      const allErrors = [...notifResult.errors, ...syncResult.errors];
