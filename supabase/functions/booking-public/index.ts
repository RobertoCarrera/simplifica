...
function getCorsHeaders(req: Request) {
  ...
  if (origin && allowedOrigins.includes(origin)) {
    // Origin is explicitly in the allowlist
    headers['Access-Control-Allow-Origin'] = origin;
  } else if (IS_LOCAL_DEV && (origin.startsWith('http://localhost:') || origin.startsWith('http://127.0.0.1:'))) {
    // Local development only — never in production
    headers['Access-Control-Allow-Origin'] = origin;
  }
  // All other origins: no Access-Control-Allow-Origin header → browser blocks the request
...
async function verifyTurnstile(token: string, ip: string) {
  ...
  // Only bypass Turnstile in local development (http:// Supabase URL)
  if (IS_LOCAL_DEV) return { success: true };
  // In production/staging, TURNSTILE_SECRET is mandatory
  if (!TURNSTILE_SECRET) {
    console.error('TURNSTILE_SECRET_KEY not configured — blocking request for security');
...
serve(async (req) => {
  ...
  try {
    ...
    // API key is mandatory in production to prevent unauthenticated access
    if (!IS_LOCAL_DEV && !BOOKING_API_KEY) {
      console.error('BOOKING_API_KEY not configured — rejecting request for security');
    ...
    if (
      ...
      // 3. Fetch active professionals for this company (top-level list)
    ...
    if (action === 'create-booking') {
      ...
      try {
        ...
        // Try to link to an existing client by email (best-effort, non-blocking)
        let linkedClientId: string | null = null;
        if (client_email) {
          const { data: existingClient } = await privateSupabase
            .from('clients')
            .select('id')
            .eq('company_id', company.id)
            .ilike('email', client_email.trim())
            .maybeSingle();
          linkedClientId = existingClient?.id ?? null;
        }
