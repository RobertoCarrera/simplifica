...
const PRIVATE_SUPABASE_KEY = Deno.env.get('PRIVATE_SUPABASE_SERVICE_ROLE_KEY') || PUBLIC_SUPABASE_KEY;
...
function madridToUTC(dateStr: string, timeStr: string): Date {
    ...
    const p: Record<string, string> = Object.fromEntries(
        formatter.formatToParts(naiveUTC)
            .filter(x => x.type !== 'literal')
            .map(x => [x.type, x.value])
    );
    const h = p.hour === '24' ? '00' : p.hour;
    // Re-interpret what Madrid shows as if it were UTC → gives the Madrid clock reading in epoch
    const madridEpoch = new Date(`${p.year}-${p.month}-${p.day}T${h}:${p.minute}:${p.second}Z`).getTime();
    ...
    try {
        ...
        if (req.method === 'GET' && url.pathname.includes('/availability')) {
            ...
            if (!weekStart || !/^\d{4}-\d{2}-\d{2}$/.test(weekStart)) {
                return new Response(JSON.stringify({ error: 'Valid week_start (YYYY-MM-DD) required' }), { status: 400, headers: corsHeaders });
            }
            ...
            if (avCalendarUserId && GOOGLE_CLIENT_ID && GOOGLE_CLIENT_SECRET) {
                try {
                    ...
                    if (integration) {
                        let accessToken = ENCRYPTION_KEY && isEncrypted(integration.access_token)
                        ...
                        if (expiresAt.getTime() - Date.now() < 5 * 60 * 1000) {
                            const refreshToken = integration.refresh_token && ENCRYPTION_KEY && isEncrypted(integration.refresh_token)
                        ...
                        const calendarId = integration.settings?.calendar_id_availability
        ...
        if (action === 'create-booking') {
            ...
            if (insertError) {
                ...
                if (insertError.code === '23505') {
                    return new Response(JSON.stringify({ error: 'Ya tienes una reserva para esa fecha y hora' }), { status: 409, headers: corsHeaders });
                }
                throw insertError;
            }
                ...
                if (bookingError) {
                        ...
                        if (calendarUserId && GOOGLE_CLIENT_ID && GOOGLE_CLIENT_SECRET) {
                            ...
                            if (integration) {
                                ...
                                const calendarEvent = {
                                    ...
                                    start: { dateTime: `${requested_date}T${requested_time}:00`, timeZone: 'Europe/Madrid' },
                                    end: { dateTime: `${requested_date}T${addMinutesToTime(requested_time, durationMinutes)}:00`, timeZone: 'Europe/Madrid' },
                                    attendees: [
                                ...
                                if (calResp.ok) {
                                    ...
                                    await privateSupabase.from('bookings')
                                        .update({ google_event_id: createdEvent.id, meeting_link: createdEvent.hangoutLink || null })
                                        .eq('id', newBooking.id);
                            ...
                            console.log('ℹ Calendar skipped — calendarUserId:', calendarUserId, 'GoogleCreds:', !!GOOGLE_CLIENT_ID && !!GOOGLE_CLIENT_SECRET);
                        }
                    } catch (calErr: any) {
                    ...
                    try {
                        ...
                        if (AWS_ACCESS_KEY_ID && AWS_SECRET_ACCESS_KEY) {
                            ...
                            sesParams.append('Message.Subject.Data', `Reserva confirmada: ${serviceName} — ${dateFormatted}`);
                            sesParams.append('Message.Body.Html.Data', clientHtml);
                            sesParams.append('Message.Body.Text.Data', `Hola ${client_name}, tu reserva de ${serviceName} para el ${dateFormatted} a las ${timeFormatted} ha sido confirmada.`);
