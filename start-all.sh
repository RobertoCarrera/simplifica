...
mintty -t "CRM :4200" bash -c "cd /f/simplifica/simplifica-crm && npm run start; exec bash" &

echo "Iniciando Portal  → http://localhost:4201"
mintty -t "Portal :4201" bash -c "cd /f/simplifica/simplifica-portal-frontend && npm run start; exec bash" &

echo "Iniciando Agenda  → http://localhost:4202"
mintty -t "Agenda :4202" bash -c "cd /f/simplifica/simplifica-agenda-frontend && npm run start; exec bash" &
