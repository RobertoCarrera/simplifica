...
const config = {
  ...
  // Forward all inbound emails to this Gmail address for backup/notification
...
export const handler = async (event) => {
  ...
  try {
    ...
    // Fix headers that cause SES to reject forwarded emails:
    // - Remove DKIM/Message-ID/Return-Path (SES regenerates them)
    // - Rewrite From: to a verified SES identity so sandbox doesn't reject it;
    //   original sender is preserved in X-Original-From for reference.
    const safeRawEmailString = rawEmailString
      ...
      .replace(/^From:(.*)$/im, (_, original) =>
        `X-Original-From:${original}\nFrom: Simplifica CRM <noreply@simplificacrm.es>`,
      );

    const safeRawEmailBuffer = Buffer.from(safeRawEmailString, 'utf-8');

    // 2. FORWARD to Gmail (backup/notification copy)
      ...
      try {
        await ses.send(
          new SendRawEmailCommand({
            ...
            Source: recipient, // Recipient domain is verified in SES
          }),
        );
      } catch (sesErr) {
